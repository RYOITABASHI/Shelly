...
| Sidebar Add Repository existence check + Alert on ghost path | 🟡 the check exists (bug #73) but on-device verification 2026-08-17 reproduced the exact ghost path it was meant to prevent: `readDirEntries` returns `[]` on any parent-directory read error (missing, unreadable, or genuinely empty — indistinguishable), and `tryAddRepo` treats that as "accept the add," so a path with a nonexistent parent is added as a repository (displayed with a fabricated version badge, indistinguishable from a real one). It also never checks for a `.git` directory, so an existing non-git folder is accepted too. The ghost entry is session-scoped only — it self-heals on the next app restart via `loadRepos()`'s `find`-based repo rescan, so it isn't a persistent state corruption. Fix in progress |
