...
| **Claude Code + Codex + Gemini on Android** | Shelly ships a known-good Claude Code runtime (2.1.112 pure-JS, Bash tool verified) and optionally tracks newer Claude Code releases after on-device smoke tests. The Claude Code 2.1.113+ Bun SEA is bundled and selectable via `SHELLY_PREFER_MUSL_CLAUDE=1` — Bash tool on the musl SEA is work-in-progress. A Shelly-managed runtime updater (`.shelly-runtime`) promotes only smoke-tested versions and falls back to the bundled golden on failure. **Shelly stable channel** (default, 7-day cooldown heuristic) catches upstream regressions before promotion; `--channel latest` opts into bleeding-edge. Shape detection rejects packaging shifts (non-ELF / non-aarch64 / suspiciously small binaries). Optional `--print` functional check exercises real inference via `SHELLY_UPDATER_FUNCTIONAL_CHECK=1`. No proot, no root. |
