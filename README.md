...
https://github.com/RYOITABASHI/Shelly/raw/main/docs/videos/demo-claude-codex-2pane.mp4

**AI reading a runtime error and suggesting the fix in real time**

https://github.com/RYOITABASHI/Shelly/raw/main/docs/videos/demo-ai-error-review.mp4
...
| **Claude Code + Codex + Gemini on Android** | Shelly keeps all three AI CLIs on managed latest paths without trusting upstream blindly. Claude/Codex native runtime candidates are staged in `.shelly-runtime`, smoke-tested on-device, then promoted by `current/` symlink only after PASS. The npm tier stages `@anthropic-ai/claude-code@latest`, `@google/gemini-cli@latest`, and `@openai/codex@latest`, applies Shelly compatibility hooks, runs `--version` probes, then promotes the tree only if all probes pass. Broken versions are logged and cooled down; offline checks keep the previous stable runtime. No proot, no root. |
