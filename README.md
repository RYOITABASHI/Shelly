...
| Sub-agent fan-out (`parallelGroup`) — branches receive an isolated pre-group context and aggregate results in declared order; unattended PlanSpec branches dispatch concurrently with a 3-branch semaphore and a locked shared capability budget | 🟡 extensively tsc/Jest-tested, including wall-clock concurrency proof and budget-race regression coverage; CI green, but on-device verification has **not** happened yet |
