...
- **Shared companion memory** — tell Shelly “remember that I prefer dark mode”; after confirmation, it writes a `_global` note (`GLOBAL_MEMORY_SCOPE`) recalled by future companion conversations and every scheduled agent. This uses `detectCompanionMemoryWrite`; explicit all-agent commands use `detectGlobalMemoryWrite`.
- **Agent results re-enter chat** — scheduled and **Run Now** completions are added to the shared thread by `lib/agent-companion-notice.ts`, alongside the existing OS notification.
...
| Agent completion chat re-entry — scheduled and **Run Now** results are appended to the shared companion thread by `lib/agent-companion-notice.ts`, in addition to OS notifications and run history | ✅ shipping |
...
| Sub-agent fan-out (`parallelGroup`) — branches receive an isolated pre-group context and aggregate results in declared order; unattended PlanSpec branches dispatch concurrently with a 3-branch semaphore and a locked shared capability budget | 🟡 extensively tsc/Jest-tested, including wall-clock concurrency proof and budget-race regression coverage; CI green, but on-device verification has **not** happened yet and is scheduled for tomorrow (2026-08-15) |
...
| User profile learning — locally observes command/agent-usage patterns for proactive agent suggestions and AI Pane personalization; can be disabled or reset in Settings → Agents; see [Privacy](#privacy) for what leaves the device | ✅ shipping; unit-tested, not yet on-device verified |
