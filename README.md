...
- **Claude Code OAuth — Phase 1 in-app flow (beta)** — Shelly v78+ installs an `xdg-open` shim at `$HOME/bin/xdg-open` and exports `BROWSER` to it, so Claude Code's `/login` (`i3()` opener) routes the auth URL into Shelly's in-app Browser Pane via the `shelly://browser` deep link instead of failing silently with ENOENT. Sign-in workflow: tap **Settings → CLAUDE LOGIN → Sign in with Claude** (or run `claude` then `/login` from any terminal pane), wait for the Browser Pane to load `auth.anthropic.com`, complete the consent screen, copy the `code#state` token shown on Anthropic's success page, then paste it back into the CLI's manual-paste prompt. **What is _not_ yet automated**: the loopback callback (`http://localhost:<random>/callback`) is not intercepted from the in-app WebView, so the localhost listener inside Claude Code never resolves on its own — manual paste is still the completion step. The credential-transplant path below remains as a fallback. Tracked as bug #102 (Phase 2 is the WebView callback intercept; deferred).
- **Gemini CLI OAuth — Phase 1 in-app flow (beta)** — same shim, same workflow, same caveats. Tap **Settings → GEMINI LOGIN → Sign in with Gemini** (or run `gemini` then `/auth`), the OAuth URL opens in the Browser Pane, complete consent on `accounts.google.com`, copy the auth code, paste back into the manual-paste prompt. The loopback redirect (`http://127.0.0.1:<port>/oauth2callback`) is similarly not auto-resolved yet. The previous `EACCES` from `spawn('xdg-open', …)` is gone now that the shim exists. Credential transplant path remains as a fallback. Tracked as bug #115.
