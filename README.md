...
| **Claude Code + Codex + Gemini on Android** | Shelly keeps all three AI CLIs on managed latest paths without trusting upstream blindly. The current verified device build runs Claude Code 2.1.122 via the extracted Bun `cli.js` Node route, Codex 0.125.0-termux with GPT-5.5, and Gemini CLI 0.40.0. Claude falls back to the musl SEA and legacy `cli.js` tiers if needed; Codex native candidates are staged in `.shelly-runtime`, smoke-tested on-device, then promoted by `current/` symlink only after PASS. The npm tier stages `@google/gemini-cli@latest` and `@openai/codex@latest`, applies Shelly compatibility hooks, runs `--version` probes, then promotes the tree only if probes pass. Broken versions are logged and cooled down; offline checks keep the previous stable runtime. No proot, no root. |
...
- **Managed CLI runtime updater** — `shelly-update-clis` downloads verified native Codex builds and updates JS CLI packages, verifies integrity, smoke-tests on-device, then hot-swaps `~/.shelly-runtime/<cli>/current` / `~/.shelly-cli` without an APK update. Claude currently defaults to the CI-extracted Bun `cli.js` route because that passed Bash-tool and paste testing on Galaxy Z Fold6.
...
| Claude Code extracted Bun `cli.js` Node route (default) + musl/legacy fallbacks + Codex CLI verified native runtime + Gemini npm CLI staged/probed before promotion | ✅ managed latest with rollback; build 764 verified Claude 2.1.122, Codex 0.125.0-termux GPT-5.5, Gemini 0.40.0 |
