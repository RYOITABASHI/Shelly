...
| **Claude Code + Codex + Gemini on Android** | Shelly ships a known-good Claude Code runtime (2.1.112 pure-JS, Bash tool verified) and optionally tracks newer Claude Code releases after on-device smoke tests. The Claude Code 2.1.113+ Bun SEA is bundled and selectable via `SHELLY_PREFER_MUSL_CLAUDE=1` — Bash tool on the musl SEA is work-in-progress. If a newer release changes packaging or fails Android compatibility checks, Shelly automatically falls back to the bundled golden version. Codex and Gemini auto-track `@latest` via a 24 h background updater with the same staging/health-check/atomic-promote pipeline. No proot, no root. |
