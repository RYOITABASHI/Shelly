...
export const X_POST_DIRECTIVE =
  '\n\n[出力フォーマット指定：X(旧Twitter)無料版への投稿]\n直前までの内容を、日本語で X 投稿1本に要約してください。文字数は X の重み付けカウントで上限280単位＝全角なら140文字以内に厳守する（日本語・全角=2、半角英数記号=1で計算。例: 全角140字でちょうど上限）。URL は実際の長さに関わらず1つ23文字としてカウントし、改行・ハッシュタグも文字数に含める。安全のため全角130字程度に収めると確実。要点を絞り、投稿本文だけを返す（前置き・後置き・「以下が要約です」等は書かない）。';
