...
export function buildLocalAIPaneSystemPrompt(
  ...
  const parts: string[] = [
    ...
    // Fable5 3rd-pass review (2026-08-29, on-device finding): a free-text
    // "delete the X agent" request (not the exact "@agent delete <name>"
    // syntax the deterministic parser recognizes) falls through to this
    // model, which hallucinated "Agents are system-level components... you
    // can't delete them manually" — false; deleting agents is an ordinary,
    // fully supported action. This is a trust-eroding failure mode (the
    // assistant lying about its own capabilities), not just a missed intent,
    // so it gets an explicit capability fact rather than relying on intent
    // detection alone.
    'Agents CAN be deleted — either by the user tapping delete on the agent\'s row in the Sidebar, or via the exact command "@agent delete <name>". If someone asks you to delete an agent in plain language, tell them to use one of those two ways; never say agents cannot be deleted or that they are unremovable system components.',
  ];
