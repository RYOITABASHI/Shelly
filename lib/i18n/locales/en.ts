/**
 * English locale — base translation file.
 * All keys must be defined here. Other locales fall back to this.
 */
const en: Record<string, string> = {
  // ── Tabs ──────────────────────────────────────────────────────────
  'tab.projects': 'Projects',
  'tab.chat': 'Chat',
  'tab.terminal': 'Terminal',
  'tab.snippets': 'Snippets',
  'tab.obsidian': 'Obsidian',
  'tab.search': 'Search',
  'tab.settings': 'Settings',

  // ── Welcome banner ────────────────────────────────────────────────
  'welcome.subtitle': 'Shelly (Unofficial) — Android Terminal Prototype',
  'welcome.hint_help': "Type 'help' for available commands",
  'welcome.hint_ai': '@codex / @local / @team to direct AI',
  'welcome.hint_natural': 'Natural language input triggers AI tool suggestions',

  // ── Command Palette ───────────────────────────────────────────────
  'palette.search': 'Search commands...',
  'palette.no_results': 'No results',
  'palette.hint_projects': 'Project folders',
  'palette.hint_chat': 'AI chat interface',
  'palette.hint_terminal': 'Raw terminal',
  'palette.hint_snippets': 'Saved commands',
  'palette.hint_browser': 'Built-in browser',
  'palette.hint_obsidian': 'Knowledge management',
  'palette.hint_search': 'History search',
  'palette.hint_settings': 'Settings',
  'palette.clear_terminal': 'Clear Terminal',
  'palette.hint_clear': 'Clear terminal output',
  'palette.new_session': 'New Session',
  'palette.hint_new_session': 'New tab session',
  'palette.restore_tmux': 'Restore tmux Session',
  'palette.hint_restore_tmux': 'List tmux sessions',
  'palette.tmux_attach': 'tmux attach',
  'palette.hint_tmux_attach': 'Attach to last tmux session',
  'palette.single_pane': 'Single Pane',
  'palette.multi_pane': 'Multi Pane',
  'palette.hint_single': 'Return to normal view',
  'palette.hint_multi': 'Side-by-side tab display',

  // ── Quick Terminal ────────────────────────────────────────────────
  'quick.title': 'Quick Terminal',
  'quick.placeholder': 'Drop-down terminal - type a command',
  'quick.running_termux': '(Running...)',
  'quick.completed': '(Completed)',

  // ── Snippets ──────────────────────────────────────────────────────
  'snippets.count': '{{count}} items',
  'snippets.sort_recent': 'Recently Used',
  'snippets.sort_frequency': 'Frequency',
  'snippets.sort_created': 'Created',
  'snippets.search_placeholder': 'Search commands, titles, tags',
  'snippets.loading': 'Loading...',
  'snippets.empty_search': 'No snippets matching "{{query}}"',
  'snippets.empty': 'No snippets yet\nLong-press a command in Terminal → Save Snippet',
  'snippets.export': 'Export (JSON)',
  'snippets.import': 'Import (JSON)',
  'snippets.export_title': 'Export',
  'snippets.export_empty': 'No snippets to export.',
  'snippets.delete_title': 'Delete Snippet',
  'snippets.delete_confirm': 'Delete "{{title}}"?',
  'snippets.cancel': 'Cancel',
  'snippets.delete': 'Delete',

  // ── AI Block ──────────────────────────────────────────────────────
  'action.execute': '▶ Run',
  'action.executing': 'Running...',
  'action.safety_title': '⚠ Confirm Command',
  'action.execute_anyway': 'Execute Anyway',

  'ai.copy': 'Copy',
  'ai.copied': 'Copied',
  'ai.speak': 'Read Aloud',
  'ai.stop': 'Stop',
  'ai.tokens': '{{count}} tokens | {{tps}} tok/s',
  'ai.sources': 'Sources:',
  'ai.retry': 'Retry',
  'ai.ask_other': 'Ask another AI',

  // ── Input Router ──────────────────────────────────────────────────
  'router.suggest': 'AI Suggestion',
  'router.code_reason': 'Excels at code generation & implementation tasks',
  'router.research_reason': 'Excels at research & information gathering',
  'router.chat_reason': 'Handles conversations & Q&A offline',
  'router.default_reason': 'Default for general-purpose tasks',
  'router.research_fallback': 'If research is needed',
  'router.detail_mention': 'Input method: @mention (direct)',
  'router.detail_nl_tool': 'Input method: Natural language + tool name',
  'router.detail_natural': 'Input method: Natural language (tool suggestion)',
  'router.detail_command': 'Input method: Shell command',
  'router.detail_target': 'Target: {{target}}',
  'router.detail_prompt': 'Prompt: {{prompt}}',
  'router.detail_detected': 'Detected tool: {{tool}}',
  'router.detail_hint': 'Hint: {{hint}}',
  'router.mention_hint': 'Use @{{target}} for direct access',
  'router.log_suggest': '[AI Suggestion] {{tool}} recommended — {{text}}',

  // ── Git Guide ─────────────────────────────────────────────────────
  'git.guide_footer': 'For more details: @git help',
  'git.run': 'Run',

  // ── Terminal Block ────────────────────────────────────────────────
  'terminal.running': 'Running...',
  'terminal.exit_code': 'exit {{code}}',
  'terminal.confirm_title': 'Confirm Before Running',
  'terminal.confirm_message': 'Run this command?\n\n$ {{command}}',
  'terminal.confirm_cancel': 'Cancel',
  'terminal.confirm_run': 'Run',

  // ── Common UI ───────────────────────────────────────────────────
  'common.cancel': 'Cancel',
  'common.close': 'Close',
  'common.add': 'ADD',
  'common.clear': 'Clear',
  'common.cannot_undo': 'This cannot be undone.',
  'common.copied_clipboard': 'Copied to clipboard.',
  'common.create': 'Create',
  'common.delete': 'Delete',
  'common.done': 'Done',
  'common.edit': 'EDIT',
  'common.error': 'Error',
  'common.more': 'more',
  'common.not_set': 'Not set',
  'common.not_connected': 'Not connected',
  'common.remove': 'Remove',
  'common.save': 'SAVE',
  'common.set': 'SET',
  'common.share': 'Share',
  'common.back': 'BACK',
  'common.copy': 'Copy',
  'common.terminal_not_connected': 'Terminal is not connected.',

  // ── Settings ──────────────────────────────────────────────────────
  'settings.title': 'Settings',
  'settings.appearance': 'Appearance',
  'settings.syntax_highlight_label': 'Syntax Highlight',
  'settings.color_theme_label': 'Color Theme',
  'settings.terminal_theme_ansi_label': 'Terminal Theme',
  'settings.font_family_label': 'Font Family',
  'settings.gpu_rendering_label': 'GPU Rendering',
  'settings.realtime_voice_label': 'Realtime Voice (Gemini Live)',
  'settings.realtime_voice_desc': 'Full-duplex voice (you can talk over the model) via the Gemini Live API instead of the default record/transcribe/speak flow. Uses your Gemini API key. Audio is billed per-token even on a free-tier key (unlike the default free flow) — off by default.',
  'settings.local_llm_label': 'Local LLM',
  'settings.local_llm_url_label': 'Local LLM URL',
  'settings.local_llm_model_label': 'Local LLM Model',
  'settings.groq_api_key_label': 'Groq API Key',
  'settings.groq_model_label': 'Groq Model',
  'settings.perplexity_api_key_label': 'Perplexity API Key',
  'settings.gemini_api_key_label': 'Gemini API Key',
  'settings.gemini_model_label': 'Gemini Model',
  'settings.autonomous_cloud_label': 'Autonomous Cloud',
  'settings.autonomous_cloud_exhaustion_label': 'On Quota Exhausted',
  'settings.optimistic_writes_label': 'Optimistic Workspace Writes',
  'settings.widget_no_confirm_label': 'Widget No-Confirm Register',
  'settings.a2a_server_label': 'A2A Server',
  'settings.a2a_server_desc': 'Exposes a read-only "list_agents" skill to A2A (Agent2Agent) protocol clients on the same network (Wi-Fi/VPN) — lets a PC-side agent ask what Shelly agents exist. Opens a real HTTP listener while the app is running. No run-triggering skill yet. Default off.',
  'settings.mcp_server_label': 'MCP Server',
  'settings.mcp_server_desc': 'Exposes read-only tools (terminal output, git status, agent list, repo list) to MCP clients (Claude Code, Claude Desktop) on the same network (Wi-Fi/VPN). Requires the bearer token below in the client\'s MCP config. Default off.',
  'settings.mcp_token_copy_label': 'Copy MCP Token',
  'settings.mcp_token_copy_action': 'Copy',
  'settings.mcp_token_copy_desc': 'Copies the bearer token an MCP client needs to authenticate — generates one on first use.',
  'settings.mcp_exec_label': 'MCP: Allow exec/write (dangerous)',
  'settings.mcp_exec_desc': 'Adds two more MCP tools — run_command and write_file — so an MCP client (Claude Code, Codex) can execute commands and write files on this device, not just read from it. Every call still blocks on an in-app approval popup on the phone and is refused outright if classified CRITICAL risk, but this is real remote-code-execution surface protected only by the bearer token above and your own approval taps. Only turn this on if you specifically want that. No effect unless MCP Server is also on. Default off.',
  'settings.mcp_token_copied_toast': 'MCP token copied',
  'settings.mcp_token_copy_failed_title': 'Failed to copy MCP token',
  'settings.llm_led_registration_label': 'LLM-Led Agent Registration',
  'settings.llm_high_risk_label': 'LLM-Proposed Webhook/CLI (High Risk)',
  'settings.telegram_inbound_label': 'Enable Inbound',
  'settings.telegram_bot_token_label': 'Bot Token',
  'settings.telegram_chat_id_label': 'Authorized Chat ID',
  'settings.team_gemini_label': 'Gemini API',
  'settings.team_cerebras_label': 'Cerebras',
  'settings.team_groq_label': 'Groq',
  'settings.team_perplexity_label': 'Perplexity',
  'settings.team_local_label': 'Local LLM',
  'settings.default_agent_label': 'Default Agent',
  'settings.experience_mode_label': 'Experience Mode',
  'settings.cli_approval_level_label': 'CLI Approval Level',
  'settings.sound_effects_label': 'Sound Effects',
  'settings.volume_label': 'Volume',
  'settings.sound_profile_label': 'Sound Profile',
  'settings.custom_context_label': 'Custom System Prompt',
  'settings.realtime_translate_label': 'Realtime Translate',
  'settings.notification_trigger_label': 'Notification Triggers',
  'settings.notification_os_access_label': 'Notification Access (OS)',
  'settings.scouter_enabled_label': 'Scouter Service',
  'settings.scouter_debug_label': 'Scouter Debug Info',
  'settings.scouter_hook_label': 'Scouter Hook Template',
  'settings.rerun_setup_label': 'Re-run Setup Wizard',
  'settings.export_logs_label': 'Export Logs',
  'settings.delete_history_label': 'Delete All History',
  'settings.force_recover_label': 'Force-recover from frozen state',
  'settings.dotfiles_pat_label': 'GitHub PAT',
  'settings.include_agent_data_label': 'Include Agents/Skills/Memory',
  'settings.sync_to_gist_label': 'Sync to Gist',
  'settings.sync_from_gist_label': 'Sync from Gist',
  'settings.section_terminal': 'Terminal',
  'settings.section_display': 'Display',
  'settings.section_ai_llm': 'AI / LLM',
  'settings.section_agents': 'Agents',
  'settings.section_telegram': 'Telegram Gateway',
  'settings.section_team': 'Team',
  'settings.section_sound': 'Sound',
  'settings.section_language': 'Language',
  'settings.section_context': 'Context',
  'settings.section_safety': 'Safety',
  'settings.section_scouter': 'Scouter',
  'settings.section_data': 'Data',
  'settings.section_recovery': 'Recovery',
  'settings.section_sync': 'Sync',
  'settings.font_size': 'Font Size',
  'settings.line_height': 'Line Height',
  'settings.theme': 'Theme',
  'settings.font': 'Font',
  'settings.font_default': 'Default',
  'settings.font_dotgothic16': 'DotGothic16',
  'settings.cursor_shape': 'Cursor Shape',
  'settings.cursor_block': 'Block',
  'settings.cursor_underline': 'Underline',
  'settings.cursor_bar': 'Bar',
  'settings.behavior': 'Behavior',
  'settings.haptic': 'Haptic Feedback',
  'settings.auto_scroll': 'Auto-Scroll',
  'settings.high_contrast': 'High Contrast Output',
  'settings.connection': 'Connection',
  'settings.ai_providers': 'AI Providers',
  'settings.language': 'Language',
  'settings.language_desc': 'App display language',
  'settings.snippet_behavior': 'Snippet Behavior',
  'settings.run_mode': 'Run Mode',
  'settings.auto_return': 'Auto-Return to Terminal',
  'settings.safety': 'Command Safety',
  'settings.enable_safety': 'Enable Safety Check',
  'settings.safety_level': 'Confirmation Level',
  'settings.close_a11y': 'Close settings',
  // G2-P3 (2026-08-25): the developer/IDE drill-down row + screen.
  'settings.developer_title': 'Developer',
  'settings.developer_open_a11y': 'Open developer settings',
  'settings.developer_row_subtitle': 'Doctor · Integrations · Webhook Allowlist · Scouter · Reset',

  // ── Settings: panels ─────────────────────────────────────────────
  'agents.title': 'AI Agents',
  'agents.default': 'Default',
  // Fable5 quality-floor Design C (2026-08-28) — lib/companion-brain.ts's
  // resolveCompanionBrain. ON (default) = 'auto': the companion (default
  // Shelly persona) thread's replies are generated by a configured Cerebras/
  // Groq/Gemini/OpenRouter key when one exists, falling back to the
  // on-device model otherwise. OFF = 'local-only': the companion never
  // leaves the device.
  'agents.companion_brain': 'Companion brain',
  'agents.companion_brain_hint_auto': 'Auto: uses your fastest configured cloud key (Cerebras → Groq → Gemini → OpenRouter) for the companion, falling back to on-device when none is configured or a cloud reply fails.',
  'agents.companion_brain_hint_local': 'On-device only: the companion always answers from the local model, even if a cloud key is configured.',
  'agents.auto_approve': 'Auto-approve',
  'agents.autonomous_cloud': 'Autonomous Cloud',
  'agents.cloud_on_exhaustion': 'On quota exhausted',
  'agents.cloud_exhaust_escalate': 'Escalate to Codex',
  'agents.cloud_exhaust_stop': 'Stop (retry later)',
  'agents.output_target': 'Output to',
  'agents.output_local': 'Local',
  'agents.output_obsidian': 'Obsidian',
  'agents.output_custom': 'Custom',
  'agents.vault_path_ph': 'Vault path (e.g. /sdcard/Documents/ObsidianVault)',
  'agents.custom_path_ph': 'Output path (e.g. /sdcard/Download/AgentOut)',
  'agents.topic_folder_ph': 'Topic folder (optional, e.g. STEAM_AI)',
  'agents.output_resolved_path': 'Current save location: {{path}}',
  'agents.output_resolved_path_hint': '`<date>` and `<title>` are filled in when an agent actually saves.',
  'agents.cloud_consent_title': 'Autonomous cloud access',
  'agents.cloud_consent_body': 'Autonomous agents will use your configured cloud API keys (Gemini / Perplexity) UNATTENDED for web tasks.\n\n• Your key authenticates the request and is NEVER sent to the model.\n• A scheduled agent will consume your quota/cost without asking each time (Gemini free tier; Perplexity is paid).\n• Secrets are still always kept on-device; sending/running still needs your approval.\n\nEnable?',
  'agents.cloud_consent_enable': 'Enable',
  // Optimistic (rollback-type) workspace writes — AppSettings.agentOptimisticWorkspaceWrites.
  // Copy is scoped to exactly what lib/agent-action-reversibility.ts permits (a
  // `draft` into the LOCAL agent-output workspace).
  // 2026-08-04: the "Undo" affordance now exists (hooks/use-ai-pane-
  // dispatch.ts wires a savepointRunner + components/panes/AIPane.tsx renders
  // the button on an eligible run's completion bubble), so the previous
  // "no Undo button / no run takes this path today" caveat is removed —
  // per docs/superpowers/DEFERRED.md's 2026-07-29 entry, that sentence was to
  // be dropped in the SAME commit that wires the undo route, never before.
  'agents.optimistic_writes': 'Optimistic Writes',
  'agents.optimistic_writes_hint': 'Draft saves into the local agent-output folder run immediately behind an automatic savepoint instead of waiting for an approval tap, with an "Undo" button on the result if you change your mind. cli / notify / webhook / social-post / dm-reply, and Obsidian or custom output, still require approval. Undo only works while the app stays running — it does not survive a restart.',
  'agents.optimistic_writes_consent_title': 'Run workspace drafts without approval',
  'agents.optimistic_writes_consent_body': 'This covers ONE thing: a run whose only action saves a draft into the local agent-output folder.\n\n• Shelly takes an automatic git savepoint first, then runs immediately with no approval tap.\n• The chat bubble for that run gets an "Undo" button that reverts the file write — available only for THIS app session (it does not survive a restart).\n• cli / notify / webhook / social-post / dm-reply — and any Obsidian or custom output — are NOT covered and still require approval.\n• Agent registration confirm, command-safety and the secret scan are unchanged.\n\nEnable?',
  'agents.optimistic_writes_consent_enable': 'Enable',
  // Rollback ("元に戻す") affordance itself — components/panes/
  // AgentUndoButton.tsx, rendered by AIPane.tsx's MessageBubble on an
  // eligible attended run's completion bubble. See
  // ChatMessage.agentRollbackOffer (store/types.ts) and
  // lib/agent-manager.ts's rollbackOfferEligible() for the eligibility chain.
  'agents.undo_run_button': 'Undo',
  'agents.undo_run_in_progress': 'Undoing…',
  'agents.undo_run_success': 'Undone — the file write was reverted.',
  'agents.undo_run_unavailable': 'No longer available (already undone, or the app was restarted).',
  'agents.undo_run_failed': 'Undo failed. The change is still in git history in agent-output — you can revert it manually.',
  // Widget-ASK no-confirm registration — AppSettings.widgetAgentRegistrationNoConfirm.
  // Copy is scoped to exactly what lib/widget-agent-registration.ts implements:
  // widget-ASK-originated `@agent` commands only; the AI Pane's own flow and
  // every hard content gate (unclear schedule, assumed/LLM-extracted values,
  // high-risk action types) are explicitly named as unchanged.
  'agents.widget_noconfirm': 'Widget No-Confirm Register',
  'agents.widget_noconfirm_hint': 'An "@agent …" command typed (or dictated) into the home-screen widget\'s ASK dialog registers immediately, without the in-app confirmation step, and posts a notification saying what got registered. ONLY the widget ASK path is affected: "@agent" typed in the AI Pane still confirms as usual. Commands with an unclear schedule, assumed values, or external-posting actions still open the normal in-app flow. Default off.',
  'agents.widget_noconfirm_consent_title': 'Register widget agents without confirmation',
  'agents.widget_noconfirm_consent_body': 'This covers ONE thing: an "@agent …" command entered in the home-screen widget\'s ASK dialog.\n\n• It registers immediately — the normal in-app confirmation bubble is skipped.\n• A notification reports each registration (name + schedule) right away.\n• "@agent" typed in the AI Pane is NOT affected and still confirms.\n• Commands with an unclear schedule, assumed values, or external-posting actions (social-post) still open the normal in-app flow.\n• Per-run action approval, command-safety and the secret scan are unchanged.\n\nEnable?',
  'agents.widget_noconfirm_consent_enable': 'Enable',
  // Posted after a widget-originated registration completed without the
  // confirm step ("act immediately, notify after" — mirrors the unattended
  // skill-save notification shape).
  'agents.widget_registered_title': 'Agent registered from widget',
  'agents.widget_registered_body': '"{{name}}" — {{schedule}}. Registered without the confirmation step (widget no-confirm setting). Manage it with @agent list.',
  'agents.notification_trigger': 'Notification Triggers',
  'agents.notification_trigger_enabled': 'Notification triggers enabled',
  'agents.notification_trigger_disabled': 'Notification triggers disabled',
  'agents.notification_trigger_failed': 'Notification trigger toggle failed',
  'agents.notification_os_access': 'Notification Access (OS)',
  'agents.notification_os_access_a11y': 'Check or grant Android notification access',
  'agents.notification_os_access_title': 'Notification access',
  'agents.notification_os_access_granted': 'Already granted — Shelly can read notifications for the trigger feature.',
  'agents.notification_os_access_not_granted': 'Not granted yet. Opening Android Settings — enable "Shelly" under Notification access, then return here.',
  'agents.notification_os_access_open_settings': 'OK',
  'agents.notification_os_access_failed': 'Notification access check failed',
  // P0-1: posted by the app-launch startup repair when a scheduled agent's
  // alarm was due but never recorded a run (Doze / OEM kill / FGS start
  // failure). The body variant used depends on whether the same repair pass's
  // re-arm attempt actually succeeded (never claim a re-arm that didn't happen).
  'agents.run_in_progress_notification_body': 'Running…',
  'agents.missed_schedule_title': 'Missed schedule',
  'agents.missed_schedule_body': '"{{name}}" did not run at its scheduled time ({{when}}) — it may have been blocked (Doze / battery / OEM). Re-armed for the next occurrence.',
  'agents.missed_schedule_body_repair_failed': '"{{name}}" did not run at its scheduled time ({{when}}) — it may have been blocked (Doze / battery / OEM). The automatic re-arm attempt also failed; open the agent to check its schedule.',
  // api-call (v1): attended "Run now" guard — this action type is
  // PlanSpec-executor-only (scheduled/unattended runs only).
  'agents.api_call_attended_unsupported': 'This agent includes an API-call step, which currently only supports scheduled execution — it can\'t be run now.',

  // ── Agent confirm/preview card (Phase 0 §2.1) ────────────────────
  'agentcard.title': 'Register this agent?',
  'agentcard.summary': "I'll run {{action}} {{schedule}} via {{route}}.",
  'agentcard.name': 'Project name',
  'agentcard.schedule': 'Schedule',
  'agentcard.schedule_unset': '(not set)',
  'agentcard.schedule_required': 'Pick a schedule — this agent won’t run until you do.',
  'agentcard.freq_once': 'Once',
  'agentcard.freq_daily': 'Daily',
  'agentcard.freq_weekly': 'Weekly',
  'agentcard.freq_interval': 'Every N min',
  'agentcard.freq_hourly': 'Hourly',
  'agentcard.weekday_multi_hint': 'Pick multiple days (e.g. Mon・Fri)',
  'agentcard.time_placeholder_hint': '⚠ Placeholder time — confirm before running',
  'agentcard.minutes_every': 'min interval',
  'agentcard.interval_hours': 'hour interval',
  'agentcard.action': 'Action',
  'agentcard.action_draft': 'draft',
  'agentcard.action_notify': 'notify',
  'agentcard.action_webhook': 'webhook',
  'agentcard.action_cli': 'command',
  'agentcard.action_intent': 'App / Share',
  'agentcard.action_dm-reply': 'DM reply',
  // Social platform posting (Track B) — dispatch itself lives elsewhere;
  // this card only authors {connectorId, text} against a pre-registered
  // connector. See social_connectors.* for the Settings registration UI.
  'agentcard.action_social-post': 'social post',
  'agentcard.socialpost_connector_label': 'Connector',
  'agentcard.socialpost_no_connectors': 'No social connectors yet. Add one in Settings → Social Connectors first.',
  'agentcard.socialpost_text_label': 'Post text',
  'agentcard.socialpost_text_hint': 'Use {{result}} to include the run result.',
  'agentcard.socialpost_warning': 'This posts to the connected platform every time this agent runs — there is no separate approval prompt at run time, so review carefully before confirming.',
  // browser-pane (2026-08-05) — attended-only Browser Pane page operation
  // (click / extract text), never auto-accepted; see store/types.ts.
  'agentcard.action_browser-pane': 'browser pane',
  // api-call (v1) — only offered for multi-step (>=2 step) agents.
  'agentcard.action_api-call': 'API call',
  'agentcard.apicall_authref': 'Credential (optional)',
  'agentcard.apicall_authref_none': 'None',
  'agentcard.apicall_host': 'Host',
  'agentcard.apicall_host_locked': 'Locked to {{host}} by the selected credential',
  'agentcard.apicall_method': 'Method',
  'agentcard.apicall_path_placeholder': 'Path (e.g. /v1/search?q={{result}})',
  'agentcard.apicall_body_placeholder': 'Request body (JSON, may contain {{result}})',
  'agentcard.apicall_body_hint': 'The literal text "{{result}}" is replaced with the prior step\'s result.',
  'agentcard.apicall_warning': 'This calls an allowlisted API host every time this agent runs — review the host, path, and credential carefully before confirming.',
  'agentcard.apicall_step_badge': 'API call',
  'agentcard.step_api_call_add': '→ API call',
  'agentcard.step_api_call_remove': '✕ remove API call',
  'agentcard.dmreply_pairing_label': 'Paired conversation',
  'agentcard.dmreply_no_pairings': 'Pair a conversation in Settings first.',
  'agentcard.dmreply_text_label': 'Reply text',
  'agentcard.dmreply_text_hint': 'Use {{result}} to include the run result. Every send still requires Review.',
  'agentcard.dmreply_weak_confidence_hint': 'weak',
  'dm_pairing.title': 'DM PAIRING',
  'dm_pairing.hint': 'Pair one live notification conversation by sending a one-time code to it.',
  'dm_pairing.reply_enabled_label': 'Allow notification replies',
  'dm_pairing.reply_enabled_hint': 'Independent send gate. Keep off unless you are actively using paired replies.',
  'dm_pairing.reply_enabled_on': 'Reply sending enabled',
  'dm_pairing.reply_enabled_off': 'Reply sending disabled',
  'dm_pairing.reply_enabled_failed': 'Could not update the reply-send gate.',
  'dm_pairing.start_button': 'START PAIRING',
  'dm_pairing.send_instruction': 'Send this code to the conversation you want to pair, then keep its notification active:',
  'dm_pairing.waiting_countdown': 'Waiting for a notification… {{seconds}}s',
  'dm_pairing.window_expired': 'The 5-minute pairing window expired.',
  'dm_pairing.retry_button': 'RETRY',
  'dm_pairing.candidates_heading': 'Choose the exact conversation',
  'dm_pairing.candidates_hint': 'Review every match. Shelly never auto-selects a notification.',
  'dm_pairing.confirm_heading': 'Confirm pairing',
  'dm_pairing.weak_confidence_note': 'This app did not expose a conversation shortcut ID; the exact notification ID/tag will be required.',
  'dm_pairing.confirm_button': 'PAIR',
  'dm_pairing.success_toast': 'Conversation paired and disk mirror flushed',
  'dm_pairing.existing_heading': 'PAIRED CONVERSATIONS',
  'dm_pairing.existing_empty': 'No paired conversations.',
  'dm_pairing.revoked_badge': 'revoked',
  'dm_pairing.revoke_button': 'REVOKE',
  'dm_pairing.remove_button': 'REMOVE',
  'dm_reply_test.button': 'RUN SELF-CONTAINED REPLY TEST',
  'dm_reply_test.running': 'TESTING…',
  'dm_reply_test.listener_access_required': 'Grant Android Notification access to Shelly before running the test.',
  'dm_reply_test.flags_required': 'Enable both notification triggers and the separate reply-send gate before running the test.',
  'dm_reply_test.pass': 'Self-contained notification reply round trip passed.',
  'dm_reply_test.unconfirmed': 'The OS accepted the reply, but Shelly did not observe the round trip.',
  'dm_reply_test.failed': 'The self-contained notification reply test failed.',
  'agent_action_confirm_title_dmreply': 'Review DM reply',
  'agent_action_confirm_body_dmreply': 'This sends a message to the paired live conversation. Review the exact target and text.',
  'agent_action_confirm_dmreply_target': 'PAIRED CONVERSATION',
  'agent_action_confirm_dmreply_text': 'REPLY TEXT',
  'agent_action_confirm_dmreply_failed': "Couldn't send the DM reply. Common causes: (1) the original notification was already read/dismissed and is gone from the shade, (2) this pairing was revoked, (3) you just replied to this same conversation moments ago (rate limit). Trigger a fresh notification in the paired app, then try again.",
  'agentcard.command_placeholder': 'command to run with the result',
  'agentcard.cli_warning': 'Commands never auto-run — each run needs in-app confirmation.',
  'agentcard.draft_save_location': 'Output will be saved to: {{path}}',
  'agentcard.intent_mode_launch': 'Launch app',
  'agentcard.intent_mode_share': 'Share',
  'agentcard.intent_target_placeholder': 'Package name or URL (e.g. com.google.android.apps.maps)',
  'agentcard.intent_share_text_placeholder': 'Text to share (optional, {{result}} inserts the run result)',
  'agentcard.intent_warning': 'This always requires your approval before it runs — never automatic.',
  'agentcard.notification_trigger_label': 'Notification triggers (optional)',
  'agentcard.notification_trigger_placeholder': 'e.g. com.whatsapp, com.slack',
  'agentcard.notification_trigger_hint_count': '{{valid}} valid, {{skipped}} skipped',
  'agentcard.notification_trigger_hint_disabled': 'Enable Notification Triggers in Settings for this to take effect',
  'agentcard.notification_senders_label': 'Authorized senders (optional — reads notification text)',
  'agentcard.notification_senders_placeholder': 'e.g. Alice, Family Group',
  'agentcard.notification_senders_hint': 'Exact-match sender names only. Notification text from ONLY these senders is passed to the agent (best-effort: only what fits in the notification; replies work only while the notification is alive). Leave empty to trigger on arrival without reading any text.',
  'agentcard.notification_senders_unset_warning': '⚠ No sender limit set. This agent will run whenever ANY notification arrives from the app(s) above — not just ones from a specific person. The notification text itself is never read (arrival-only trigger). Add a sender above to narrow this, or leave it as-is if that\'s what you intend.',
  'agentcard.runon': 'Run on',
  'agentcard.runon_auto': 'Auto',
  'agentcard.runon_on-device': 'On-device',
  'agentcard.runon_cloud': 'Cloud',
  'agentcard.sched_daily': 'daily at {{time}}',
  'agentcard.sched_daily_multi': 'daily at {{times}}',
  'agentcard.sched_weekly': 'every {{day}} at {{time}}',
  'agentcard.sched_interval': 'every {{n}} min',
  'agentcard.sched_hourly': 'every {{n}} hours',
  'agentcard.sched_once': 'now',
  'agentcard.sched_notification_trigger': 'on notification',
  'agentcard.add_time': '+ Add another time',
  'agentcard.daily_multi_hint': 'Add up to 4 times',
  'agentcard.remove_time': 'Remove {{time}}',
  'agentcard.once_hint': 'Runs immediately on confirm — not saved.',
  'agentcard.once_autonomous_hint': 'Runs immediately and keeps a private driver audit for verification.',
  'agentcard.once_notification_trigger_hint': 'Registers and waits — runs when a notification arrives from the chosen app(s), not now.',
  'agentcard.autonomous': 'Autonomous',
  'agentcard.use_skill': 'Use skill: {{name}} ({{count}}×)',
  'agentcard.use_skill_hint': 'Reuse a recipe that worked before for a similar task.',
  'agentcard.orchestration': 'Multi-step ({{count}} steps, each gated):',
  'agentcard.char_limit': 'Final output hard limit: {{count}} characters',
  'agentcard.autonomous_route': 'Autonomous Codex',
  'agentcard.autonomous_route_web': 'Autonomous {{engine}}',
  'agentcard.autonomous_route_local': 'Autonomous on-device',
  'agentcard.autonomous_hint': 'Run multi-step via the gated agent (asks you to approve risky steps).',
  'agentcard.autonomous_route_hint': 'Autonomous chat runs through the gated Codex driver. API-key cloud backends are not used here.',
  'agentcard.autonomous_route_hint_web': 'Cloud consent on: collects unattended on {{engine}} (grounded); escalates to Codex only on failure.',
  'agentcard.autonomous_route_hint_local': 'Runs unattended on the on-device local LLM (nothing leaves the device).',
  'agentcard.run_now': 'Run now',
  'agentcard.confirm': 'Confirm',
  'agentcard.cancel': 'Cancel',

  // ── Schedule readiness checklist (P1 scheduling-reliability audit,
  // 2026-07-15) — a one-time, dismissible card shown after a device's first
  // scheduled agent registration. Never a registration gate: the agent this
  // follows already exists by the time this card appears.
  'schedulereadiness.title': 'Keep this schedule reliable',
  'schedulereadiness.intro': 'One-time check — this agent is already registered. These settings help it survive Android battery restrictions.',
  'schedulereadiness.exact_alarm_title': 'Exact alarm scheduling',
  'schedulereadiness.exact_alarm_ok': 'Granted — this schedule fires at its exact time.',
  'schedulereadiness.exact_alarm_missing': 'Not granted. Android may delay this schedule by minutes to hours under battery restrictions.',
  'schedulereadiness.exact_alarm_action': 'Grant',
  'schedulereadiness.battery_title': 'Battery optimization exemption',
  'schedulereadiness.battery_ok': 'Exempted — Shelly can run in the background when it needs to.',
  'schedulereadiness.battery_missing': 'Not exempted. Android may kill Shelly in the background before a scheduled run completes.',
  'schedulereadiness.battery_action': 'Exempt',
  'schedulereadiness.samsung_title': 'Samsung "Sleeping apps" (One UI)',
  'schedulereadiness.samsung_body': 'Samsung has its own battery control on top of Android’s: Settings › Battery and device care › Background usage limits. Make sure Shelly is not listed under "Sleeping apps" or "Deep sleeping apps" — add it to "Never sleeping apps" if that list exists on your device.',
  'schedulereadiness.samsung_action': 'Open battery settings',
  'schedulereadiness.dismiss': 'Got it',

  // ── Chat-native agent confirmation (Phase 7) — used for social-post / tool-pinned
  // orchestration instead of the card: this summary text plus a Confirm/Cancel pair.
  'agentplan.summary_name': "I'll call this: {{name}}",
  'agentplan.summary_schedule': 'Schedule: {{schedule}}',
  'agentplan.summary_action': 'Action: {{action}}',
  // 2026-08-24 Fable5 product review finding: the confirm summary's Action
  // line rendered the raw internal action.type name verbatim ("Action:
  // notify") — reused agentcard.action_* (the FORM PICKER's terse option
  // labels, appropriately short for a dropdown) for this prose line too.
  // These separate, natural-language equivalents are used ONLY by
  // actionText() in lib/agent-plan-summary.ts for the confirm-bubble/card
  // summary text; agentcard.action_* is untouched and still drives the
  // picker.
  'agentplan.action_label_draft': 'save a draft',
  'agentplan.action_label_notify': 'send you a notification',
  'agentplan.action_label_webhook': 'call a webhook',
  'agentplan.action_label_cli': 'run a command',
  'agentplan.action_label_intent': 'open an app',
  'agentplan.action_label_dm-reply': 'send a DM reply',
  'agentplan.action_label_api-call': 'make an API call',
  // Computed but unused in practice (the browser-pane switch case in
  // actionText() always returns its own dedicated URL/selector line
  // instead) — present anyway so a future refactor that starts relying on
  // this fallback doesn't silently render the raw i18n key string.
  'agentplan.action_label_browser-pane': 'act in the browser',
  'agentplan.autonomous_note': 'Runs autonomously (no per-run human approval).',
  'agentplan.memory_note': 'Will remember after running: {{fact}}',
  'agentplan.skill_note': 'Reuses the existing skill "{{name}}" ({{count}} prior successes).',
  'agentplan.schedule_restate_hint': '⚠ I could not pin down a firm schedule. Please restate it with an explicit time (e.g. "daily at 8am"). This cannot be registered until then.',
  'agentplan.socialpost_line': 'Post to {{platform}} ({{connector}}) (posts the run result as-is)',
  'agentplan.socialpost_line_with_preview': 'Post to {{platform}} ({{connector}}): "{{preview}}"',
  // 2026-07-28: multi-platform simultaneous posting (DEFERRED.md's "エージェ
  // ント1件から複数プラットフォームへ同時配信できない" authoring-side follow-up)
  // — shown as an extra line alongside the normal "Action:" line only when
  // lib/agent-nl-parser.ts's detectMultiSocialActions produced 2+ draft.actions.
  'agentplan.summary_multi_targets': 'Destinations: {{targets}}',
  'agentplan.draft_line_with_path': 'draft (saved to {{path}})',
  // browser-pane (2026-08-05) — the confirm bubble MUST spell out the exact
  // selector + URL the human is approving; see lib/agent-plan-summary.ts.
  'agentplan.browserpane_line_extract': 'Browser Pane: extract text from "{{selector}}" on {{url}} (attended only — you approve every run on screen)',
  'agentplan.browserpane_line_click': 'Browser Pane: click "{{selector}}" on {{url}} (attended only — you approve every run on screen)',
  'agentplan.browserpane_line_fill': 'Browser Pane: fill "{{selector}}" on {{url}} (attended only — you approve every run on screen)',
  'agentplan.browserpane_unset_field': '(not set)',
  // Phase C (2026-07-22): a plain field-only correction ("make it 9am") now
  // patches the draft in place instead of requiring cancel+redo — see
  // lib/agent-draft-patch.ts. Reworded from the older "cancel and re-describe"
  // copy to reflect that.
  'agentplan.confirm_prompt': 'Got it — I\'ll do this as described above. Say the word and I\'ll start, or tell me what to change (e.g. "make it 9am"); say "cancel" to drop it.',
  // 2026-07-23: Sidebar "Edit" entry point (components/layout/Sidebar.tsx)
  // routes an already-registered agent through this SAME chat-confirm flow —
  // the draft is fully formed already, so "register" here would misleadingly
  // suggest a brand-new/duplicate agent is about to be created instead of the
  // existing one being updated. See summarizeAgentDraftAsText's isEditing
  // param.
  'agentplan.confirm_prompt_edit': 'Got it — I\'ll update it to what\'s described above. Say the word, or tell me what else to change; "cancel" drops the edit.',
  'agentplan.registered_notice': 'Got it — "{{name}}" is set. I\'ll do this {{scheduleDescription}}{{autonomousSuffix}}. You can check on it anytime with @agent list.{{correctionHint}}',
  'agentplan.updated_notice': 'Got it — "{{name}}" is updated. I\'ll do this {{scheduleDescription}}{{autonomousSuffix}} from now on.',
  // Phase A/B (2026-07-22) — type-to-confirm and the assumed-schedule
  // annotation for the chat-native flow (hooks/use-ai-pane-dispatch.ts /
  // lib/agent-plan-summary.ts).
  'agentplan.confirm_unclear_hint': 'There is a pending draft. Reply "register" / "OK" to confirm, "cancel" to discard it, or just say what to change (e.g. "make it 9am").',
  'agentplan.llm_conversation_fallback_notice': 'Switching to simple step-by-step questions.',
  // 2026-07-23: edit-context counterpart of confirm_unclear_hint (see
  // confirm_prompt_edit's own comment).
  'agentplan.confirm_unclear_hint_edit': 'There is a pending edit. Reply "update" / "OK" to confirm, "cancel" to discard it, or just say what to change (e.g. "make it 9am").',
  // bug #157 fix (docs/superpowers/DEFERRED.md): appended to a draft bubble
  // that just lost the pane's single pendingAgentSession slot to a newer,
  // unrelated draft — typed confirm/cancel now resolves the newer one
  // instead; the tap-to-confirm buttons on this bubble still work.
  'agentplan.superseded_notice': '⚠ A different request came in — please use the button above to confirm this draft (a typed reply will resolve the newer one instead).',
  'agentplan.superseded_notice_edit': '⚠ A different request came in — please use the button above to confirm this edit (a typed reply will resolve the newer one instead).',
  'agentplan.schedule_assumed_note': 'Interpreted "{{word}}" as {{time}}.',
  // Deferred-start ("来週あたりから…") anchor line — shown next to the
  // schedule line whenever startNotBefore is set and still in the future.
  'agentplan.start_not_before_note': 'Starts from: {{date}}.',
  'agentplan.next_fire_note': 'Next run: {{datetime}}',
  // 2026-07-23: shown when one or more fields above were filled in by the
  // hybrid LLM-extraction fallback (lib/agent-llm-fallback.ts) instead of
  // the deterministic parser — see lib/agent-plan-summary.ts's
  // hasDraftAssumptions doc comment for why this always forces one confirm
  // round-trip, same as an assumed schedule.
  'agentplan.llm_extracted_note': '🤖 Some fields above were inferred by AI from your message — please double-check them before confirming.',
  // Security-audit finding (2026-08-10): disclosure line for a
  // notification-triggered draft/notify agent on the chat-native confirm
  // path (see agent-plan-summary.ts's summarizeAgentDraftAsText). The
  // unscoped variant is the one that actually fires today — no producer of
  // this draft ever fills in authorizedSenders — but the scoped variant is
  // kept for forward-compatibility.
  'agentplan.notification_trigger_unscoped_note': '⚠ Triggers on notification: runs whenever ANY notification arrives from {{packages}} — not limited to a specific sender, and the notification text is never read.',
  'agentplan.notification_trigger_scoped_note': 'Triggers on notification: runs only for notifications from {{senders}} (app: {{packages}}).',
  // Phase C (2026-07-22): header line prepended (by hooks/use-ai-pane-dispatch.ts)
  // above the re-posted summary when a follow-up reply patched an
  // ALREADY-APPLIED change (the justRegisteredAgent quick-correct window —
  // updateAgent has already run by the time this text is shown, so "Updated"
  // is accurate here). See lib/agent-draft-patch.ts / summarizeAgentDraftAsText's
  // changedFields param, which marks the touched line(s) with ★.
  'agentplan.patch_updated_header': 'Updated:',
  // 2026-07-23: on-device test found the SAME "Updated:" header being reused
  // for a patch to a still-PENDING draft/edit (nothing committed yet — a
  // separate confirm reply is still required, see the hard invariant on
  // applyPatchToPendingSession) — read together with the trailing "Register/
  // Update this agent...?" question right below it, "Updated:" reads as
  // "already done", making the confirm question feel redundant/confusing.
  // This wording makes clear the change only landed in the DRAFT so far.
  'agentplan.patch_pending_header': 'Applied to the draft (not saved yet):',
  // 2026-07-23: appended to the auto-register / chat-native-confirm success
  // message, telling the user the short correction window (see
  // hooks/use-ai-pane-dispatch.ts's JUST_REGISTERED_STALE_MS) exists at all.
  'agentplan.correction_hint': 'Need a quick fix? Just say it (e.g. "actually make it 8pm").',
  // Edge case: applyDraftPatch found a correction to apply, but the agent it
  // targets no longer exists (deleted via another surface in the gap).
  'agentplan.correction_agent_missing': 'That agent no longer exists, so nothing was changed.',
  // 2026-07-27 bug fix: shown instead of a schedule-change line when a
  // correction-window reply ("今すぐ実行して") resolves to parseSchedule's
  // 'once' sentinel while the agent already has a real recurring schedule —
  // see lib/agent-draft-patch.ts's applyCorrectionToJustRegisteredAgent doc
  // comment. The recurring schedule is left untouched; this is an ADDITIONAL
  // one-off run, not a schedule replacement.
  'agentplan.run_now_started': 'Running "{{name}}" right now — its recurring schedule stays unchanged.',
  'agentplan.run_now_done': 'Done.',
  'agentplan.run_now_failed': 'The run-now request failed',
  // 2026-08-10 on-device QA finding: a local-LLM (on-device route) one-shot
  // @agent run took ~3m10s with the "▶ Running…" bubble frozen the entire
  // time (agent execution is a single request/response, not token-streamed
  // — see hooks/use-ai-pane-dispatch.ts's startAgentRunProgressTicker doc
  // comment), which reads as "the app is frozen". This elapsed-time suffix
  // is appended to that same bubble every couple seconds while the run is
  // in flight so it reads as "still working" instead.
  'agentplan.run_now_progress': 'still working… ({{seconds}}s)',
  // 2026-07-28 bug fix: shown on a patch reply to a still-PENDING (not yet
  // registered) draft when "今"/"今すぐ" resolved against an already-real
  // recurring schedule — see lib/agent-draft-patch.ts's
  // applyPatchToPendingSession doc comment. The recurring schedule line above
  // is left unmarked (unchanged); this note explains the additional run-now
  // action that will fire once the draft is actually confirmed.
  'agentplan.run_once_on_confirm_note': "This will also run once, right after you confirm — its recurring schedule stays unchanged.",
  // ── Shared ("remember this for every agent") memory writes ────────────────
  // A `_global` note is recalled by EVERY agent, so unlike an ordinary
  // per-agent note it always costs one explicit confirm turn — see
  // lib/agent-global-memory-intent.ts. Nothing is stored until the user
  // answers this question with a confirm phrase.
  'globalmemory.confirm_prompt':
    'Save this so EVERY agent remembers it, now and in future runs?\n\n  "{{text}}"\n\nReply "OK" to save it, or "cancel" to discard. (This is shared across all your agents — for one agent only, say it without "all agents".)',
  'globalmemory.confirm_unclear':
    'Not saved yet. Reply "OK" to save this for every agent, or "cancel" to discard it:\n\n  "{{text}}"',
  'globalmemory.discarded_unclear':
    'Discarded the shared note (no confirmation was given). Nothing was saved — say it again if you still want every agent to remember it.',
  'globalmemory.cancelled': 'Discarded. Nothing was saved.',
  'globalmemory.saving': 'Saving to shared memory…',
  'globalmemory.saved':
    '✅ Saved to shared memory — every agent will see this from now on:\n\n  "{{text}}"',
  'globalmemory.failed': '❌ Could not save the shared note',
  // Design 2-b ("keep an eye on X" commitment detection, 2026-08-28):
  // appended to globalmemory.saved only when the write came from a
  // watch-phrase ("keep an eye on…"), not an ordinary "remember this". This
  // note is passive and recall-only — it surfaces again next time the topic
  // comes up, it does NOT poll or actively monitor anything — so the
  // acknowledgment must never let that be assumed. Points at agent
  // registration, which IS the active-monitoring mechanism.
  'globalmemory.watch_hint': "Just so you know, this is a passive note — I'll only bring it up again if it comes up in conversation. If you need it actively checked on a schedule, you can register an agent for that instead.",
  // ── Agent deletion confirmation ────────────────────────────────────────────────
  'agentdelete.confirm_prompt':
    'I found "{{name}}". I can delete it, including its plan and schedule. Say "OK" and I\'ll delete it, or "cancel" to keep it.',
  'agentdelete.confirm_unclear':
    'I haven\'t deleted "{{name}}" yet. Say "OK" to delete it, or "cancel" to keep it.',
  'agentdelete.discarded_unclear':
    'I didn\'t get a clear confirmation, so I\'ve dropped the delete request. "{{name}}" is still there.',
  'agentdelete.cancelled': 'Got it — I kept "{{name}}".',
  'agentdelete.deleting': 'I\'m deleting "{{name}}"…',
  'agentdelete.deleted': '✅ I deleted "{{name}}".',
  'agentdelete.failed': '❌ I couldn\'t delete "{{name}}"',
  // 2026-08-29: free-text delete detector (detectFreeTextAgentDeleteIntent,
  // lib/agent-nl-parser.ts) found more than one agent matching the named
  // candidate — same ambiguous-name situation `@agent delete <name>` already
  // handles (lib/agent-manager.ts's resolveNamedAgentOrError), surfaced here
  // for the free-text entry point instead of ever guessing which one.
  'agentdelete.ambiguous':
    'More than one agent matches that: {{names}}. Could you be more specific, or use "@agent delete <name>" with the exact name?',
  // G2-P3 follow-up (2026-08-25): companion-only entry point into
  // MemoryWorkbenchPane, so a user with no registered agents can still
  // see/edit/delete what the companion journal auto-wrote.
  'companion_memory.title': 'Companion Memory',
  'companion_memory.description': 'What Shelly remembers about your conversations, and standing preferences shared with every agent.',
  'companion_memory.open': 'View companion memory',
  'companion_memory.open_a11y': 'Open companion memory notes',
  // Fable5 review Gap A (2026-08-25): persistent banner (not a one-time
  // dismiss) shown whenever settings.localLlmUrl is unset, since
  // digestConversationForJournal skips every write without it.
  'companion_memory.dormant_title': 'Dormant — no local LLM configured',
  'companion_memory.dormant_hint': 'The journal only writes when a local LLM is set up, so nothing from your conversations is being saved yet. Configure one in Local LLM settings to turn this on.',
  'api_keys.title': 'API Keys',
  'api_keys.paste_placeholder': 'Paste {{name}} API key',
  'webhook_allowlist.title': 'Webhook trusted hosts',
  'webhook_allowlist.description': 'Hosts you have already vetted. Every webhook still requires approval.',
  'webhook_allowlist.placeholder': 'hooks.example.com',
  'webhook_allowlist.add': 'ADD',
  'webhook_allowlist.invalid': 'Enter a hostname only, without a scheme, port, path, or wildcard.',

  // ── Social Connectors (Settings) — credential registration for the
  // free-API social/publishing dispatch path (agent action 'social-post').
  // Metadata only; secret values live in SecureStore and are never rendered
  // back here. Field-name keys (social_connectors.field_*) are shared across
  // platforms that reuse a field name (e.g. webhookUrl, appPassword).
  'social_connectors.title': 'Social Connectors',
  'social_connectors.description': 'Credentials for the "social post" agent action. Only labels and hosts are shown here — secret values are stored encrypted and never displayed again.',
  'social_connectors.empty': 'No connectors registered yet.',
  'social_connectors.add_button': '+ Add connector',
  'social_connectors.select_platform': 'Select platform',
  'social_connectors.label_placeholder': 'Label (e.g. My Discord)',
  'social_connectors.id_placeholder': 'id (lowercase letters, numbers, hyphens)',
  'social_connectors.host_placeholder': 'Instance host (e.g. mastodon.social)',
  'social_connectors.id_invalid': 'ID must be lowercase letters, numbers, and hyphens only.',
  'social_connectors.id_duplicate': 'A connector with this ID already exists.',
  'social_connectors.label_required': 'Enter a label.',
  'social_connectors.host_required': 'Enter the instance host.',
  'social_connectors.field_required': 'Enter {{field}}.',
  'social_connectors.added_toast': '{{label}} connected.',
  'social_connectors.remove_a11y': 'Remove connector',
  'social_connectors.saving': 'Saving…',
  'social_connectors.platform_discord': 'Discord',
  'social_connectors.platform_slack': 'Slack',
  'social_connectors.platform_telegram': 'Telegram',
  'social_connectors.platform_mastodon': 'Mastodon',
  'social_connectors.platform_misskey': 'Misskey',
  'social_connectors.platform_wordpress': 'WordPress',
  'social_connectors.platform_bluesky': 'Bluesky',
  'social_connectors.hint_discord': 'Server Settings → Integrations → Webhooks → copy webhook URL',
  'social_connectors.hint_slack': 'Create an Incoming Webhook at api.slack.com/apps',
  'social_connectors.hint_telegram': 'Message @BotFather to create a bot and get a token; get your chat_id by messaging @userinfobot',
  'social_connectors.hint_mastodon': 'Your instance → Settings → Development → New Application, needs write:statuses scope',
  'social_connectors.hint_misskey': 'Settings → API → generate an access token',
  'social_connectors.hint_wordpress': 'Users → Profile → Application Passwords (requires WP 5.6+)',
  'social_connectors.hint_bluesky': 'Settings → App Passwords → create one (do NOT use your main account password)',
  'social_connectors.field_webhookUrl': 'Webhook URL',
  'social_connectors.field_botToken': 'Bot Token',
  'social_connectors.field_chatId': 'Chat ID',
  'social_connectors.field_accessToken': 'Access Token',
  'social_connectors.field_apiToken': 'API Token',
  'social_connectors.field_username': 'Username',
  'social_connectors.field_appPassword': 'App Password',
  'social_connectors.field_handle': 'Handle',
  'social_connectors.platform_x': 'X',
  'social_connectors.hint_x': 'developer.x.com → create an app → copy its Client ID. Opens X in your browser to sign in; you’re redirected back here automatically.',
  'social_connectors.field_refreshToken': 'Refresh Token',
  'social_connectors.field_clientId': 'Client ID',
  'social_connectors.x_client_id_placeholder': 'Client ID (from developer.x.com)',
  'social_connectors.x_client_id_required': 'Enter your X app’s Client ID.',
  'social_connectors.connect_x': 'Connect X',
  'social_connectors.connecting': 'Opening browser…',

  // 2026-07-24: examples dropped from this question — a slot-fill answer the
  // deterministic parser can't understand now falls back to the local LLM
  // before re-asking (see hooks/use-ai-pane-dispatch.ts), so free-form
  // phrasing no longer needs steering toward the parser's exact patterns.
  'slot_fill.question_task_detail_fallback': 'Could you tell me more specifically what you would like me to do?',
  'slot_fill.question_schedule': 'When should this run?',
  'slot_fill.question_notification_trigger': 'Which app\'s notifications should trigger this? (e.g. com.whatsapp or an app name like Slack)',
  'slot_fill.question_output_path': 'Where should the result be saved? (Leave blank or say "skip" to use Shelly\'s default folder.)',
  'slot_fill.question_social_connector': 'Which connector should this post to? Reply with the number or its label.',
  'slot_fill.question_autonomous': 'Should this agent run unattended (autonomously)? Answer yes or no.',
  'slot_fill.social_connector_giveup_caveat': "Couldn't tell which connector you meant, so this is registered as a draft (file save) instead. Say the connector's exact label next time to post directly.",
  // browser-pane slot-fill (2026-08-05) — asked when a page-operation intent
  // was detected but the target URL / CSS selector wasn't in the utterance.
  'slot_fill.question_browser_url': 'Which page should this act on? Give the exact URL (e.g. https://example.com/news).',
  'slot_fill.question_browser_selector': 'Which element on that page? Give a CSS selector (e.g. h1, .headline, #submit).',
  'slot_fill.browser_pane_giveup_caveat': "Couldn't pin down the page URL / element for the browser operation, so this is registered as a draft (file save) instead. Re-register with an exact URL and CSS selector to drive the Browser Pane.",
  // 2026-07-24: prepended to the re-asked question on a failed slot-fill
  // attempt so a retry never reads byte-identical to the first ask (on-device
  // feedback: repeating the exact same question with no acknowledgment that
  // the previous answer wasn't understood read as unhelpful/robotic).
  'slot_fill.not_understood': "Sorry, I didn't understand that.",
  'slot_fill.cancelled': 'Registration cancelled.',
  'codex_login.title': 'Codex Login',
  'codex_login.confirm_title': 'Sign in with ChatGPT?',
  'codex_login.confirm_body': 'Opens the Browser Pane to auth.openai.com for the device-code flow. After you approve in the browser, Shelly writes ~/.codex/auth.json (mode 0600). Run `shelly doctor` afterwards to confirm.',
  'codex_login.sign_in': 'Sign in',
  'codex_login.sign_in_chatgpt': 'Sign in with ChatGPT',
  'codex_login.sign_in_a11y': 'Sign in with ChatGPT for Codex',
  'codex_login.description': "Sign in with your ChatGPT subscription via device-code OAuth. Runs in a new terminal pane and opens the verification page in Shelly's Browser Pane.",
  'integrations.title': 'Integrations',
  'integrations.open_mcp_a11y': 'Open MCP Servers settings',
  'integrations.open_llama_a11y': 'Open Local LLM llama.cpp settings',
  'recovery.title': 'Recovery',
  'recovery.action': 'Force-recover from frozen state',
  'recovery.action_a11y': 'Force-recover from frozen state',
  'recovery.confirm_title': 'Force-recover Shelly?',
  'recovery.confirm_body': 'Wipes ~/.shelly-cli.staging and the stale update lockfile. Live install (~/.shelly-cli) is preserved, so your CLIs keep working. Use this only if Shelly freezes on launch and task-kill from the recents list does not recover.\n\nAfter recovery, fully close Shelly from recents and relaunch. The next launch will refresh from upstream automatically.',
  'recovery.recover': 'Recover',
  'recovery.warn_title': 'Recovery completed with warnings',
  'recovery.warn_body': 'Cleaned {{cleaned}} item(s). {{errors}} could not be removed:',
  'recovery.complete_title': 'Recovery complete',
  'recovery.complete_body': 'Cleaned {{cleaned}} item(s). Force-stop Shelly from recents and relaunch.',
  'recovery.failed': 'Recovery failed',
  'doctor.title': 'Doctor',
  'doctor.run': 'Run diagnostics',
  'doctor.run_a11y': 'Run shelly-doctor diagnostics',
  'doctor.running': 'Running…',
  'doctor.failed_title': 'Doctor failed',
  'doctor.ok_group': 'OK ({{count}})',
  'doctor.warn_group': 'Needs attention ({{count}})',
  'doctor.check_node': 'Node binary',
  'doctor.check_bash': 'Bash library',
  'doctor.check_exec_wrapper': 'Exec wrapper',
  'doctor.check_xdg_open': 'xdg-open shim',
  'doctor.check_codex_tui': 'Codex TUI',
  'doctor.check_codex_js': 'Codex JS dispatcher',
  'doctor.check_codex_auth': 'Codex auth',
  'doctor.check_download_credentials': 'Leftover credentials in Download',
  'doctor.check_private_file': '{{name}} permissions',
  'doctor.check_private_file_hint': 'mode {{mode}}, should be 0600/0700',
  'doctor.check_env_keys': 'API keys in environment',
  'scouter.title': 'Scouter',
  'scouter.enabled': 'Scouter enabled',
  'scouter.disabled': 'Scouter disabled',
  'scouter.failed': 'Scouter failed',
  'scouter.debug_title': 'Scouter Debug',
  'scouter.debug_failed': 'Scouter Debug failed',
  'scouter.hooks_title': 'Scouter Hooks',
  'scouter.hooks_failed': 'Scouter Hooks failed',
  'scouter.enable_first': 'Turn Scouter on first, then copy hook templates.',
  'scouter.copy_debug': 'Copy debug info',
  'scouter.copy_debug_a11y': 'Copy Scouter debug info',
  'scouter.open_monitor': 'Open Scouter monitor',
  'scouter.open_monitor_a11y': 'Open Scouter detail',
  'scouter.copy_hooks': 'Copy hook templates',
  'scouter.copy_hooks_a11y': 'Copy Scouter hook templates',
  'scouter.import_pets': 'Import pet ZIP',
  'scouter.import_pets_a11y': 'Import Codex pet ZIP',
  'scouter.import_pets_done_title': 'Pets imported',
  'scouter.import_pets_done_body': 'Imported {{count}} pet(s): {{ids}}',
  'scouter.import_pets_failed': 'Pet import failed',
  'scouter.import_pets_unavailable': 'This Shelly build cannot import Codex pet ZIP files.',
  'theme.blue': 'Blue',
  'theme.red': 'Red',
  'theme.purple': 'Purple',
  'theme.scouter_green': 'Green',
  'theme.case_file': 'Case File',
  'wallpaper.title': 'Wallpaper',
  'wallpaper.image': 'Image',
  'wallpaper.pick': 'Pick',
  'wallpaper.change': 'Change',
  'wallpaper.image_opacity': 'Image Opacity',
  'wallpaper.panel_opacity': 'Panel Opacity',
  'wallpaper.terminal_transparency': 'Show wallpaper in Terminal (experimental)',
  'wallpaper.terminal_transparency_hint': 'On by default, matching the other panes. Turn this off if you prefer an opaque terminal background.',
  'wallpaper.permission_title': 'Permission needed',
  'wallpaper.permission_body': 'Shelly needs access to your photo library to pick a wallpaper.',
  'wallpaper.pick_failed': 'Pick failed',

  // ── Onboarding ────────────────────────────────────────────────────
  'onboard.welcome': 'Welcome to Shelly!',
  'onboard.welcome_desc': 'An Android terminal with built-in AI assistants. Let\'s get you started.',
  'onboard.terminal_title': 'Terminal Basics',
  'onboard.terminal_desc': 'Type shell commands just like a regular terminal.\nRuns natively on your device — no setup required.',
  'onboard.ai_title': 'AI Assistants',
  'onboard.ai_desc': 'Use @codex, @local, or @team to ask AI directly.\nOr just type in natural language — we\'ll suggest the right tool.',
  'onboard.git_title': 'Git for Beginners',
  'onboard.git_desc': 'Type @git followed by what you want to do.\nGets step-by-step guidance in plain language.',
  'onboard.customize_title': 'Make It Yours',
  'onboard.customize_desc': 'Themes, keybindings, plugins — customize everything in Settings.',
  'onboard.next': 'Next',
  'onboard.back': 'Back',
  'onboard.done': 'Get Started!',
  'onboard.skip': 'Skip',

  // ── Setup Wizard (Native PTY) ────────────────────────────────────
  'wizard2.welcome_title': 'Welcome to Shelly',
  'wizard2.welcome_subtitle': 'A powerful terminal with built-in AI — right on your phone.',
  'wizard2.feat_terminal': 'Full Terminal',
  'wizard2.feat_terminal_desc': 'Native Linux shell, no extras needed',
  'wizard2.feat_ai': 'AI Assistants',
  'wizard2.feat_ai_desc': 'Codex and local AI — built in',
  'wizard2.feat_native': 'Native Speed',
  'wizard2.feat_native_desc': 'Direct execution, zero overhead',
  'wizard2.skip_all': 'Skip',
  'wizard2.next': 'Next',
  'wizard2.back': 'Back',
  'wizard2.cli_title': 'AI Coding Agents',
  'wizard2.cli_subtitle': 'Pick the ones you want to use.\nYou can always change this in Settings.',
  'wizard2.cli_claude_desc': 'Legacy agent removed',
  'wizard2.cli_gemini_desc': 'Legacy agent removed',
  'wizard2.cli_codex_desc': 'OpenAI\'s coding agent',
  'wizard2.cli_hint': 'All optional. Terminal works without any AI.',
  'wizard2.setup_selected': 'Set Up',
  'wizard2.skip_setup': 'Skip for Now',
  'wizard2.auth_title': 'Sign In',
  'wizard2.auth_hint': 'Setting up authentication...',
  'wizard2.installing_title': 'Installing',
  'wizard2.installing_desc': 'Setting up your AI tools...',
  'wizard2.done_title': 'You\'re All Set!',
  'wizard2.configured_label': 'CONFIGURED',
  'wizard2.done_no_cli': 'No AI configured yet — that\'s fine!\nJust use the terminal, or set up AI later in Settings.',
  'wizard2.done_hint': 'You can change everything in Settings anytime.',
  'wizard2.get_started': 'Get Started',
  'setup2.free_badge': 'FREE',

  // ── Interactive Setup (shelly setup) ──────────────────────────────
  'setup.welcome_title': 'Welcome to Shelly',
  'setup.welcome_desc': 'Let\'s set up your terminal. This takes about 2 minutes.\nYou can skip any step.',
  'setup.next': 'Next',
  'setup.skip': 'Skip',
  'setup.back': 'Back',
  'setup.done': 'Done',
  'setup.retry': 'Retry',
  'setup.cli_title': 'Install AI Coding Agents',
  'setup.cli_desc': 'Pick the tools you want to use. All are optional.',
  'setup.cli_claude': 'Legacy agent removed',
  'setup.cli_claude_desc': 'Use Codex or Local LLM instead',
  'setup.cli_gemini': 'Legacy agent removed',
  'setup.cli_gemini_desc': 'Use Codex or Local LLM instead',
  'setup.cli_codex': 'Codex CLI',
  'setup.cli_codex_desc': 'OpenAI\'s coding agent',
  'setup.cli_installing': 'Installing...',
  'setup.cli_installed': 'Installed',
  'setup.cli_already': 'Already installed',
  'setup.cli_failed': 'Installation failed',
  'setup.cli_install_all': 'Install Selected',
  'setup.cli_none_selected': 'No tools selected — skipping.',
  'setup.auth_title': 'Authenticate',
  'setup.auth_desc': 'Sign in to your installed tools.',
  'setup.auth_browser': 'Sign in with browser',
  'setup.auth_apikey': 'Enter API key instead',
  'setup.auth_skip_tool': 'Skip',
  'setup.git_title': 'Git Configuration',
  'setup.git_desc': 'Set up your Git identity for commits.',
  'setup.git_name': 'Name',
  'setup.git_email': 'Email',
  'setup.git_name_placeholder': 'e.g. John Doe',
  'setup.git_email_placeholder': 'e.g. john@example.com',
  'setup.git_save': 'Save',
  'setup.git_configured': 'Git configured',
  'setup.git_already': 'Git already configured',
  'setup.git_ssh_prompt': 'Generate SSH key for GitHub?',
  'setup.git_ssh_yes': 'Generate SSH Key',
  'setup.git_ssh_no': 'Skip SSH',
  'setup.git_ssh_generated': 'SSH key generated. Copy the public key above and add it to GitHub.',
  'setup.project_title': 'Project Folders',
  'setup.project_desc': 'Select folders to show in the sidebar.',
  'setup.project_scanning': 'Scanning...',
  'setup.project_none': 'No project folders found.',
  'setup.project_register': 'Add Selected',
  'setup.done_title': 'You\'re All Set!',
  'setup.done_desc': 'Your terminal is ready.\nRun `shelly setup` anytime to change these settings.',
  'setup.done_start': 'Start Using Shelly',
  'setup.path_error': 'npm not found in PATH. Bundled tools may not be extracted yet.',
  'setup.path_hint': 'Try restarting the app.',
  'setup.rerun_label': 'Re-run Setup Wizard',
  'setup.rerun_desc': 'Run the initial setup again',

  // ── Package Manager ───────────────────────────────────────────────
  'pkg.title': 'Packages',
  'pkg.search': 'Search packages...',
  'pkg.installed': 'Installed',
  'pkg.available': 'Available',
  'pkg.upgradable': 'Upgradable',
  'pkg.install': 'Install',
  'pkg.remove': 'Remove',
  'pkg.upgrade': 'Upgrade',
  'pkg.upgrade_all': 'Upgrade All',
  'pkg.refresh': 'Refresh',
  'pkg.installing': 'Installing {{name}}...',
  'pkg.removing': 'Removing {{name}}...',
  'pkg.no_packages': 'No packages found',
  'pkg.connect_first': 'Tools not available',

  // ── Dotfiles ──────────────────────────────────────────────────────
  'dotfiles.title': 'Dotfiles Sync',
  'dotfiles.sync_to_gist': 'Upload to Gist',
  'dotfiles.sync_from_gist': 'Download from Gist',
  'dotfiles.last_sync': 'Last synced: {{time}}',
  'dotfiles.never_synced': 'Never synced',
  'dotfiles.pat_required': 'GitHub Personal Access Token required',
  'dotfiles.syncing': 'Syncing...',

  // ── Plugin ────────────────────────────────────────────────────────
  'plugin.title': 'Plugins',
  'plugin.installed': 'Installed',
  'plugin.no_plugins': 'No plugins installed',
  'plugin.enable': 'Enable',
  'plugin.disable': 'Disable',
  'plugin.uninstall': 'Uninstall',

  // ── Accessibility ─────────────────────────────────────────────────
  'a11y.terminal_input': 'Terminal command input',
  'a11y.send_command': 'Send command',
  'a11y.copy_output': 'Copy output to clipboard',
  'a11y.run_snippet': 'Run snippet',
  'a11y.delete_snippet': 'Delete snippet',
  'a11y.open_palette': 'Open command palette',
  'a11y.close': 'Close',
  'a11y.expand': 'Expand details',
  'a11y.collapse': 'Collapse details',

  // Cerebras
  'settings.cerebras_title': 'Cerebras API',
  'settings.cerebras_key': 'Cerebras API Key',
  'settings.cerebras_key_desc': 'Chat AI — get from cloud.cerebras.ai (free)',
  'settings.cerebras_model': 'Cerebras Model',
  'settings.cerebras_configured': 'Cerebras configured ✓',

  // OpenRouter
  'settings.openrouter_title': 'OpenRouter API',
  'settings.openrouter_key': 'OpenRouter API Key',
  'settings.openrouter_key_desc': 'Chat AI — get from openrouter.ai/keys',
  'settings.openrouter_model': 'OpenRouter Model',
  'settings.openrouter_configured': 'OpenRouter configured ✓',
  'settings.cerebras_hint': 'Free: 1M tokens/day. Sign up at cloud.cerebras.ai',

  // Chat onboarding
  'onboarding.welcome': '🎉 Setup complete!\n\nThis is Shelly\'s main screen. You can type naturally and Shelly will execute commands for you.\n\nLet\'s try it — type **"file list"** below.',
  'onboarding.after_cmd': '👆 Your natural language was converted to a command and executed!\n\nNext, let\'s set up AI chat so you can have natural conversations.\n\n**Cerebras** is a free AI that responds to greetings, questions, and everyday conversation.',
  'onboarding.cerebras_prompt': '🔑 Get your free Cerebras API key:\n1. Open cloud.cerebras.ai\n2. Create an account (free)\n3. Copy your API key\n4. Paste it below',
  'onboarding.cerebras_done': '✅ Cerebras is ready! Try saying "hello" — you\'ll get a natural response.\n\n🎤 Want voice input too? Set up **Groq** (free) for speech-to-text.',
  'onboarding.groq_prompt': '🔑 Get your free Groq API key:\n1. Open console.groq.com\n2. Create an account (free)\n3. Copy your API key\n4. Paste it below',
  'onboarding.complete': '🚀 All set!\n\nWhat you can do:\n• Chat naturally — Cerebras responds\n• Say "build me an app" — Codex generates code\n• @codex / @local — choose a specific AI\n• Voice input 🎤 — tap the mic button\n• Terminal tab — full shell access\n\nAsk me anything!',
  'onboarding.setup_cerebras': 'Set up Cerebras (free)',
  'onboarding.setup_groq': 'Set up Groq (free)',
  'onboarding.skip': 'Set up later',
  'onboarding.open_browser': 'Open in browser',
  'onboarding.save_key': 'Save API key',
  'onboarding.key_placeholder': 'Paste API key here...',
  'onboarding.gemini_bridge': 'Codex is ready. Type `codex` in the terminal to start a coding session.',
  'onboarding.try_gemini': 'Got it!',
  'onboarding.gemini_bridge_next': 'Set up more AI tools',

  // CLI session
  'session.started': '{{cli}} session started. All messages will go to {{cli}} until you type "logout".',
  'session.ended': '{{cli}} session ended.',

  // Auto savepoint
  'savepoint.files_changed': '{{count}} file(s) changed',
  'savepoint.undo': 'Undo',
  'savepoint.reverted': 'Undone',
  'savepoint.view_changes': 'View changes',
  'savepoint.revert_failed': 'Could not undo the changes.',
  // Preview panel
  'preview.title': 'Preview',
  'preview.open': 'Preview',
  'preview.button': 'Preview',
  'preview.tab_web': 'Web',
  'preview.tab_code': 'Code',
  'preview.tab_files': 'Files',
  'preview.no_url': 'Start a dev server or open an HTML file',
  'preview.no_changes': 'No recent changes',
  'preview.connect_termux': 'Preview unavailable',
  'preview.truncated': 'File truncated (showing first 1000 lines)',
  'preview.binary': 'Binary file — cannot preview',
  'preview.pdf_unavailable': 'PDF preview unavailable',
  'preview.open_external': 'Open externally',
  'preview.back_terminal': 'Back to Terminal',
  'preview.diff_view': 'Diff view',
  'preview.full_file': 'Full file',
  // ProcessGuard
  'guard.title': 'Background Protection',
  'guard.problem': 'Android is force-stopping background processes. Your terminal sessions may be killed when you switch apps.',
  'guard.open_settings': 'Open Settings',
  'guard.copy': 'Copy',
  'guard.copied': 'Copied',
  'guard.next': 'Next',
  'guard.done': 'Done',
  'guard.back': 'Back',
  'guard.step_counter': 'Step {current} / {total}',
  // Setup Wizard — Protect step
  'setup2.protect_title': 'Background Protection',
  'setup2.protect_desc': 'Android may kill background processes. Follow these steps to keep Shelly running.',
  'setup2.protect_continue': 'Continue',
  'setup2.protect_skip': 'Skip',
  // FirstMate
  'firstmate.title': 'What would you like to do?',
  'firstmate.web': 'Web Dev',
  'firstmate.ai': 'AI Dev',
  'firstmate.files': 'File Mgmt',
  'firstmate.learning': 'Learn Code',
  'firstmate.skip': 'Skip for now',
  'firstmate.updating': 'Updating packages...',
  'firstmate.storage': 'Setting up storage access...',
  'firstmate.installing': 'Installing...',
  'firstmate.done': 'Done!',
  'firstmate.warn': 'Setup completed with some warnings.',
  // SmartKeyBar
  'keybar.default': 'Default',
  'keybar.vim': 'Vim',
  'keybar.git': 'Git',
  'keybar.repl': 'REPL',
  'keybar.nav': 'Nav',
  // VoiceChain
  'voice.tap_to_talk': 'Tap to talk',
  'voice.listening': 'Listening...',
  'voice.transcribing': 'Transcribing...',
  'voice.thinking': 'Thinking...',
  'voice.executing': 'Running command...',
  'voice.speaking': 'Speaking...',
  'voice.realtime_title': 'Realtime Voice',
  'voice.you_label': 'You:',
  'voice.ai_label': 'AI:',
  'voice.realtime_status_idle': 'Idle',
  'voice.realtime_status_connecting': 'Connecting…',
  'voice.realtime_status_listening': 'Listening — talk anytime',
  'voice.realtime_status_speaking': 'Speaking…',
  'voice.realtime_status_error': 'Error',
  'voice.realtime_key_required': 'Add a Gemini API key in Settings to use realtime voice.',
  'voice.realtime_error': 'Realtime voice error: {{error}}',
  'voice.auto_on': 'Auto-continue ON',
  'voice.auto_off': 'Auto-continue OFF',
  'voice.agent_ready_to_confirm': 'Agent "{{name}}" is ready. Tap Confirm to create it.',
  // Terminal Appearance
  'settings.terminal_appearance': 'Terminal Appearance',
  'settings.terminal_theme': 'Theme',
  'settings.terminal_appearance_desc': 'Theme, font lock, key bar',
  'settings.font_size_lock': 'Lock font size',
  'settings.font_size_lock_desc': 'Disable pinch-to-zoom in terminal',
  'settings.hide_keybar_hw': 'Auto-hide key bar',
  'settings.hide_keybar_hw_desc': 'Hide key bar when hardware keyboard is detected',
  // Slides (feature showcase during setup)
  'setup2.slide1_title': 'Just Talk to It',
  'setup2.slide1_desc': 'Type in natural language.\nShelly converts it to the right command.',
  'setup2.slide1_example': '"Show me all files" → ls -la',
  'setup2.slide2_title': 'Smart AI Selection',
  'setup2.slide2_desc': 'Codex, cloud APIs, or local AI —\nShelly automatically picks the best one.',
  'setup2.slide3_title': 'Organize with Words',
  'setup2.slide3_desc': '"Sort my photos by date"\nFile management in plain language.',
  'setup2.slide4_title': 'Voice Chat',
  'setup2.slide4_desc': 'Speak to Shelly and hear AI responses.\nHands-free terminal control.',
  'setup2.slide5_title': 'Create Projects',
  'setup2.slide5_desc': '"Make a Todo app"\nAI generates the whole project for you.',
  'setup2.slide6_title': 'Safe by Design',
  'setup2.slide6_desc': 'Dangerous commands are flagged\nbefore execution. Always in control.',

  // ── Auth Wizard ────────────────────────────────────────────────────
  'auth.title': 'AI Authentication',
  'auth.description': 'Sign in to your AI tools to start using them.\nChoose the easiest method for each tool.',
  'auth.bridge_required': 'Authentication unavailable.',
  'auth.status_summary': '{{authenticated}} / {{total}} tools authenticated',
  'auth.status_authenticated': 'Authenticated',
  'auth.status_not_authenticated': 'Not authenticated',
  'auth.status_not_installed': 'Not installed',
  'auth.status_checking': 'Checking...',
  'auth.browser_signin': 'Get API key in browser',
  'auth.browser_signin_desc': 'Opens the provider website to get your key',
  'auth.oauth_signin': 'Sign in with browser (recommended)',
  'auth.oauth_signin_desc': 'No API key needed. Just sign in with your browser',
  'auth.oauth_waiting': 'Waiting for auth... Complete in your browser',
  'auth.api_key_label': 'Paste your API key ({{envVar}})',
  'auth.api_key_placeholder': 'Paste your API key here...',
  'auth.save': 'Save',
  'auth.get_key_link': 'Get an API key (opens browser)',
  'auth.done': 'Done',
  'auth.skip': 'Skip for now',
  'auth.error': 'Error',
  'auth.oauth_timeout': 'Browser sign-in timed out. Please try again.',
  'auth.key_empty': 'Please enter an API key.',
  'auth.verify_failed': 'API key was saved but verification failed. Please check the key.',
  'auth.save_failed': 'Failed to save API key.',
  'auth.browser_failed': 'Could not open browser.',
  'auth.settings_title': 'AI Authentication',
  'auth.settings_subtitle': 'Manage API keys for CLI tools',
  'auth.settings_button': 'Manage AI Authentication',

  // ── Chat Screen (index.tsx) ─────────────────────────────────────
  'chat.demo_suffix': '*Demo mode* — Set up an API key to run real commands',
  'chat.demo_suffix_short': '*Demo mode*',
  'chat.demo_claude': "Hello! Codex is available for coding tasks.\n\nTry `@codex review my code` to ask questions.\n\n*Demo mode*",
  'chat.demo_gemini': "Hello! Add a Gemini API key to use Google Gemini in Shelly.\n\nTry `@gemini summarize this` or `@local summarize this`.\n\n*Demo mode*",
  'chat.demo_perplexity': "Hello! I'm Perplexity AI.\n\nThis is a **demo mode** response. To use for real:\n- Go to **Settings** → set Perplexity API key\n\nSpecializes in web search-based answers with citations.\n\n*Demo mode*",
  'chat.demo_local': "Hello! I'm a Local LLM.\n\nThis is a **demo mode** response. To use for real:\n- Set up **Ollama** (`ollama serve`)\n- Enable Local LLM in **Settings**\n\nRuns as a private, offline AI.\n\n*Demo mode*",
  'chat.demo_welcome': "Welcome to Shelly!\n\nCurrently running in **demo mode**.\n\nTo get started:\n1. Open **Settings** → sign in or set API keys for AI tools\n2. Try the examples below\n\nExamples:\n- `ls` — List files\n- `@codex question` — Codex\n- `@local question` — Local LLM",
  'chat.context_analyzing': 'Analyzing...',
  'chat.context_gen_failed': 'context.md generation failed: {{error}}',
  'chat.cmd_running': 'Running...',
  'chat.connect_termux': 'Terminal unavailable.',
  'chat.not_connected': 'Not connected',
  'chat.no_ai_configured': 'No AI configured. Use @mention to specify a tool, or set up in Settings.\n\nExamples: `@codex question`, `@local question`',
  'chat.opening_browser': 'Opening in browser: {{url}}',
  'chat.unsupported_agent': 'Unsupported agent: @{{target}}\n\nAvailable agents: @gemini, @codex, @local, @perplexity, @team, @git',
  'chat.delete_title': 'Delete message',
  'chat.delete_confirm': 'Delete this message?',
  'chat.cancel': 'Cancel',
  'chat.delete': 'Delete',
  'chat.delete_message_title': 'Delete message',
  'chat.delete_message_body': 'Delete this message?',
  'chat.clear_conversation_button': 'Clear conversation',
  'chat.clear_conversation_title': 'Clear conversation',
  'chat.clear_conversation_body': 'Delete all messages in this conversation?',
  'chat.safety_risk': '{{level}} RISK',
  'chat.safety_confirm': 'Confirm before running',
  'chat.safety_cancel': 'Cancel',
  'chat.safety_run': 'Run',

  // ── Chat Message List ───────────────────────────────────────────
  'chat.empty_title': 'Shelly',
  'chat.companion_label': 'Shelly',
  // 2026-08-24 (Fable5 design consult, "一人の相棒" Phase 3): dropped the
  // internal "thread" vocabulary — a user was never told what a "thread"
  // is, so both variants read as an app-internals leak rather than
  // something a companion would actually say. Reworded as the same
  // persona picking up (or setting down) a different tool, without
  // naming which specific provider (that's shown in the pane header/
  // border color instead — see PaneSlot.tsx's provider badge) so this
  // stays accurate for a plain-key comparison this function already does
  // (it can't distinguish "gemini -> cerebras" from "gemini -> gemini"
  // without a provider param it doesn't currently take).
  'chat.switched_to_companion_thread': "I'll take it from here.",
  'chat.switched_to_pane_thread': "I'll think this one through with a different model.",
  // G1-P2 (2026-08-25): used instead of the two keys above when
  // carryForwardOnThreadSwitch actually found continuity to bring along —
  // see store/ai-pane-store.ts's addAiPaneThreadSwitchNotice.
  'chat.carried_forward_to_companion': "I brought our conversation with me — I'll take it from here.",
  'chat.carried_forward_to_pane': "I brought our conversation along — I'll think this one through with a different model.",
  'chat.empty_subtitle': 'Ask anything. I can see your terminal output.',
  // Fable5 review item #7 (2026-08-25): one-time first-agent onboarding
  // nudge, shown as a plain companion chat message (never a modal/wizard —
  // see CLAUDE.md's "旧 AuthWizard / WelcomeWizard は廃止") when the device
  // has no registered agents yet. The example utterance is parser-verified
  // against lib/agent-nl-parser.ts's parseAgentNL — see
  // __tests__/agent-onboarding-nudge.test.ts.
  'chat.agent_onboarding_nudge': "I can also run things for you in the background — that's an **agent**. Describe a task once in plain English and I'll keep doing it on a schedule (or right now).\n\nTry telling me:\n`every day at 8am, summarize the news`\n\nI'll show you exactly what I'm about to set up before anything runs.",
  // Fable5 product-review Gap A (2026-08-25): one-time plain-chat notice —
  // never a modal/card, same standing rule as chat.agent_onboarding_nudge —
  // posted by lib/agent-companion-notice.ts's
  // postCompanionJournalDormancyNotice the first time
  // lib/companion-journal.ts's digestConversationForJournal detects it had
  // something worth journaling but no local LLM configured to write it
  // with. See AppSettings.companionJournalDormancyNoticeShown.
  'chat.companion_journal_dormant': "By the way, I can't save notes from our chats yet — no local LLM is set up. Add one in Settings → Local LLM if you'd like me to start remembering things between conversations.",
  // Design 2-a ("welcome back" session-start greeting, 2026-08-28): a plain
  // companion chat message — never a modal/card, same standing rule as the
  // onboarding nudge above — appended once per app-process-lifetime when
  // there's a recent companion-journal note to reference. Deliberately
  // TEMPLATE-based, never handed to the local model to freeform-generate —
  // see lib/companion-greeting.ts's module doc for why (quality-floor risk:
  // self-generated fake confirmations / refusal-flavored non-answers).
  'chat.companion_greeting': 'Welcome back. Last time we were talking about "{{topic}}" — happy to pick that back up, or start something new.',
  // Design 2-b: used instead of the plain greeting above when the note being
  // referenced came from an explicit "keep an eye on X" watch-phrase rather
  // than an ordinary journal note — see lib/companion-greeting.ts.
  'chat.companion_greeting_watch': 'By the way, you asked me to keep an eye on "{{topic}}" — how\'s that going?',
  'chat.sample_list_files': 'List files',
  'chat.sample_ask_claude': 'Ask Codex',
  'chat.sample_ask_gemini': 'Ask Gemini',
  'chat.sample_git_status': 'Git status',
  'chat.sample_files': 'List files',
  'chat.sample_hello': 'Hello',
  'chat.sample_build': 'Build me an app',
  'chat.stop_generating': 'Stop generating',

  // ── Chat Bubble ─────────────────────────────────────────────────
  'chat.thinking': 'Thinking...',
  'chat.sources': 'Sources:',

  // ── Command Input ───────────────────────────────────────────────
  'input.placeholder_stdin': 'stdin...',
  'input.placeholder_ai': 'Ask anything...',
  'input.placeholder_shell': 'Enter command...',
  'input.recording': 'Recording...',
  'input.sensitive_title': 'Sensitive file detected',
  'input.sensitive_message': '{{name}} may contain sensitive information.\nSend to AI anyway?',
  'input.sensitive_cancel': 'Cancel',
  'input.sensitive_attach': 'Attach',
  'input.gallery': 'Gallery',
  'input.camera': 'Camera',
  'input.file': 'File',
  'input.paste': 'Paste',
  'input.voice': 'Voice',

  // ── Mention Dropdown ────────────────────────────────────────────
  'mention.local_desc': 'Ask local AI',
  'mention.claude_desc': 'Legacy agent removed',
  'mention.codex_desc': 'Fast code assistant',
  'mention.cerebras_desc': 'Fast chat (Qwen3-235B)',
  'mention.gemini_desc': 'Google Gemini API',
  'mention.perplexity_desc': 'Real-time web search',
  'mention.git_desc': 'Git operations guide',
  'mention.team_desc': 'Multi-AI parallel',
  'mention.browser_desc': 'Open URL in browser',
  'mention.plan_desc': 'Step-by-step plan',
  'mention.arena_desc': 'Blind AI comparison',
  'mention.actions_desc': 'Set up CI/CD',

  // ── AI Dispatch ─────────────────────────────────────────────────
  'dispatch.gemini_no_key': 'Gemini API key is not set. Add it in Settings → API Keys.',
  'gemini_api_key_missing': 'Gemini API key is not set. Enter it in Settings.',
  'dispatch.perplexity_no_key': 'Perplexity API key is not set.\nGet one at https://www.perplexity.ai/settings/api',
  'dispatch.api_key_not_set': 'API key not set',
  'dispatch.delegated_to': 'Delegated to {{tool}}.\nReason: {{reason}}',
  'dispatch.local_llm_error': 'Could not connect to Local LLM. Check connection in Settings.',
  'dispatch.team_no_agents': 'No agents available for @team.\nEnable agents in Settings.',
  'dispatch.team_asking': 'Asking {{count}} agents...',
  'dispatch.claude_connect': 'This provider was removed. Use @codex or @local.',
  'dispatch.agent_no_key': 'No AI provider is configured for @agent.',
  'dispatch.agent_no_termux': '@agent is unavailable. Check setup in Settings.',
  'dispatch.termux_not_connected': 'Terminal not available',
  'dispatch.agent_started': 'Agent mode started',

  // ── Local LLM ───────────────────────────────────────────────────
  'llm.timeout_large': 'Timeout (60s). The model may be too large.',
  'llm.timeout': 'Timeout. The model may be too large.',
  'llm.connect_failed': 'Could not connect to local LLM. Make sure llama-server is running.',
  'llm.model_missing': 'Local LLM model is not installed. Select or download a model in Settings.',
  'llm.autostart_failed': 'Could not start llama-server. Check Local LLM setup in Settings.',
  'llm.category_chat': 'Chat',
  'llm.category_code': 'Code Generation',
  'llm.category_research': 'Research',
  'llm.category_file_ops': 'File Operations',
  'llm.category_unknown': 'Unknown',

  // ── Intent Router ───────────────────────────────────────────────
  'intent.not_installed': '{{name}} is not installed yet. {{desc}}\n\nStart setup?',
  'intent.needs_setup': '{{id}} needs to be set up. Start installation?',
  'intent.delegated': 'Delegated to {{label}}.\nReason: {{reason}}',

// ── Terminal Screen ─────────────────────────────────────────────
  'terminal.connecting_terminal': 'Starting terminal...',
  'terminal.retry_count': 'Retry {{count}} / 5',
  'terminal.connected': 'Connected',
  'terminal.connecting': 'Starting...',
  'terminal.connecting_retry': 'Starting ({{count}})',
  'terminal.disconnected': 'Disconnected',
  'terminal.jp_placeholder': 'Japanese input...',
  'terminal.paste': 'Paste',
  'terminal.cannot_connect': 'Cannot connect to terminal',
  'terminal.cannot_connect_desc': 'Could not connect to {{url}}.\nFollow these steps to set up.',
  'terminal.one_tap_setup': 'One-tap setup',
  'terminal.one_tap_copied': 'Copied!',
  'terminal.one_tap_desc': 'Command copied to clipboard',
  'terminal.or_manually': '— or manually —',
  'terminal.manual_setup': 'MANUAL SETUP',
  'terminal.install_tools': 'Install terminal tools',
  'terminal.add_autostart': 'Add auto-start to .bashrc',
  'terminal.open_termux': 'Open terminal',
  'terminal.reload_tab': 'Reload this tab',
  'terminal.reload': 'Reload',

  // ── Search Screen ───────────────────────────────────────────────
  'search.title': 'Command History',
  'search.items': '{{count}} items',
  'search.placeholder': 'Search commands and output...',
  'search.no_commands': 'No commands have been run yet',
  'search.no_results': 'No results found',

  // ── Projects Screen ─────────────────────────────────────────────
  'projects.title': 'Projects',
  'projects.new_chat': 'New Chat',
  'projects.chats': 'Chats ({{count}})',
  'projects.folders': 'Folders',
  'projects.folders_count': 'Folders ({{count}})',
  'projects.search_chats': 'Search chats...',
  'projects.no_messages': 'No messages',
  'projects.messages_count': '{{count}} messages',
  'projects.delete_title': 'Delete Chat',
  'projects.delete_confirm': 'Delete "{{title}}"?',
  'projects.cancel': 'Cancel',
  'projects.delete': 'Delete',
  'projects.connect_termux': 'No project folders available',
  'projects.scanning': 'Scanning...',
  'projects.no_folders': 'No folders found in\n~/dev/ or ~/projects/',
  'projects.no_chats': 'No chats yet.\nTap + New Chat to start.',
  'projects.no_matching': 'No matching chats',

  // ── Command Execution ─────────────────────────────────────────────
  'exec.open_in_terminal': 'Open in Terminal',

  // ── Timeline ─────────────────────────────────────────────────────
  'timeline.title': 'Timeline',
  'timeline.view_diff': 'View changes',
  'timeline.revert': 'Revert to this point',
  'timeline.revert_confirm': 'Revert to this point? Unsaved changes will be lost.',
  'timeline.revert_success': 'Reverted successfully',
  'timeline.revert_fail': 'Failed to revert',
  'timeline.empty': 'No savepoints yet',
  'timeline.show_more': 'Show more',
  'timeline.action_title': 'Savepoint',

  // ── Creator Screen ──────────────────────────────────────────────
  'creator.tagline': 'Create with natural language',
  'creator.tab_create': 'Create',
  'creator.tab_history': 'History',
  'creator.tab_tools': 'Tools',
  'creator.sent_title': 'Sent to Terminal',
  'creator.sent_message': 'Check and run the command in the Terminal tab.',
  'creator.open_folder': 'Open folder',
  'creator.open_folder_msg': 'Location: ~/Projects/{{path}}',
  'creator.location_title': 'Location',
  'creator.location_msg': '~/Projects/{{path}}',
  'creator.export_empty': 'No projects yet.',
  'creator.export_failed': 'Export failed. Please try again.',

  // ── Obsidian Screen ─────────────────────────────────────────────
  'obsidian.setup_required': 'Setup required',
  'obsidian.setup_gemini': 'Go to Settings → AI → set up an AI provider.',
  'obsidian.setup_vault': 'Go to Settings → Obsidian → set Vault Path.',
  'obsidian.collect': 'Collect',
  'obsidian.collecting': 'Collecting from arXiv / Semantic Scholar / Perplexity...',
  'obsidian.no_briefing': 'No briefing for today',
  'obsidian.collect_hint': 'Tap "Collect" to fetch\nthe latest STEAM/EdTech updates',
  'obsidian.collect_now': 'Collect now',
  'obsidian.back_list': 'List',
  'obsidian.back_detail': 'Detail',
  'obsidian.discuss': 'Discuss',
  'obsidian.sns_draft': 'SNS Draft',
  'obsidian.research': 'Research',
  'obsidian.copy_url': 'Copy URL',
  'obsidian.copied': 'Copied',
  'obsidian.discussion': 'Discussion',
  'obsidian.clear': 'Clear',
  'obsidian.chat_empty': 'Ask questions or discuss this article/paper freely.\n\nExamples:\n"What are the limitations of this study?"\n"How could this apply in practice?"\n"What related prior research exists?"',
  'obsidian.chat_placeholder': 'Type a question or thought...',
  'obsidian.send': 'Send',
  'obsidian.api_not_configured': 'API key not configured. Please check Settings → AI.',
  'obsidian.generate': 'Generate',
  'obsidian.sns_generate_hint': 'Tap "Generate" to create\ndrafts for X, Threads, and note at once',
  'obsidian.writing_gemini': 'Writing with AI...',
  'obsidian.over_limit': 'over limit',
  'obsidian.generation_failed': 'Generation failed: {{error}}',
  'obsidian.research_assistant': 'Research Assistant',
  'obsidian.task_outline': 'Paper Outline',
  'obsidian.task_related': 'Related Work',
  'obsidian.task_critique': 'Critical Analysis',
  'obsidian.task_abstract': 'Abstract Draft',
  'obsidian.task_outline_desc': 'Create a paper outline based on this research',
  'obsidian.task_related_desc': 'Suggest related prior research and literature',
  'obsidian.task_critique_desc': 'Analyze limitations, issues, and improvements',
  'obsidian.task_abstract_desc': 'Draft an abstract for your paper',
  'obsidian.select_generate': 'Select a task and tap "Generate"',
  'obsidian.generating_gemini': 'Generating with AI...',
  'obsidian.no_response': 'No response',

  // ── Settings Screen (section headers & labels) ──────────────────
  'settings.display_title': 'Display',
  'settings.glass_title': 'Glass background',
  'settings.glass_subtitle': 'Customize terminal with wallpaper and opacity',
  'settings.theme_title': 'Theme',
  'settings.cursor_title': 'Cursor shape',
  'settings.behavior_title': 'Behavior',
  'settings.llm_interpreter_label': 'Learning Mode (AI Interpreter)',
  'settings.llm_interpreter_desc': 'AI explains command results in natural language after execution',
  'settings.external_keyboard_label': 'External Keyboard Shortcuts',
  'settings.external_keyboard_desc': 'Show ShortcutBar in Terminal tab',
  'settings.sound_title': 'Sound & Effects',
  'settings.advanced_show': 'Show advanced settings',
  'settings.advanced_hide': 'Hide advanced settings',
  'settings.termux_title': 'Terminal',
  'settings.termux_subtitle': 'Native terminal (JNI forkpty) — no setup required',
  'settings.local_llm_title': 'Local LLM (llama-server)',
  'settings.local_llm_subtitle': 'Experimental & advanced — the recommended 2B model uses about 2-3GB RAM. Runs llama-server locally for AI chat',
  'settings.use_local_llm': 'Use Local LLM',
  'settings.mcp_title': 'MCP Servers',
  'settings.mcp_subtitle': 'Server plugins for compatible MCP clients',
  'settings.custom_context_title': 'Custom Context',
  'settings.custom_context_subtitle': 'Markdown auto-injected into Local LLM. Write design principles or rules',
  'settings.cli_approve_title': 'CLI Auto-Approve',
  'settings.cli_approve_subtitle': 'Permission approval when using AI CLI tools via Chat tab',
  'settings.approve_none': 'All manual',
  'settings.approve_none_desc': 'Confirm every time (safe)',
  'settings.approve_safe': 'Read-only auto',
  'settings.approve_safe_desc': 'Auto-approve file reads',
  'settings.approve_all': 'Full auto',
  'settings.approve_all_desc': 'Auto-approve all (advanced)',
  'settings.default_agent_title': 'Default Agent',
  'settings.default_agent_subtitle': 'Preferred CLI when Local LLM is not in use',
  'settings.groq_title': 'Groq API',
  'settings.groq_subtitle': 'Ultra-fast chat & Whisper transcription (Llama 3.3 70B)',
  'settings.perplexity_title': 'Perplexity API',
  'settings.perplexity_subtitle': 'Paper & web search. Invoke with @perplexity',
  'settings.gemini_api_title': 'Gemini API',
  'settings.gemini_api_subtitle': 'Use a Google AI Studio API key for Gemini.',
  'settings.team_title': '@team Table',
  'settings.team_subtitle': 'Ask multiple AIs the same question, local LLM synthesizes results',
  'settings.obsidian_title': 'Obsidian',
  'settings.obsidian_subtitle': 'Auto-collection & knowledge management settings',
  'settings.snippets_title': 'Snippets',
  'settings.snippets_subtitle': 'Configure how command snippets are executed',
  'settings.snippet_backup_title': 'Snippet Backup',
  'settings.project_backup_title': 'Project Backup',
  'settings.data_title': 'Data',
  'settings.diagnostics_title': 'Diagnostics',
  'settings.diagnostics_subtitle': 'Check connection status and installed tools at once',
  'settings.about_title': 'About',
  'settings.font_size_label': 'Font size',
  'settings.font_size_desc': 'Current: {{size}}px',
  'settings.line_height_label': 'Line height',
  'settings.line_height_desc': 'Current: {{height}}x',
  'settings.bg_opacity_label': 'Background opacity',
  'settings.blur_label': 'Blur intensity',
  'settings.haptic_label': 'Tap vibration',
  'settings.haptic_desc': 'Vibrate on button press',
  'settings.auto_scroll_label': 'Auto-scroll',
  'settings.auto_scroll_desc': 'Scroll to bottom after output',
  'settings.high_contrast_label': 'High contrast output',
  'settings.high_contrast_desc': 'Readable white/black output on OLED',
  'settings.experience_label': 'Experience mode',
  'settings.experience_learning': 'Learning mode (for beginners)',
  'settings.experience_fast': 'Fast mode (for experienced)',
  'settings.sound_label': 'Sound effects',
  'settings.sound_desc': 'Sound feedback on UI actions',
  'settings.auto_reconnect_label': 'Auto-reconnect',
  'settings.auto_reconnect_desc': 'Auto-reconnect on disconnect (max 5 times)',
  'settings.timeout_label': 'Timeout',
  'settings.timeout_desc': 'Command timeout: {{seconds}}s',
  'settings.api_key': 'API Key',
  'settings.model': 'Model',
  'settings.groq_configured': '✓ Configured — default for chat & voice transcription',
  'settings.groq_hint': 'https://console.groq.com — free, no credit card required',
  'settings.perplexity_configured': '✓ Configured — available for @perplexity paper search',
  'settings.perplexity_hint': 'https://www.perplexity.ai/settings/api',
  'settings.gemini_configured': '✓ Gemini API configured',
  'settings.gemini_hint': 'https://aistudio.google.com/app/apikey',
  'settings.auto_collect_label': 'Auto-collect',
  'settings.auto_collect_desc': 'Automatically collect articles and papers at a scheduled time each morning',
  'settings.collect_time_label': 'Collection time',
  'settings.collect_time_desc': 'Time to run auto-collection (e.g. 6:00)',
  'settings.max_items_label': 'Max items/day',
  'settings.max_items_desc': 'Maximum number of articles/papers to collect per day',
  'settings.collect_period_label': 'Collection period',
  'settings.collect_period_desc': 'Only collect articles/papers from the last N days',
  'settings.run_mode_label': 'Run mode',
  'settings.return_terminal_label': 'Return to Terminal',
  'settings.return_terminal_desc': 'Auto-switch to Terminal tab after running a snippet',
  'settings.clear_all': 'Clear all sessions',
  'settings.export_logs': 'Export logs',

  // ── Settings Screen (extra keys) ────────────────────────────────
  'settings.display': 'Display',
  'settings.reset_title': 'Reset',
  'settings.reset_action': 'Reset all settings to defaults',
  'settings.reset_confirm_title': 'Reset all settings?',
  'settings.reset_confirm_body': 'This resets font, theme, API keys stored here, local LLM config, and every other setting back to defaults. Paired conversations, saved agents, and skills are not affected.',
  'settings.reset_done_toast': 'Settings reset to defaults.',
  'settings.reset_hints_action': 'Reset one-time hints',
  'settings.reset_hints_done_toast': 'One-time hints will show again.',
  'settings.profile_learning_label': 'Profile Learning',
  'settings.profile_learning_desc': 'Learn command, project, and AI usage patterns locally for personalized suggestions and AI context.',
  'settings.nacre_bridge_label': 'Nacre Bridge',
  'settings.nacre_bridge_desc': 'While Shelly is in the foreground, share your current directory, git branch, and safe recent command terms (never raw commands or secrets) with the Nacre IME to improve its conversion suggestions. Default on.',
  'settings.profile_facts_label': 'View/Edit Facts',
  'settings.profile_facts_desc': 'View learned facts and delete individual entries.',
  'settings.profile_facts_action': 'View',
  'settings.profile_facts_title': 'Learned facts',
  'settings.profile_facts_empty': 'No learned facts yet.',
  'settings.profile_facts_delete_hint': 'Long-press a fact to delete it.',
  'settings.profile_facts_load_failed_title': 'Could not load profile facts',
  'settings.profile_fact_delete_title': 'Delete fact',
  'settings.profile_fact_delete_body': 'Delete this learned fact?',
  'settings.profile_fact_delete_failed_title': 'Could not delete profile fact',
  'settings.profile_reset_label': 'Reset Profile',
  'settings.profile_reset_desc': 'Delete learned local profile facts and usage patterns.',
  'settings.profile_reset_action': 'Reset',
  'settings.profile_reset_confirm_title': 'Reset profile?',
  'settings.profile_reset_confirm_body': 'This deletes Shelly’s learned command, project, AI usage, and preference profile from this device.',
  'settings.profile_reset_done_toast': 'Profile reset.',
  'settings.profile_reset_failed_title': 'Profile reset failed',
  'settings.font_size_current': 'Current: {{size}}px',
  'settings.line_height_current': 'Current: {{height}}x',
  'settings.glass_bg': 'Glass background',
  'settings.glass_bg_desc': 'Customize terminal with wallpaper and opacity',
  'settings.wallpaper': 'Wallpaper',
  'settings.change_wallpaper': 'Change wallpaper',
  'settings.select_wallpaper': 'Select wallpaper',
  'settings.wallpaper_set': 'Wallpaper set — adjust opacity below',
  'settings.wallpaper_hint': 'Select an image from your photo library',
  'settings.bg_opacity': 'Background opacity',
  'settings.bg_opacity_desc': 'Terminal background opacity: {{value}}%',
  'settings.blur_intensity': 'Blur intensity',
  'settings.blur_intensity_desc': 'Wallpaper blur amount: {{value}}',
  'settings.not_connected': 'Not connected',
  'settings.error_label': 'Error',
  'settings.url_error': 'URL Error',
  'settings.url_ws_hint': 'URL must start with ws:// or wss://',
  'settings.url_http_hint': 'URL must start with http:// or https://',
  'settings.saved': 'Saved',
  'settings.ws_url_updated': 'WebSocket URL updated:\n{{url}}',
  'settings.llm_url_updated': 'Local LLM URL updated:\n{{url}}',
  'settings.model_error': 'Please enter a model name',
  'settings.model_updated': 'Model updated: {{model}}',
  'settings.llm_model_hint': 'Recommended: Qwen3.5 0.8B. Use 0.8B for autonomous always-on work, 2B for heavier reasoning, and 4B only for short quality checks.',
  'settings.llm_success_title': 'llama-server connection successful',
  'settings.llm_success_msg': 'Connected to llama-server.',
  'settings.llm_fail_title': 'llama-server connection failed',
  'settings.llm_fail_msg': '{{url}} : connection failed.\n\nCheck if llama-server is running:\n$ llama-server --model ~/models/Qwen3.5-0.8B-Q4_K_M.gguf --port 8080 --host 127.0.0.1',
  'settings.llm_fail_error': 'Error occurred during connection.',
  'settings.save_failed': 'Save failed',
  'settings.unexpected_error': 'An unexpected error occurred.',
  'settings.update_failed': 'Update failed',
  'settings.update_failed_msg': 'Failed to write file:\n{{error}}',
  'settings.export': 'Export',
  'settings.export_empty': 'No projects yet.',
  'settings.export_failed': 'Export failed. Please try again.',
  'settings.export_no_snippets': 'No snippets. Save snippets first.',
  'settings.export_cancelled': 'Cancelled.',
  'settings.export_no_logs': 'No logs. Run some commands first.',
  'settings.export_log_error': 'Failed to export logs',
  'settings.clear_all_title': 'Clear all sessions',
  'settings.clear_all_msg': 'Delete all command history?',
  'settings.clear_cancel': 'Cancel',
  'settings.clear_delete': 'Delete',

  // ── GitHub Integration ─────────────────────────────────────────────
  'github.suggest_sync': '{count} savepoints not synced to GitHub. Sync now?',
  'github.sync': 'Sync',
  'github.later': 'Later',
  'github.setup_pat': 'To publish to GitHub, you need a Personal Access Token (PAT).',
  'github.pat_steps': '1. Tap "Open in browser" below\n2. Tap "Generate new token (classic)"\n3. Check "repo" scope\n4. Tap "Generate token"\n5. Copy the token and paste it below',
  'github.open_browser': 'Open in browser',
  'github.paste_token': 'Paste your token:',
  'github.set_token': 'Set token',
  'github.connected': 'Connected to GitHub as @{username}',
  'github.invalid_token': 'Invalid token. Please check and try again.',
  'github.repo_name': 'Repository name',
  'github.publish': 'Publish',
  'github.public': 'Public',
  'github.private': 'Private',
  'github.publishing': 'Publishing to GitHub...',
  'github.published': 'Published! {url}',
  'github.syncing': 'Syncing to GitHub...',
  'github.synced': 'Synced ({count} savepoints)',
  'github.view': 'View on GitHub',
  'github.disconnect': 'Disconnect GitHub',
  'github.auth_expired': 'Your GitHub token may have expired.',
  'github.reconfigure': 'Reconfigure',
  'github.retry': 'Try again',
  'github.ci_detecting': 'Analyzing project structure...',
  'github.ci_confirm': 'Set up automatic builds and tests on every push?',
  'github.ci_setup': 'Set up CI',
  'github.ci_customize': 'Customize',
  'github.ci_done': 'CI configured! Builds run automatically on push.',
  'github.build_success': 'Latest build: ✅ Passed ({time})',
  'github.build_failure': 'Latest build: ❌ Failed',
  'github.view_logs': 'View logs',
  'github.fix_errors': 'Fix errors',

  // ── Auto-check proposal (after push) ────────────────────────────
  'autocheck.proposal_title': 'Auto-check available',
  'autocheck.proposal_body': 'Want to automatically check if your code is working every time you save to GitHub? Results show up right here in chat.',
  'autocheck.btn_enable': 'Turn on',
  'autocheck.btn_later': 'Maybe later',
  'autocheck.setting_up': 'Setting up auto-check...',
  'autocheck.done': 'Auto-check is on! Every time you save to GitHub, it\'ll check your code and let you know right here.',
  'autocheck.error': 'Couldn\'t set up auto-check: {error}',
  'autocheck.result_pass': 'Auto-check complete — everything looks good!',
  'autocheck.result_fail': 'Auto-check found a problem',
  'autocheck.view_details': 'See details',
  'autocheck.polling': 'Checking your code...',
  'autocheck.timeout': 'Auto-check is still running. You can check results on GitHub later.',

  // ── Advanced CI wizard (Settings) ──────────────────────────────
  'wizard.actions_title': 'Auto-check Settings',
  'wizard.step1_title': 'What to check?',
  'wizard.step1_desc': 'Pick what to check automatically (you can pick multiple)',
  'wizard.action_build': 'Build check',
  'wizard.action_test': 'Run tests',
  'wizard.action_deploy': 'Deploy',
  'wizard.action_release': 'Create release',
  'wizard.step2_title': 'When to check?',
  'wizard.trigger_push': 'Every time you save',
  'wizard.trigger_daily': 'Once a day',
  'wizard.trigger_manual': 'Only when I ask',
  'wizard.step3_title': 'Looks good?',
  'wizard.confirm_actions': 'Checking: {actions}',
  'wizard.confirm_trigger': 'When: {trigger}',
  'wizard.confirm_notify': 'Results show up in chat',
  'wizard.btn_next': 'Next',
  'wizard.btn_back': 'Back',
  'wizard.btn_setup': 'Set up',
  'wizard.btn_redo': 'Start over',
  'wizard.detecting': 'Looking at your project...',
  'wizard.setting_up': 'Setting things up...',
  'wizard.done': 'All set! Auto-check will run {trigger}.',
  'wizard.done_push': 'every time you save',
  'wizard.done_daily': 'once a day',
  'wizard.done_manual': 'when you ask',
  'wizard.error': 'Something went wrong: {error}',
  'wizard.no_pat': 'Connect to GitHub first in Settings.',
  'wizard.no_remote': 'Save your project to GitHub first.',
  'wizard.select_one': 'Pick at least one',

  // ── Template Gallery ───────────────────────────────────────────
  'template.web': 'Web App',
  'template.api': 'API Server',
  'template.cli': 'CLI Tool',
  'template.mobile': 'Mobile App',
  'template.static': 'Static Site',
  'template.script': 'Script',
  'template.document': 'Docs',

  // ── Error Summary Bubble ───────────────────────────────────────
  'error_summary.header': 'Error detected',
  'error_summary.suggest_fix': 'Suggest fix',
  'error_summary.ask_team': 'Ask @team',
  'error_summary.suggest_fix_prompt': 'Suggest a fix for this error: {{errorText}}\nTranslation: {{translation}}',
  'error_summary.ask_team_prompt': 'Help with this error: {{errorText}}\nTranslation: {{translation}}',

  // ── Approval Bubble ────────────────────────────────────────────
  'approval.header': 'AI is requesting permission',
  'approval.approved': 'Approved',
  'approval.denied': 'Denied',
  'approval.allow': 'Allow',
  'approval.deny': 'Deny',
  'approval.ask_team': 'Ask @team',
  'approval.ask_team_prompt': 'Approval proxy: {{command}}\nTranslation: {{translation}}',

  // ── Edit Sheet ─────────────────────────────────────────────────
  'edit_sheet.preset_larger': 'Larger',
  'edit_sheet.preset_smaller': 'Smaller',
  'edit_sheet.preset_color': 'Change color',
  'edit_sheet.preset_bold': 'Bold',
  'edit_sheet.preset_spacing': 'More spacing',
  'edit_sheet.preset_delete': 'Delete',
  'edit_sheet.preset_larger_inst': 'Make this font-size 1.5x',
  'edit_sheet.preset_smaller_inst': 'Make this font-size 0.75x',
  'edit_sheet.preset_color_inst': 'Change this element color to something more prominent',
  'edit_sheet.preset_bold_inst': 'Make this font-weight bold',
  'edit_sheet.preset_spacing_inst': 'Make this padding and margin 1.5x',
  'edit_sheet.preset_delete_inst': 'Hide this element (display: none)',
  'edit_sheet.placeholder': 'How do you want to change it?',

  // ── Plan Card ──────────────────────────────────────────────────
  'plan.execute': 'Run',
  'plan.next': 'Next',
  'plan.skip': 'Skip',

  // ── Snippet Import ─────────────────────────────────────────────
  'import.title': 'Import Snippets',
  'import.select_file_desc': 'Please select a Shelly format JSON file.',
  'import.select_file': 'Select file',
  'import.validating': 'Validating...',
  'import.retry': 'Try again',
  'import.preview': 'Preview',
  'import.total': 'Total: {{n}}',
  'import.new_count': 'New: {{n}}',
  'import.duplicate_count': 'Duplicate: {{n}}',
  'import.duplicate_handling': 'Handle duplicates',
  'import.strategy_skip': 'Skip (keep existing)',
  'import.strategy_overwrite': 'Overwrite (update with new data)',
  'import.strategy_keep_both': 'Keep both',
  'import.execute': 'Import',
  'import.importing': 'Importing...',
  'import.done': 'Done',
  'import.close': 'Close',
  'import.file_read_error': 'Failed to read the file',

  // ── Creator: Plan Lane ─────────────────────────────────────────
  'creator.plan_placeholder': 'Enter what you want to build, and the plan will appear here.',
  'creator.plan_thinking': 'Thinking...',
  'creator.plan_files': '{{n}} files',
  'creator.plan_cancel': 'Cancel',
  'creator.plan_confirm': 'Build ▶',
  'creator.type_web': 'Web App',
  'creator.type_script': 'Script',
  'creator.type_document': 'Document',
  'creator.type_unknown': 'Project',

  // ── Creator: Build Lane ────────────────────────────────────────
  'creator.build_placeholder': 'After confirming the plan, build logs will stream here.',

  // ── Creator: Result Lane ───────────────────────────────────────
  'creator.result_placeholder': 'When complete, results and next actions will appear here.',
  'creator.result_error': "Something went wrong. Please try again.",
  'creator.result_retry': 'Try again',
  'creator.result_save_location': 'Save location',
  'creator.result_file_count': 'File count',
  'creator.result_files': '{{n}} files',
  'creator.result_type': 'Type',
  'creator.result_suggestions': 'What you can do next',
  'creator.result_open_folder': 'Open folder',
  'creator.result_run_terminal': 'Run in Terminal',
  'creator.result_recipe_saved': 'Recipe saved',
  'creator.result_save_recipe': 'Save as Recipe',
  'creator.result_new_project': 'Create new',

  // ── Creator: AI Block ──────────────────────────────────────────
  'ai_block.tool_suggest_label': 'Which tool would you like to use?',
  'ai_block.hint_example': 'Example: {{example}}',

  // ── Import Projects Modal ──────────────────────────────────────
  'import_projects.title': 'Import Projects',
  'import_projects.select_desc': 'Select a shelly-projects-YYYYMMDD.json file.',
  'import_projects.select_file': 'Select file',
  'import_projects.validating': 'Checking...',
  'import_projects.invalid_json': 'Invalid JSON format. Please select a shelly-projects format file.',
  'import_projects.read_error': "Couldn't read the file. Please try again.",
  'import_projects.import_error': 'An error occurred during import. Your data was not changed.',
  'import_projects.total': 'Total',
  'import_projects.new': 'New',
  'import_projects.duplicate': 'Duplicate',
  'import_projects.duplicate_method': 'How to handle duplicates',
  'import_projects.strategy_skip': 'Skip',
  'import_projects.strategy_overwrite': 'Overwrite',
  'import_projects.strategy_keep_both': 'Keep both',
  'import_projects.strategy_skip_desc': "Don't change existing projects",
  'import_projects.strategy_overwrite_desc': 'Update existing projects with new data',
  'import_projects.strategy_keep_both_desc': 'Append (2) to the name and keep both',
  'import_projects.import_btn': 'Import',
  'import_projects.importing': 'Importing...',
  'import_projects.success_title': 'Import complete!',
  'import_projects.summary_added': 'Added',
  'import_projects.summary_updated': 'Updated',
  'import_projects.summary_skipped': 'Skipped',
  'import_projects.summary_failed': 'Failed',
  'import_projects.close': 'Close',
  'import_projects.error_title': "Couldn't load",
  'import_projects.retry': 'Try again',

  // ── Fullscreen Terminal ────────────────────────────────────────
  'fullscreen.connecting': 'Connecting...',
  'fullscreen.connected': 'Connected',
  'fullscreen.error': 'Error',
  'fullscreen.disconnected': 'Disconnected',
  'fullscreen.close': 'Close',
  'fullscreen.hint': 'Ctrl+C: Interrupt  Tab: Complete  ↑↓: History',

  // ── Sidebar / File Tree ────────────────────────────────────────
  'time.seconds_ago_short': '{{count}}S AGO',
  'time.minutes_ago_short': '{{count}}M AGO',
  'time.hours_ago_short': '{{count}}H AGO',
  'time.days_ago_short': '{{count}}D AGO',
  'sidebar.tasks': 'TASKS',
  'sidebar.running': 'RUNNING',
  'sidebar.log': 'LOG',
  'sidebar.scheduled': 'SCHEDULED',
  'sidebar.agents': 'MY AGENTS',
  // 2026-08-24 (Fable5 design consult, "一人の相棒" Phase 3, G2-P2): a
  // non-collapsible group label — same visual treatment as the existing
  // MY AGENTS/RUNNING subheaders inside TASKS — separating the developer/
  // IDE sections (Quick Launch through Profiles) from the companion-
  // relevant ones above (Tasks, Skills, Imported Skills). Purely a visual
  // grouping cue; every section below it is still its own independent,
  // fully-functional accordion, nothing merged or hidden.
  'sidebar.group_developer': 'DEVELOPER',
  'sidebar.agent_manual': 'manual',
  'sidebar.agent_autonomous': 'autonomous',
  'sidebar.agent_paused': 'paused',
  // Runtime approval now defaults to ON (Fable5 review 2026-08-25, reversing
  // the 2026-07-14 "defaults to OFF" directive) — this agent's EFFECTIVE
  // approval mode (per-agent override, else the global default) must stay
  // visible either way, since an agent can still opt out.
  // 2026-07-24 (DEFERRED.md §対応方針(3)): "no-approval" alone reads as "no
  // approval needed at all", but this setting only covers the notify-dispatch
  // approval layer — a SEPARATE capability-broker boundary-crossing approval
  // (e.g. reading /sys/class/power_supply) can still prompt even when this is
  // off. Clarified in-label rather than adding new UI.
  'sidebar.agent_approval_auto': 'runs automatically (dispatch only)',
  'sidebar.agent_approval_manual': 'approval required',
  'sidebar.agent_last': 'Last',
  'sidebar.agent_next_run': 'Next run',
  'sidebar.agent_last_run': 'Last run',
  'sidebar.agent_last_error': 'Error',
  'sidebar.agent_never_run': 'Not run yet',
  'sidebar.agent_missed_run': 'Scheduled run at {{when}} was not recorded — it may have been blocked (Doze / battery / OEM). Check the battery-optimization exemption.',
  // P1-B (2026-07-15 scheduling-reliability audit): exact-alarm special
  // access (SCHEDULE_EXACT_ALARM, Android 12+) can be revoked at any time
  // after registration — this surfaces that LATER revocation, not just the
  // one-time schedule-readiness nudge shown at registration.
  'sidebar.agent_exact_alarm_missing': 'Exact-alarm scheduling is not granted — this schedule may drift under battery restrictions. Grant it in Settings.',
  'sidebar.agent_route': 'Route',
  'sidebar.agent_route_guard': 'Guard',
  'sidebar.agent_route_keyword': 'Keyword',
  'sidebar.agent_route_secret': 'Secret guard',
  'sidebar.agent_route_no_cloud': 'Cloud fallback disabled',
  'sidebar.agent_route_score': 'Scores ({{confidence}}% conf)',
  'sidebar.agent_steps': 'Steps ({{count}}):',
  'sidebar.agent_route_why': 'Why',
  'sidebar.agent_memory_title': 'Memory ({{count}} notes)',
  'sidebar.agent_memory_view': 'Memory',
  'sidebar.agent_view_runs': 'View Run History',
  'sidebar.agent_view_runs_a11y': 'View run history for {{name}}',
  'sidebar.agent_memory_view_a11y': 'View memory for {{name}}',
  'sidebar.agent_memory_empty': 'No saved memories',
  'sidebar.agent_more_actions_a11y': 'More actions for {{name}}',
  'sidebar.agent_more_actions_subtitle': 'History, memory, and autonomous mode',
  'sidebar.autonomous_toggle_menu_on': 'Turn AUTO On',
  'sidebar.autonomous_toggle_menu_off': 'Turn AUTO Off',
  'sidebar.skills': 'SKILLS',
  'sidebar.skill_empty': 'No saved skills yet',
  'sidebar.skill_uses': '{{count}}× used',
  'sidebar.skill_detail_a11y': 'Open skill {{name}}',
  'sidebar.skill_delete': 'Delete skill',
  'sidebar.skill_delete_a11y': 'Delete skill {{name}}',
  'sidebar.skill_save_title': 'Save as skill?',
  'sidebar.skill_save_body': 'This run of "{{name}}" succeeded. Save it as a reusable skill?',
  'sidebar.skill_save_yes': 'Save skill',
  'sidebar.skill_save_delete': 'Delete skill',
  'sidebar.skill_saved_title': 'Skill saved automatically',
  'sidebar.skill_saved_body': '“{{name}}” was saved after an unattended run.',
  'sidebar.skill_save_failed_title': 'Could not save skill',
  'sidebar.skill_improve_title': 'Improve this skill?',
  'sidebar.skill_improve_body': '"{{name}}" succeeded after an earlier failure. Add this lesson to the skill?\n\n{{note}}',
  'sidebar.skill_improve_yes': 'Improve skill',
  'sidebar.skill_improve_revert': 'Undo improvement',
  'sidebar.skill_improved_title': 'Skill improved automatically',
  'sidebar.skill_improved_body': '“{{name}}” learned from an unattended run (a resolved failure was added as a caution).',
  'sidebar.skill_improve_failed_title': 'Could not improve skill',
  'sidebar.imported_skills_title': 'IMPORTED SKILLS',
  'sidebar.imported_skills_empty': 'No imported skills yet',
  'sidebar.imported_skill_detail_a11y': 'Open imported skill {{name}}',
  'sidebar.imported_skill_source': 'Source',
  'sidebar.imported_skill_warnings': 'Warnings',
  'sidebar.imported_skill_files': 'Bundled files',
  'sidebar.imported_skill_files_none': '(none)',
  'sidebar.skill_approve': 'Approve',
  'sidebar.skill_reject': 'Reject',
  'sidebar.skill_remove': 'Remove',
  'sidebar.skill_approved_toast': 'Skill approved',
  'sidebar.skill_action_failed_title': 'Skill action failed',
  'sidebar.skill_action_failed_generic': 'Something went wrong. Try again.',
  'sidebar.skill_import_button': '+ IMPORT SKILL',
  'sidebar.skill_import_placeholder': '~/my-skill or /sdcard/Download/my-skill',
  'sidebar.skill_import_confirm': 'Import',
  'sidebar.skill_import_success': "Imported '{{name}}' — pending review",
  'sidebar.skill_import_pick_file': 'Or pick a SKILL.md file (no permission needed)',
  'sidebar.skill_import_pick_file_a11y': 'Pick a SKILL.md file to import',
  'sidebar.skill_catalog_browse_button': '⌄ BROWSE CATALOG',
  'sidebar.skill_catalog_title': 'SKILL CATALOG',
  'sidebar.skill_catalog_loading': 'Loading catalog…',
  'sidebar.skill_catalog_empty': 'The catalog has no skills right now.',
  'sidebar.skill_catalog_unavailable': 'Skill catalog is not available yet.',
  'sidebar.skill_catalog_retry': 'Retry',
  'sidebar.skill_catalog_add': 'Add',
  'sidebar.skill_catalog_added': 'Added',
  'sidebar.skill_catalog_add_a11y': 'Add catalog skill {{name}}',
  'sidebar.agent_run_now': 'Run now',
  'sidebar.agent_set_notification_trigger': 'Set Notification Trigger',
  'sidebar.agent_edit_notification_trigger': 'Edit Notification Trigger',
  'sidebar.agent_notification_trigger_modal_title': 'Notification Trigger — {{name}}',
  'sidebar.agent_pause': 'Pause',
  'sidebar.agent_resume': 'Resume',
  'sidebar.agent_pause_a11y': 'Pause agent {{name}} (stops its schedule)',
  'sidebar.agent_resume_a11y': 'Resume agent {{name}}',
  'sidebar.agent_detail_a11y': 'Open {{name}} details',
  // RUNNING sub-section + save-path detail (Fable5 UX consultation, 2026-07-21)
  'sidebar.agent_running_step': 'Running · STEP {{step}}/{{total}} · {{tool}} · {{elapsed}}',
  'sidebar.agent_running_plain': 'Running · {{elapsed}}',
  'sidebar.agent_running_stalled': 'No response for {{elapsed}} — stop it?',
  'sidebar.agent_stop': 'Stop',
  'sidebar.agent_stop_a11y': 'Stop agent {{name}}',
  'sidebar.agent_stop_failed_title': 'Could not stop agent',
  'sidebar.agent_stop_failed_body': 'Could not stop "{{name}}". It may have already finished.',
  'sidebar.agent_saved_path': 'Saved to: {{path}}',
  'sidebar.agent_saved_path_open': 'Open',
  'agent_detail.task': 'Task',
  'agent_detail.contract': 'Authority & Approval',
  'agent_detail.reliability': 'Reliability',
  'agent_detail.memory': 'Memory',
  'agent_detail.steps': 'Steps',
  'agent_detail.routing': 'Routing',
  'sidebar.repositories': 'REPOSITORIES',
  'sidebar.file_tree': 'FILE TREE',
  'sidebar.device': 'DEVICE',
  'sidebar.ports': 'PORTS',
  'sidebar.profiles': 'PROFILES',
  'sidebar.collapse': 'Collapse',
  'sidebar.directory_not_found_title': 'Directory not found',
  'sidebar.directory_not_found_body': 'The path "{{path}}" does not exist on this device. Double-check the spelling or pick an existing folder.',
  'sidebar.directory_not_git_title': 'Not a git repository',
  'sidebar.directory_not_git_body': 'The path "{{path}}" exists but doesn\'t contain a .git folder. Pick a folder that\'s already a git repository.',
  'sidebar.agent_failed_title': 'Agent failed',
  'sidebar.agent_failed_body': 'Could not start "{{name}}".',
  'sidebar.agent_already_running_toast': '"{{name}}" is already starting or running',
  'sidebar.agent_completed_title': 'Agent completed',
  'sidebar.no_log_preview': 'No log preview available.',
  'sidebar.tool': 'Tool',
  'sidebar.show_result_a11y': 'Show result for {{name}}',
  'sidebar.run_agent_now_a11y': 'Run agent {{name}} now',
  'sidebar.stop_all': 'STOP ALL',
  'sidebar.resume_all': 'RESUME',
  'sidebar.stop_all_a11y': 'Stop all agents (global kill-switch)',
  'sidebar.resume_all_a11y': 'Resume all agents',
  'sidebar.pause_agent_a11y': 'Pause agent {{name}}',
  'sidebar.resume_agent_a11y': 'Resume agent {{name}}',
  'sidebar.delete_agent_title': 'Delete agent',
  'sidebar.delete_agent_body': 'Delete "{{name}}"?',
  'sidebar.delete_agent_failed': 'Delete failed',
  'sidebar.delete_agent_a11y': 'Delete agent {{name}}',
  'sidebar.agent_update_failed_title': 'Could not update agent',
  'sidebar.autonomous_toggle_a11y': 'Toggle autonomous mode for {{name}}',
  'sidebar.autonomous_tool_restricted_title': 'Autonomous requires Codex or Local',
  'sidebar.autonomous_tool_restricted_body': '{{tool}} uses an API-key backend. Autonomous mode can run only Codex OAuth or Local LLM. Switch this agent to Codex?',
  'sidebar.autonomous_use_codex': 'Use Codex',
  'sidebar.tasks_empty': 'Nothing yet — just tell Shelly what to remember or when to check in, and it\'ll show up here.',
  'sidebar.agent_capabilities_a11y': 'What can agents do?',
  'agent_capabilities.title': 'What Can Agents Do?',
  'agent_capabilities.intro': 'Say what you want in plain language and Shelly registers a background agent that runs on the device, by itself — even with the screen off. Here is the real capability surface, and a few utterances you can copy straight into an AI pane.',
  'agent_capabilities.capabilities_heading': 'Capabilities',
  'agent_capabilities.examples_heading': 'Try it',
  'agent_capabilities.copy': 'Copy',
  'agent_capabilities.copied': 'Copied',
  'agent_capabilities.copy_a11y': 'Copy example: {{utterance}}',
  'sidebar.codex_sessions': 'CODEX SESSIONS',
  'sidebar.codex_sessions_empty': 'No Codex sessions yet.',
  'sidebar.codex_resume_a11y': 'Resume Codex session {{name}}',
  'sidebar.codex_resume_failed_title': 'Could not resume Codex',
  'sidebar.codex_resume_failed_body': 'No terminal session is available for the resume command.',
  'sidebar.codex_resume_failed_terminal_busy_body': 'No shell-ready terminal is available. Return an existing Codex terminal to the shell or close a terminal tab, then try again.',
  'sidebar.codex_resume_failed_terminal_cap_body': 'The terminal session limit has been reached. Close a terminal tab, then try again.',
  'sidebar.codex_resume_failed_layout_full_body': 'There is no room to open a Terminal pane. Close a pane, then try again.',
  'sidebar.no_repositories': 'No repositories yet. Tap + ADD REPOSITORY to browse your code.',
  'sidebar.remove_repository_title': 'Remove repository',
  'sidebar.remove_repository_body': 'Remove "{{name}}" from the sidebar? This does not delete the files on disk.',
  'sidebar.ports_unavailable': 'Listener detection unavailable on Android 10+ (SELinux).',
  'quick_launch.title': 'QUICK LAUNCH',
  'quick_launch.launch_a11y': 'Launch {{name}} in a new terminal pane',
  'modal.back_from': 'Back from {{title}}',
  'modal.close': 'Close {{title}}',
  'pane.terminal.title': 'Terminal',
  'pane.ai.title': 'AI Chat',
  'pane.agent_chat.title': 'Agent Chat',
  'pane.browser.title': 'Browser',
  'pane.markdown.title': 'Markdown',
  'pane.preview.title': 'Preview',
  'pane.ask.title': 'Ask Shelly',
  'pane.agent_runs.title': 'Agent Runs',
  'pane.terminal.header': 'Terminal',
  'pane.ai.header': 'AI',
  'pane.agent_chat.header': 'Agent Chat',
  'pane.browser.header': 'Browser',
  'pane.markdown.header': 'Markdown',
  'pane.preview.header': 'Preview',
  'pane.ask.header': 'Ask',
  'pane.agent_runs.header': 'Agent Runs',
  'pane.memory_workbench.title': 'Memory',
  'pane.memory_workbench.header': 'Memory Workbench',
  'pane.memory_workbench.search_placeholder': 'Search notes by text or tag...',
  'pane.memory_workbench.section_agent': 'Agent notes — {{name}} ({{count}})',
  'pane.memory_workbench.section_global': 'Shared notes — all agents ({{count}})',
  'pane.memory_workbench.section_companion': 'Companion journal ({{count}})',
  'pane.memory_workbench.empty_agent': 'No saved notes for this agent',
  'pane.memory_workbench.empty_global': 'No shared notes',
  'pane.memory_workbench.empty_companion': 'No journal notes yet. Switching providers automatically records what your conversation established.',
  'pane.memory_workbench.no_results': 'No notes match the search',
  'pane.memory_workbench.tags_label': 'Tags (comma-separated)',
  'pane.memory_workbench.save': 'Save',
  'pane.memory_workbench.cancel': 'Cancel',
  'pane.memory_workbench.show_full': 'Show full text',
  'pane.memory_workbench.collapse': 'Collapse',
  'pane.memory_workbench.delete_title': 'Delete note?',
  'pane.memory_workbench.delete_body': 'This permanently removes the saved note. It cannot be undone.',
  'pane.memory_workbench.delete_confirm': 'Delete',
  'pane.memory_workbench.delete_failed': 'Failed to delete the note.',
  'pane.memory_workbench.save_failed': 'Failed to save the note.',
  'pane.memory_workbench.edit_a11y': 'Edit note',
  'pane.memory_workbench.delete_a11y': 'Delete note',
  'pane.memory_workbench.refresh_a11y': 'Reload notes',
  'pane.change_type_a11y': 'Change pane type',
  'pane.switch_agent_a11y': 'Switch agent',
  'pane.switch_agent': 'Switch Agent',
  'pane.split_a11y': 'Split pane',
  'pane.maximize_a11y': 'Maximize pane',
  'pane.close_a11y': 'Close pane',
  'pane.split_pane': 'Split Pane',
  'pane.split_right': 'Split Right',
  'pane.split_down': 'Split Down',
  'pane.open_in_new': 'Open In New Pane',
  'pane.empty_title': 'NO PANES OPEN',
  'pane.empty_subtitle': 'Add a pane to get started',
  'pane.add_pane': 'ADD PANE',
  'pane.layout': 'LAYOUT',
  'pane.select_tab': 'Select Tab',
  'pane.add_terminal_tab_a11y': 'Add terminal tab',
  // ── Agent Runs pane (components/panes/AgentRunsPane.tsx) ──────────
  'agent_runs.header_scoped': 'Runs — {{name}}',
  'agent_runs.clear_filter': 'All agents',
  'agent_runs.refresh': 'Refresh run history',
  'agent_runs.empty_title': 'No run history yet',
  'agent_runs.empty_body': 'Background agent runs appear here once an agent has run at least once.',
  'agent_runs.runs_count': '{{count}} runs',
  'agent_runs.toggle_detail_a11y': 'Show run details',
  'agent_runs.status_success': 'Success',
  'agent_runs.status_error': 'Error',
  'agent_runs.status_skipped': 'Skipped',
  'agent_runs.status_unavailable': 'Unavailable',
  'agent_runs.age_just_now': 'just now',
  'agent_runs.age_minutes': '{{count}}m ago',
  'agent_runs.age_hours': '{{count}}h ago',
  'agent_runs.age_days': '{{count}}d ago',
  'agent_runs.age_absolute': '{{when}}',
  'agent_runs.section_route': 'Routing',
  'agent_runs.section_output': 'Output',
  'agent_runs.section_error': 'Error',
  'agent_runs.section_steps': 'Steps',
  'agent_runs.section_actions': 'Actions',
  'agent_runs.route_route': 'Route',
  'agent_runs.route_tool': 'Tool',
  'agent_runs.route_guard': 'Guard',
  'agent_runs.route_why': 'Why',
  'agent_runs.route_keyword': 'Keyword',
  'agent_runs.route_secrets': 'Secrets',
  'agent_runs.route_no_cloud': 'No cloud fallback',
  'agent_runs.route_score': 'Score',
  'agent_runs.value_yes': 'Yes',
  'agent_runs.no_output': '(no output captured)',
  'agent_runs.saved_path': 'Saved to: {{path}}',
  'agent_runs.action_save_skill': 'Save as skill',
  'agent_runs.action_rerun': 'Re-run',
  'agent_runs.action_undo': 'Undo',
  'agent_runs.action_undo_unavailable': 'Undo (n/a)',
  'agent_runs.undo_unavailable_hint':
    'Undo is only available for AI Pane chat results, which keep a git savepoint. A background agent run keeps no rollback handle, so its side effects cannot be reverted from here.',
  'agent_runs.rerun_failed_title': 'Re-run failed',
  'agent_runs.rerun_failed_body': '{{name}} could not be started: {{error}}',
  'agent_runs.section_gate_decisions': 'Gate decisions',
  'agent_runs.gate_decisions_loading': 'Loading gate decisions…',
  'agent_runs.gate_decisions_empty': 'No gate decisions recorded for this run.',
  'agent_runs.gate_decision_toggle_a11y': 'Show gate decision reason',
  'agent_runs.gate_decision_allow': 'Allowed',
  'agent_runs.gate_decision_deny': 'Denied',
  'agent_runs.gate_decision_gray': 'Escalated',
  'agent_runs.gate_decision_reason': 'Reason: {{reason}}',
  'agent_runs.gate_decision_signals': 'Signals: {{signals}}',
  'agent_chat.title': 'Agent Chat',
  'agent_chat.read_only': 'READ ONLY',
  'agent_chat.reply_ready': 'READY',
  'agent_chat.reply_locked': 'LOCKED',
  'agent_chat.refresh_a11y': 'Refresh Agent Chat',
  'agent_chat.resume_selected_a11y': 'Resume selected Codex session in Terminal',
  'agent_chat.interrupt_selected_a11y': 'Interrupt the bound Codex terminal',
  'agent_chat.session_tab_a11y': 'Select Codex session {{name}}',
  'agent_chat.rename_session_a11y': 'Rename Codex session {{name}}',
  'agent_chat.rename_session_title': 'Rename Codex session',
  'agent_chat.rename_session_body': 'Choose a sidebar title for "{{name}}".',
  'agent_chat.rename_session_placeholder': 'Session title',
  'agent_chat.dismiss_session_a11y': 'Hide Codex session {{name}}',
  'agent_chat.dismiss_session_title': 'Hide Codex session',
  'agent_chat.dismiss_session_body': 'Hide "{{name}}" from Agent Chat? The Codex JSONL file stays on disk.',
  'agent_chat.loading': 'Loading Codex sessions...',
  'agent_chat.empty_title': 'No Codex session observed',
  'agent_chat.empty_body': 'Start Codex in a Terminal pane. Agent Chat will show Scouter and Codex JSONL snapshots here.',
  'agent_chat.empty_events_title': 'No chat events yet',
  'agent_chat.empty_events_body': 'Scouter sees this Codex session. New prompts and replies will appear after the JSONL watcher receives message events.',
  'agent_chat.no_session': 'No Codex session is mapped yet.',
  'agent_chat.session_fallback': 'Codex',
  'agent_chat.model': 'Model {{model}}',
  'agent_chat.tokens': '{{count}} tokens',
  'agent_chat.last_event': 'Updated {{time}}',
  'agent_chat.phase1_hint': 'Phase 1 preview: messages are read-only. Replies, approvals, and PTY binding are not enabled yet.',
  'agent_chat.phase3_hint': 'Phase 3 preview: PTY binding is shown. Replies and approvals are still disabled.',
  'agent_chat.phase4_hint': 'Phase 4 preview: replies, interrupts, and approval decisions target the bound foreground Codex PTY.',
  'agent_chat.operational_hint': 'Replies, interrupts, and approval decisions target the bound foreground Codex PTY.',
  'agent_chat.reply_placeholder': 'Reply to Codex...',
  'agent_chat.reply_locked_placeholder': 'Open or resume the bound Codex terminal to reply',
  'agent_chat.reply_no_session_placeholder': 'No Codex session selected',
  'agent_chat.reply_input_a11y': 'Codex reply text',
  'agent_chat.send_reply_a11y': 'Send reply to Codex',
  'agent_chat.reply_not_ready_title': 'Codex is not ready',
  'agent_chat.reply_not_ready_body': 'Open or resume the bound Codex terminal, then try again after it is idle.',
  'agent_chat.reply_failed_body': 'Shelly could not write to the bound Codex terminal.',
  'agent_chat.interrupt_failed_title': 'Could not interrupt Codex',
  'agent_chat.interrupt_failed_body': 'Shelly could not interrupt the bound Codex terminal.',
  'agent_chat.resume_notice_pending': 'Resuming Codex session...',
  'agent_chat.resume_notice_queued': 'Resume command sent to Terminal. Wait for Codex to reconnect, then reply here.',
  'agent_chat.resume_notice_focused': 'Bound Codex terminal focused.',
  'agent_chat.interrupt_notice_pending': 'Interrupting the bound Codex terminal...',
  'agent_chat.interrupt_notice_sent': 'Interrupt sent to the bound Codex terminal.',
  'agent_chat.reply_notice_sent': 'Reply sent to Terminal. Waiting for Codex to respond...',
  'agent_chat.reply_notice_observed': 'Reply appeared in the Codex log.',
  'agent_chat.reply_status_checking': 'Checking the bound Codex terminal...',
  'agent_chat.reply_status_ready': 'Ready to send replies to the bound Codex terminal.',
  'agent_chat.reply_status_empty_message': 'Type a reply before sending.',
  'agent_chat.reply_status_no_session': 'Select a Codex session first.',
  'agent_chat.reply_status_not_reliably_bound': 'Open or resume this Codex session to bind a foreground terminal.',
  'agent_chat.reply_status_terminal_missing': 'The bound terminal is missing. Press play to resume this session.',
  'agent_chat.reply_status_terminal_exited': 'The bound terminal exited. Press play to resume this session.',
  'agent_chat.reply_status_busy': 'Codex is still working. Wait until it is idle before replying.',
  'agent_chat.reply_status_screen_unavailable': 'Shelly cannot read the bound terminal screen yet.',
  'agent_chat.reply_status_not_codex_terminal': 'The bound terminal is not showing Codex. Press play to resume this session.',
  'agent_chat.reply_status_codex_launch_failed': 'Codex failed to launch in the bound terminal. Reload or resume with a fresh terminal.',
  'agent_chat.reply_status_interactive_prompt': 'Codex is waiting for a terminal choice. Select an option in Terminal before sending another prompt.',
  'agent_chat.reply_status_no_approval_prompt': 'The bound terminal is not showing an approval prompt yet. Review it in Terminal.',
  'agent_chat.interrupt_status_no_session': 'Select or resume a Codex session before interrupting.',
  'agent_chat.binding_reliable': 'Bound {{pty}}',
  'agent_chat.binding_candidate': 'Candidate {{pty}}',
  'agent_chat.binding_unbound': 'Unbound',
  'agent_chat.error_prefix': 'Agent Chat refresh failed: {{message}}',
  'agent_chat.role_user': 'You',
  'agent_chat.role_assistant': 'Codex',
  'agent_chat.tool_prefix': 'Tool: {{tool}}',
  'agent_chat.approval_title': 'Approval requested',
  'agent_chat.approval_action_hint': 'Review the request, then allow or deny through the bound Codex terminal.',
  'agent_chat.approval_locked_hint': 'Open or resume this Codex session before approving from Agent Chat.',
  'agent_chat.approval_allow': 'Allow',
  'agent_chat.approval_deny': 'Deny',
  'agent_chat.approval_allow_a11y': 'Allow the Codex approval request',
  'agent_chat.approval_deny_a11y': 'Deny the Codex approval request',
  'agent_chat.approval_not_ready_title': 'Approval is not ready',
  'agent_chat.approval_notice_allowed': 'Approval sent to the bound Codex terminal.',
  'agent_chat.approval_notice_denied': 'Denial sent to the bound Codex terminal.',
  'agent_chat.status_idle': 'Idle',
  'agent_chat.status_thinking': 'Thinking',
  'agent_chat.status_tool_running': 'Running tool',
  'agent_chat.status_waiting_input': 'Waiting for input',
  'agent_chat.status_error': 'Error',
  'agent_chat.status_completed': 'Completed',
  'worktrees.title': 'WORKTREES',
  'worktrees.new_title': 'NEW WORKTREE',
  'worktrees.new_subtitle': 'Creates an isolated branch + working copy under ~/.shelly-worktrees/.',
  'worktrees.agent': 'AGENT',
  'worktrees.branch': 'BRANCH',
  'worktrees.branch_hint': 'Existing branches are reused; new ones are created with `-b`.',
  'worktrees.create_failed': 'Create worktree failed',
  'worktrees.remove_title': 'Remove worktree',
  'worktrees.remove_body': 'Remove the "{{branch}}" worktree? The branch itself will not be deleted.',
  'worktrees.removed_warnings': 'Removed with warnings',
  'worktrees.select_repo': 'Select a repository to manage worktrees.',
  'worktrees.empty': 'No worktrees yet. Add one per agent to work in parallel.',
  'profiles.required_title': 'Required',
  'profiles.required_body': 'Name and Host are required.',
  'profiles.back_a11y': 'Back to profiles',
  'profiles.edit_profile': 'Edit Profile',
  'profiles.add_profile': 'Add Profile',
  'profiles.name_required': 'Name *',
  'profiles.host_required': 'Host / HostName *',
  'profiles.port': 'Port',
  'profiles.user': 'User',
  'profiles.identity_file_optional': 'Identity File (optional)',
  'profiles.jump_host_optional': 'Jump Host (optional)',
  'profiles.remove_body': 'Remove "{{name}}"?',
  'profiles.imported_title': 'Imported',
  'profiles.imported_body': 'Added {{count}} profile(s) from ~/.ssh/config',
  'profiles.no_new_title': 'No new profiles',
  'profiles.no_new_body': 'No new hosts found in ~/.ssh/config',
  'profiles.empty': 'No saved profiles',
  'profiles.importing': 'Importing...',
  'profiles.import_from_ssh': 'Import from ~/.ssh/config',
  'recent_logs.title': 'RECENT LOGS',
  'recent_logs.share_title': 'Shelly Terminal Logs',
  'recent_logs.share_failed_title': 'Share failed',
  'recent_logs.share_failed_body': 'Unable to open the share sheet.',
  'recent_logs.copy_a11y': 'Copy recent logs',
  'recent_logs.share_a11y': 'Share recent logs',
  'recent_logs.lines_buffered': '{{count}} lines buffered',
  'recent_logs.no_buffered_logs': 'No buffered logs',
  'llama.status_check': 'Server status check',
  'llama.setup_title': 'llama.cpp Setup',
  'llama.setup_body': 'Install llama.cpp locally.\n\nEstimated time: ~{{minutes}} min\n\nContinue?',
  'llama.start_setup': 'Start setup',
  'llama.setup_aborted': 'Setup aborted.',
  'llama.download_complete': '{{name}} download complete.',
  'llama.download_failed': 'Download failed.',
  'llama.no_startup_output': 'No startup output was returned. Run llama.cpp Setup once, then try Start again.',
  'llama.start_failed': 'Failed to start.',
  'llama.stop_server': 'Stop server',
  'llama.delete_model_title': 'Delete model',
  'llama.delete_model_body': 'Delete {{name}}?',
  'llama.delete_blocked_title': 'Cannot delete model',
  'llama.delete_blocked_body': 'Models cannot be deleted while the server is starting or while the model is active. Stop the server and try again.',
  'llama.setting_up': 'Setting up...',
  'llama.setup_button': 'llama.cpp Setup',
  'llama.running': 'Running',
  'llama.starting': 'Starting',
  'llama.stopped': 'Stopped',
  'llama.unknown': 'Unknown',
  'llama.installed': 'Installed',
  'llama.active': 'Active',
  'llama.start': 'Start',
  'llama.storage_used': 'Storage used: {{size}}GB',
  'llama.model_catalog': 'Model Catalog',
  'llama.recommended': 'Recommended',
  'llama.download': 'Download ({{size}}GB)',
  'file_tree.directory': 'Directory',
  'file_tree.file': 'File',
  'file_tree.rename': 'Rename',
  'file_tree.copy_path': 'Copy path',
  'file_tree.path_copied': 'Path copied',
  'file_tree.delete_title': 'Delete {{name}}?',
  'file_tree.deleted': 'Deleted',
  'file_tree.rm_failed': 'rm failed: {{error}}',
  'file_tree.folder_created': 'Folder created',
  'file_tree.file_created': 'File created',
  'file_tree.create_failed': 'create failed: {{error}}',
  'file_tree.renamed': 'Renamed',
  'file_tree.mv_failed': 'mv failed: {{error}}',
  'file_tree.empty_hint': 'Add a repository above to browse files.',
  'file_tree.search_placeholder': 'SEARCH FILES...',
  'file_tree.new_folder': 'New Folder',
  'file_tree.new_file': 'New File',
  'file_tree.folder_name': 'folder name',
  'file_tree.file_name_placeholder': 'file.ext',

  // ── MCP Section ────────────────────────────────────────────────
  'mcp.approval_title': 'Shelly MCP server: approval needed',
  'mcp.approval_risk_label': 'Risk',
  'mcp.approval_hint': 'This came from an MCP client on your network (e.g. Claude Code), not from you tapping something in Shelly.',
  'mcp.approval_deny': 'Deny',
  'mcp.approval_approve': 'Approve',
  'mcp.title': 'MCP Servers',
  'mcp.no_selection_title': 'No selection',
  'mcp.no_selection_msg': 'No MCP servers are enabled.',
  'mcp.loading_config': 'Loading MCP client settings',
  'mcp.writing_config': 'Writing MCP client settings',
  'mcp.applied_title': 'MCP settings applied',
  'mcp.applied_msg': '{{count}} MCP servers added to client settings.\n\nRestart your MCP client to take effect.',
  'mcp.write_error_title': 'Error',
  'mcp.write_error_msg': 'Failed to write settings file.',
  'mcp.apply_error_title': 'Error',
  'mcp.apply_error_msg': 'An error occurred while applying settings.',
  'mcp.copied_title': 'MCP settings copied',
  'mcp.copied_msg': 'Add the following MCP server block to your client config:\n\nCopied to clipboard ({{count}} servers)',
  'mcp.not_connected_title': 'Not connected',
  'mcp.not_connected_msg': 'Terminal is not ready yet.',
  'mcp.server_install': '{{name}} install',
  'mcp.server_start': '{{name}} start',
  'mcp.server_stop': '{{name}} stop',
  'mcp.server_status_check': '{{name}} status check',
  'mcp.servers_enabled': 'servers enabled',
  'mcp.apply_btn': 'Apply to MCP client',
  'mcp.copy_btn': 'Copy settings',
  'mcp.recommended': 'Recommended',
  'mcp.start': 'Start',
  'mcp.stop': 'Stop',
  'mcp.status_running': 'Running',
  'mcp.status_starting': 'Starting...',
  'mcp.status_error': 'Error',
  'mcp.status_stopped': 'Stopped',
  'mcp.remote_status': 'Remote server (always available)',
  'mcp.npx_status': 'Via npx (auto-start)',
  'mcp.footer': "MCP servers add external context for compatible clients. Copy the generated config into your client settings.",
  'mcp.type_local': 'Local',
  'mcp.type_remote': 'Remote',

  // ── Speech Input ───────────────────────────────────────────────
  'speech.mic_permission': 'Microphone permission is required',
  'speech.recording_error': 'Recording error: {{error}}',
  'speech.file_not_found': 'Recording file not found',
  'speech.transcription_prompt': 'Please transcribe this audio accurately. The audio is likely Japanese or English — detect the language from the audio. Output only the transcribed text, no extra explanation.',
  'speech.transcription_http_error': 'Transcription error: HTTP {{status}}',
  'speech.api_key_required': 'Groq API key is required for speech transcription. Please enter it in Settings.',
  'speech.transcription_error': 'Transcription error: {{error}}',

  // ── AI Dispatch ────────────────────────────────────────────────
  'dispatch.offline_fallback': 'Offline detected — Falling back to local LLM...',
  'dispatch.offline_no_local': 'Offline and unable to connect to local LLM',

  // ── Chat (index.tsx) ───────────────────────────────────────────
  'chat.auto_saved': 'Auto-saved — {{total}} files changed',
  'chat.security_skip': 'Commit skipped\n\n{{labels}}\n\nAPI keys or secrets found in code.',
  'chat.session_ended': '{{agent}} session ended.',

  // ── First Launch MOTD ────────────────────────────────────────────
  'motd.welcome': 'Welcome to Shelly',
  'motd.cli_preinstalled': 'The following CLI tools are pre-installed:',
  'motd.login_prompt': 'Log in with your account to get started:',

  // ── Updates ─────────────────────────────────────────────────────
  'updates.title': 'UPDATES',
  'updates.subtitle': 'Shelly updates',
  'updates.advanced': 'Advanced',
  'updates.refresh': 'Refresh',
  'updates.check_for_updates': 'Check for updates',
  'updates.open_a11y': 'Open updates',
  'updates.show_a11y': 'Show updates',
  'updates.checking': 'Checking for updates...',
  'updates.checking_short': 'Checking...',
  'updates.status_unavailable': 'Update status unavailable',
  'updates.available': 'Update available',
  'updates.latest_status': 'Shelly is up to date',
  'updates.current_version': 'Shelly current v{{versionName}} ({{versionCode}})',
  'updates.current_unavailable': 'Current version unavailable',
  'updates.available_version': 'Shelly available v{{versionName}} ({{versionCode}})',
  'updates.dev_builds': 'Dev builds',
  'updates.dev_status_unavailable': 'Dev build unavailable',
  'updates.dev_available': 'Dev build available',
  'updates.dev_same_status': 'No newer dev build',
  'updates.dev_older_status': 'Dev build is older than installed',
  'updates.dev_available_version': 'Dev available v{{versionName}} ({{versionCode}})',
  'updates.dev_install': 'Install dev',
  'updates.dev_android_confirm': 'Installs the latest workflow_dispatch APK from android-dev. Android will ask you to confirm installation.',
  'updates.dev_same_hint': 'The android-dev APK matches the installed versionCode.',
  'updates.dev_no_release_title': 'No dev APK release',
  'updates.dev_no_release_body': 'The android-dev release is not available yet.',
  'updates.current_codex_version': 'Codex current v{{version}}',
  'updates.current_codex_unavailable': 'Codex current version unavailable',
  'updates.available_codex_version': 'Codex available v{{version}}',
  'updates.codex_latest_status': 'Codex is up to date',
  'updates.codex_available': 'Codex update available',
  'updates.codex_status_unavailable': 'Codex update status unavailable',
  'updates.codex_runtime_source': 'Codex source: {{source}}',
  'updates.codex_runtime_bundled': 'Bundled with APK',
  'updates.codex_runtime_app_data': 'Runtime update',
  'updates.codex_runtime_broken_source': 'Bundled fallback (runtime needs reset)',
  'updates.codex_runtime_broken_status': 'Codex runtime needs attention',
  'updates.codex_update': 'Update Codex',
  'updates.codex_reinstall': 'Repair Codex',
  'updates.codex_next_terminal_hint': 'Open a new terminal tab after changing Codex.',
  'updates.codex_ready_title': 'Codex updated',
  'updates.codex_ready_body': 'The new Codex runtime will be used in newly opened terminal tabs.',
  'updates.codex_install_failed_title': 'Codex update failed',
  'updates.codex_reset': 'Reset',
  'updates.codex_reset_title': 'Codex reset',
  'updates.codex_reset_body': 'New terminal tabs will use the Codex version bundled with the APK.',
  'updates.codex_reset_failed_title': 'Codex reset failed',
  'suggestions.agent.title': 'Agent suggestion',
  'suggestions.agent.message': 'You often work with {{signal}}. Create a helper agent and confirm it in chat.',
  'suggestions.agent.accept': 'Accept',
  'suggestions.agent.dismiss': 'Dismiss',
  'updates.codex_unavailable_title': 'No Codex update',
  'updates.codex_unavailable_body': 'The Codex runtime release is not available yet.',
  'updates.codex_log_title': 'Codex update log',
  'updates.codex_log_prepare': 'Preparing Codex runtime update',
  'updates.codex_log_installing': 'Downloading, verifying, and installing Codex v{{version}}',
  'updates.codex_log_ready': 'Codex runtime is ready',
  'updates.codex_log_error': 'Codex update failed',
  'updates.details_unavailable': 'Update details unavailable',
  'updates.downloading': 'Downloading...',
  'updates.installing': 'Installing...',
  'updates.update': 'Update',
  'updates.latest': 'Latest',
  'updates.unavailable': 'Unavailable',
  'updates.unknown': 'unknown',
  'updates.android_confirm': 'Android will ask you to confirm installation.',
  'updates.no_release_title': 'No public APK release',
  'updates.no_release_body': 'The update release is not available yet.',
  'updates.verify_failed_title': 'Cannot verify installed version',
  'updates.verify_failed_body': 'Shelly could not read the current Android versionCode, so the update was not downloaded.',
  'updates.up_to_date_title': 'Up to date',
  'updates.up_to_date_body': 'Installed versionCode {{current}} is already newer than or equal to available {{available}}.',
  'updates.ready_title': 'Update ready',
  'updates.later': 'Later',
  'updates.install': 'Install',
  'updates.install_failed_title': 'Install failed',
  'updates.download_failed_title': 'Download failed',
  'updates.download_log_title': 'Download log',
  'updates.download_log_elapsed': '{{seconds}}s elapsed',
  'updates.download_log_prepare': 'Preparing download folder',
  'updates.download_log_download': 'Downloading {{name}}',
  'updates.download_log_download_progress': 'Downloading {{name}} - {{progress}}',
  'updates.download_log_paused': 'Download paused - {{progress}} - {{reason}}',
  'updates.download_log_verify': 'Verifying SHA-256',
  'updates.download_log_ready': 'APK is ready',
  'updates.download_log_fallback_direct': 'Android DownloadManager failed ({{reason}}) - retrying with a direct download instead',
  'updates.download_log_error': 'Download failed',
  'updates.download_missing_title': 'Downloaded APK not found',
  'updates.download_missing_body': 'The downloaded APK is missing or failed verification. Download the update again.',
  'updates.download_install_alert_body': 'APK is downloaded. Android will ask you to confirm installation.',
  'updates.download_install_ready_hint': 'APK is downloaded. Tap Install when you return to Shelly.',
  'updates.build_details': 'Build details',
  'updates.loading_log': 'Loading log...',
  'updates.failed_log': 'Failed log',
  'updates.release_source': 'Release source',
  'updates.no_recent_builds': 'No recent builds found.',
  'updates.loading': 'Loading...',

  // ── UI Overhaul (Step 3) ──────────────────────────────────────────
  'agentbar.add_agent': 'ADD AGENT',
  'agentbar.all_agents_enabled': 'All agents enabled',
  'agentbar.enable': 'ENABLE',
  'agentbar.add_or_layout_a11y': 'Add pane or change layout',
  'agentbar.switch_pane_a11y': 'Switch to {{name}} pane',
  'agentbar.show_recent_logs_a11y': 'Show recent logs',
  'agentbar.running_count': '{{count}} RUNNING',
  'sidebar.add_repository': '+ ADD REPOSITORY',
  'sidebar.add_repo_title': 'ADD REPOSITORY',
  'sidebar.add_repo_placeholder': '~/projects/my-repo',
  'sidebar.add_repo_cancel': 'CANCEL',
  'sidebar.add_repo_add': 'ADD',
  'sidebar.cloud_coming_soon': 'Cloud storage integration coming soon. Configure in Settings.',
  'sidebar.cloud_open_settings': 'Open Settings',
  'agent_action_confirm_title': 'Confirm CLI action',
  'agent_action_confirm_body': 'This autonomous agent is requesting a shell command. Review the exact command before allowing it.',
  'agent_action_confirm_command': 'Command',
  'agent_action_confirm_safety': 'Safety',
  'agent_action_confirm_allow': 'Allow',
  'agent_action_confirm_decline': 'Decline',
  'agent_action_confirm_not_ready': 'The action approval request is no longer available.',
  'agent_action_confirm_expired': 'The action approval request has expired.',
  'agent_action_confirm_title_intent': 'Confirm app action',
  'agent_action_confirm_body_intent': 'This autonomous agent is requesting to launch another app or share content. Review the target before allowing it.',
  'agent_action_confirm_intent_mode': 'Mode',
  'agent_action_confirm_intent_target': 'Target',
  'agent_action_confirm_intent_share_text': 'Share text',
  'agent_action_confirm_intent_failed': 'Failed to fire the app action. The request was declined.',
  'agent_action_confirm_webhook_host': 'Destination host',
  'agent_action_confirm_webhook_known_host': 'Known host — previously vetted in Settings. Approval is still required.',
  'agent_action_confirm_webhook_new_host': 'New host — not in your trusted-host list. Review carefully before approving.',
  'agent_action_confirm_webhook_preview': 'Payload preview (first 500 characters)',
  'agent_action_confirm_title_socialpost': 'Confirm social post',
  'agent_action_confirm_body_socialpost': 'This autonomous agent is requesting to publish a post to a connected social account. Once published, this cannot be undone — review the exact text before allowing it.',
  'agent_action_confirm_socialpost_host': 'Destination',
  'agent_action_confirm_socialpost_known_host': 'Known connector — previously vetted in Settings. Approval is still required.',
  'agent_action_confirm_socialpost_new_host': 'New connector — not in your trusted-host list. Review carefully before approving.',
  'agent_action_confirm_socialpost_preview': 'Post text preview',
  'agent_action_confirm_title_browserpane': 'Confirm browser action',
  'agent_action_confirm_body_browserpane': 'This agent is requesting to interact with the currently open Browser Pane page. Review the exact action and the allowed pages before allowing it. This never runs unattended.',
  'agent_action_confirm_browserpane_action': 'Action',
  'agent_action_confirm_browserpane_selector': 'Element (CSS selector)',
  'agent_action_confirm_browserpane_value': 'Value to fill in',
  'agent_action_confirm_browserpane_allowlist': 'Allowed pages',
  'agent_action_confirm_browserpane_failed': 'Failed to run the browser action. The request was declined.',
  'agent_action_confirm_browserpane_no_pane': 'No Browser Pane is currently open. Open one showing the target page, then try again.',
  // 2026-08-05 on-device QA finding: URL mismatch is the most common failure
  // reason, yet the generic "declined" message above gave no clue why
  // (DEFERRED.md 2026-08-05 QA sweep bug 4). Browser-pane actions only ever
  // act on whatever page is CURRENTLY shown — they never navigate there for
  // you — so the fix is a page the user has to take themselves.
  'agent_action_confirm_browserpane_failed_url_mismatch': "The Browser Pane's current page doesn't match this agent's allowed pages. This action never navigates for you — open the target page in the Browser Pane yourself, then try again.",
  'agent_action_confirm_browserpane_extracted_title': 'Extracted text',
  'agent_action_confirm_browserpane_extracted_empty': '(The matched element had no text.)',
  'tts_code_block_omitted': 'Code block omitted.',
  'tts_truncated_suffix': ' The rest is omitted.',
  'ai_pane_images_require_gemini': 'Image attachments need the Gemini provider — switch this pane to Gemini (or add a Gemini API key in Settings), then send again.',
  'ai_pane_image_default_prompt': 'Describe this image.',
  'ai_pane_stop_streaming': 'Stop',

  // ── Code Block Actions (Nacre Bridge slice 1: AI Pane → Terminal insert) ──
  'code_block.copy_label': 'COPY',
  'code_block.insert_label': 'INSERT',
  'code_block.apply_label': 'APPLY',
  'code_block.copy_a11y': 'Copy code',
  'code_block.insert_a11y': 'Insert into terminal',
  'code_block.apply_a11y': 'Apply to staged file',
  'code_block.copied_toast': 'Copied to clipboard',
  'code_block.inserted_toast': 'Inserted into terminal',
  // Shown when no Terminal pane was open/live, so a new one was opened and
  // the text queued for it instead — see CodeBlockWithAction's handleInsert.
  'code_block.inserted_new_pane_toast': 'Opened a new terminal and queued the insert',
  'code_block.insert_failed_toast': 'Insert failed: {{error}}',
  'code_block.wrote_file_toast': 'Wrote {{path}}',
  'code_block.apply_failed_toast': 'Apply failed: {{error}}',
};

export default en;
