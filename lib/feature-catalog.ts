...
export const FEATURE_CATALOG: Feature[] = [
  ...
  { id: 'agent-action-social-post', name: 'Agent Action: Social Post', description: 'Posts the agent run result to a connected social/chat platform via a registered connector — Discord, Slack, Telegram, Mastodon, Misskey, WordPress, or Bluesky (set up under Settings → Social Connectors). Say "Blueskyに投稿して" / "post this to Discord" and the NL parser resolves the platform + your registered connector automatically. Each platform is one agent per connector today — simultaneous multi-platform posting from a single agent is not yet supported. X/Twitter posting goes through Agent Action: App-Act instead, not this connector system.', category: 'agent' },
