...
/**
 * Environment info for the Issue body. Both fields are best-effort —
 * the draft still posts fine if these resolve to `(unknown)`.
 *
 *   shellyVersion  — package.json version (bundled in the APK).
 *   bashrcVersion  — contents of $HOME/.bashrc_version (generated by
 *                    HomeInitializer on every launch), read via the
 *                    same execCommand bridge we use for the token.
 */
export async function getEnvInfo(): Promise<{ shellyVersion: string; bashrcVersion: string }> {
  let shellyVersion = '(unknown)';
  try {
    // Bundled via Metro — Shelly's package.json lives at repo root.
    const pkg = require('../package.json');
    if (typeof pkg?.version === 'string') shellyVersion = pkg.version;
  } catch {}

  let bashrcVersion = '(unknown)';
  try {
    const r = await execCommand('cat "$HOME/.bashrc_version" 2>/dev/null', 3000);
    const v = r.stdout.trim();
    if (r.exitCode === 0 && v) bashrcVersion = v;
  } catch {}

  return { shellyVersion, bashrcVersion };
}
