...
""".trimIndent()
    ...
    // 227: re-point $HOME/bin/bash symlink (libbash.so -> shelly_shell native
    ...
    // 228: Restore claude(): re-add the Bun polyfill heredoc + a minimal
    //      single-tier claude() launcher (install @2.1.112 -> ~/.shelly-cli,
    //      run cli.js on bundled node with LD_PRELOAD=libexec_wrapper.so +
    //      CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=0). Chunk 2 of CC re-enable.
    ...
    fun initialize(context: Context): File {
        ...
        if (!bashrc.exists() || currentVersion < BASHRC_VERSION) {
            ...
            // v227: Claude Code re-enable. Minimal single-tier claude(): install
            // @2.1.112 (last npm release shipping cli.js) into ~/.shelly-cli on
            // first run, then run cli.js on the bundled bionic node with the
            // proven launcher env from before 3095aa47. Crucial bits:
            //   - LD_PRELOAD=libexec_wrapper.so so node's child_process.spawn
            //     (Claude's Bash-tool snapshot shell) is redirected through
            //     linker64 — confirmed on-device 2026-06-12 (raw execve of an
            //     app-data binary is EACCES under Knox; the interposer rewrites
            //     it to `linker64 <bin>`).
            //   - CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=0 so that redirect (and
            //     LD_LIBRARY_PATH) propagate to the snapshot bash and its
            //     children instead of being stripped by CC.
            //   - NODE_OPTIONS requires the node-compat preload (platform spoof)
            //     and the Bun polyfill (globalThis.Bun.* that cli.js calls).
            //   - BASH_ENV=/ENV= empty so the non-interactive snapshot bash does
            //     not source unexpected rc files.
            sb.appendLine("claude() {")
            sb.appendLine("  local __cli_js=\"\$HOME/.shelly-cli/node_modules/@anthropic-ai/claude-code/cli.js\"")
            sb.appendLine("  local __bun_tmp=\"\${BUN_TMPDIR:-\$HOME/.bun-tmp}\"")
            sb.appendLine("  local __claude_tmp=\"\${CLAUDE_TMPDIR:-\$HOME/.claude-tmp}\"")
            sb.appendLine("  __shelly_mkdir -p \"\$__bun_tmp\" \"\$__claude_tmp\" \"\$HOME/.shelly-cli\" 2>/dev/null || true")
            sb.appendLine("  if [ ! -f \"\$__cli_js\" ]; then")
            sb.appendLine("    echo '[shelly] Installing Claude Code @2.1.112 (first run, needs network)...' >&2")
            sb.appendLine("    if ! npm_config_os=linux npm_config_cpu=arm64 npm_config_libc=musl _run $libDir/node $libDir/node_modules/npm/bin/npm-cli.js install --prefix \"\$HOME/.shelly-cli\" --force --omit=optional --no-save '@anthropic-ai/claude-code@2.1.112' >&2; then")
            sb.appendLine("      echo 'claude: install failed (first run needs network)' >&2; return 1")
            sb.appendLine("    fi")
            sb.appendLine("  fi")
            sb.appendLine("  [ -f \"\$__cli_js\" ] || { echo 'claude: cli.js missing after install' >&2; return 1; }")
            sb.appendLine("  local __opts=\"--require=\$__shelly_node_compat_preload --require=\$__shelly_claude_node_preload\"")
            sb.appendLine("  [ -n \"\$NODE_OPTIONS\" ] && __opts=\"\$NODE_OPTIONS \$__opts\"")
            sb.appendLine("  USE_BUILTIN_RIPGREP=0 DISABLE_AUTOUPDATER=1 DISABLE_INSTALLATION_CHECKS=1 CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=0 SHELL=\"\$HOME/bin/bash\" BASH=\"\$HOME/bin/bash\" SHELLY_LIB_DIR=\"$libDir\" PATH=\"\$HOME/bin:$libDir:\${PATH:-/system/bin:/vendor/bin}\" LD_LIBRARY_PATH=\"$libDir\" TMPDIR=\"\${TMPDIR:-\$HOME/.tmp}\" BUN_TMPDIR=\"\$__bun_tmp\" CLAUDE_CODE_TMPDIR=\"\${CLAUDE_CODE_TMPDIR:-\$__claude_tmp}\" CLAUDE_TMPDIR=\"\$__claude_tmp\" NODE_OPTIONS=\"\$__opts\" LD_PRELOAD=\"$libDir/libexec_wrapper.so\" BASH_ENV= ENV= _run $libDir/node \"\$__cli_js\" \"\$@\"")
            sb.appendLine("}")
