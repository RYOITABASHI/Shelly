...
""".trimIndent()
    ...
    private const val BASHRC_VERSION = 153
    ...
    fun initialize(context: Context): File {
        ...
        if (!bashrc.exists() || currentVersion < BASHRC_VERSION) {
            ...
            sb.appendLine("      const onExit = typeof spawnOptions.onExit === 'function' ? spawnOptions.onExit : null;")
            ...
            sb.appendLine("          normalized.onExit(subprocess, code ?? 0, signal || null, null);")
            sb.appendLine("        });")
            ...
            sb.appendLine("          const isMode = function(v) { return v === 'inherit' || v === 'ignore' || v === 'pipe'; };")
            sb.appendLine("          const map = function(v, fallback) { return isMode(v) ? v : fallback; };")
            sb.appendLine("          spawnOptions.stdio = [map(spec.stdin, 'ignore'), map(spec.stdout, 'pipe'), map(spec.stderr, 'pipe')];")
            sb.appendLine("        }")
            sb.appendLine("        if (spec.shell !== undefined) spawnOptions.shell = spec.shell;")
            sb.appendLine("      }")
            sb.appendLine("      if (spawnOptions.stdio === undefined && (")
