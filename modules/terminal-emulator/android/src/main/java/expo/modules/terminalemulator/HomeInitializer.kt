...
function patchCodex(libDir) {
  ...
  const replacement = 'spawn("/system/bin/linker64", ["' + libDir + '/codex_tui", ...process.argv.slice(2)] /*shelly-codex*/';
...
""".trimIndent()
     ...
     *        in jniLibs. The sed patch now replaces the upstream musl
     *        binaryPath with Shelly's bundled native Codex binary via
     *        linker64 — same as node/git/bash. No proot, no rootfs.
     ...
     *        curl/node/Codex/python stop failing HTTPS because
    ...
    //     through the native Codex dispatcher. Fixes bug #114
    //     "codex-termux has no interactive mode" — the TUI binary was always
    //     shipped in the npm tarball but CI was only copying codex-exec.bin
    //     into jniLibs.
    ...
    //      Codex binary path so the wrapper can rewrite
    ...
    // 203: Make Codex current_exe() see the real selected Codex binary path
    //      by overriding /proc/self/exe readlink in the Codex-scoped
    ...
    // v238: SHELLY_HELPER_SHIM v2 — `shelly config` (typed in the real,
    // native PTY terminal) previously fell through to the shim's `usage()`
    // (it only recognized the `scouter` subcommand), so it exited without
    // ever telling the RN layer to open ConfigTUI — see docs/superpowers/
    // DEFERRED.md's 2026-08-06/07 on-device QA entry. lib/pseudo-shell.ts's
    // `config`/`workflow`/`voice` handling is a SEPARATE JS-side
    // implementation only reachable from the legacy block-terminal's
    // runCommand() (store/terminal-store.ts) — the native PTY terminal never
    // calls it, by design (NativeTerminalView writes straight to the PTY
    // fd). `config` now appends to `$HOME/.shelly-command-queue`, drained by
    // app/_layout.tsx the same way the deep-link queue is (shell can't
    // `am start` an Activity under Knox, so a polled file is the only
    // shell->RN bridge). `workflow`/`voice` still fall through to
    // scouter-only usage() — they need real command results returned to the
    // terminal, not just a one-shot UI toggle, so are left as a follow-up.
    // v239: v238's own regeneration guard for $HOME/bin/shelly used `! -f`
    // (missing-file only) — a device whose shim had gone 0 bytes (existing
    // file, no SHELLY_HELPER_SHIM marker inside an empty file) never
    // re-triggered regeneration on any later bashrc rebuild, including
    // v238's own rollout: v238 shipped correctly in .bashrc's
    // BASHRC_VERSION sentinel, but the shim itself silently stayed the
    // stale 0-byte file underneath it on an affected device (found
    // on-device: both `shelly config` and `shelly scouter status` were
    // silent no-ops). Switched the guard to `! -s` (missing OR empty) —
    // see that condition's own comment for why this still preserves the
    ...
    fun initialize(context: Context): File {
        ...
        // binary (libcodex_tui.so). Nothing in the
        ...
        //      default set.
        //   3. Bundled bionic `codex_tui` (ChatGPT subscription auth via
        ...
        if (!bashrc.exists() || currentVersion < BASHRC_VERSION) {
            ...
            // `! -f` only catches a MISSING file — a device whose
            // `$HOME/bin/shelly` had gone 0 bytes (an interrupted write, an
            // old crash, etc.) has a file that both exists (`-f` true) and
            // has no SHELLY_HELPER_SHIM marker to match (grep on an empty
            // file always finds nothing) — this condition was permanently
            // false for that device, so the shim NEVER regenerated again on
            // any future bashrc rebuild (found on-device: `shelly config` /
            // `shelly scouter status` both silently no-op'd against a 0-byte
            // file untouched since an old build). `! -s` (missing OR empty)
            // still preserves the original safety intent — a real custom
            // script the user dropped at this path (non-empty, no marker) is
            // still left alone — while an empty file is unambiguously not a
            // meaningful custom script, so it's safe to treat like a missing
            // one and regenerate.
            sb.appendLine("if [ ! -s \"\$HOME/bin/shelly\" ] || __shelly_grep -q SHELLY_HELPER_SHIM \"\$HOME/bin/shelly\" 2>/dev/null; then")
            sb.appendLine("__shelly_cat > \"\$HOME/bin/shelly\" <<'SHELLY_HELPER_EOF'")
            sb.appendLine("#!/system/bin/sh")
            sb.appendLine("# SHELLY_HELPER_SHIM v3 — generated by Shelly")
            ...
            sb.appendLine("  }")
            sb.appendLine("}")
            sb.appendLine("if (args[0] === 'config') {")
            sb.appendLine("  try {")
            sb.appendLine("    fs.appendFileSync(path.join(home, '.shelly-command-queue'), 'config\\n');")
            sb.appendLine("    console.log('[shelly] Opening settings…');")
            sb.appendLine("    process.exit(0);")
            sb.appendLine("  } catch (e) {")
            sb.appendLine("    console.error('shelly: failed to signal config open: ' + (e && e.message ? e.message : e));")
            sb.appendLine("    process.exit(1);")
            sb.appendLine("  }")
            sb.appendLine("}")
            // v240: `install` needs a real RESULT printed back to the terminal
            // (success/failure text), not just a one-shot UI toggle like
            // `config` above — the exact gap v238's own comment called out as
            // the reason `workflow`/`voice` were left unhandled. Same
            // shell->RN bridge shape as `config` (append to
            // .shelly-command-queue, drained by app/_layout.tsx's
            // drainCommandQueue), but round-trip: this script blocks
            // (synchronous Atomics.wait poll, no external `sleep` binary
            // dependency) on a per-request result file the RN layer writes
            // once lib/optional-pack-installer.ts's installOptionalPack()
            // resolves. See docs/superpowers/DEFERRED.md's 2026-08-25
            // on-device QA entry for why this exists (the original
            // lib/pseudo-shell.ts `install` handler is unreachable from this
            // native PTY terminal, same as `config` was before v238).
            sb.appendLine("if (args[0] === 'install') {")
            sb.appendLine("  const packId = args[1];")
            sb.appendLine("  if (!packId || packId === 'list') {")
            sb.appendLine("    console.log('Usage: shelly install <pack-id>');")
            sb.appendLine("    console.log('');")
            sb.appendLine("    console.log('Available packs:');")
            ...
            sb.appendLine("  const resultDir = path.join(home, '.shelly-install-results');")
            sb.appendLine("  const resultPath = path.join(resultDir, reqId + '.json');")
            sb.appendLine("  try {")
            sb.appendLine("    fs.appendFileSync(path.join(home, '.shelly-command-queue'), 'install:' + reqId + ':' + packId + '\\n');")
            sb.appendLine("  } catch (e) {")
            sb.appendLine("    console.error('shelly: failed to queue install request: ' + (e && e.message ? e.message : e));")
            sb.appendLine("    process.exit(1);")
            sb.appendLine("  }")
            sb.appendLine("  console.log(\"Installing pack '\" + packId + \"'…\");")
            sb.appendLine("  const deadlineMs = Date.now() + 20 * 60 * 1000;")
            sb.appendLine("  const sab = new SharedArrayBuffer(4);")
            sb.appendLine("  const waitFlag = new Int32Array(sab);")
            sb.appendLine("  while (Date.now() < deadlineMs) {")
            sb.appendLine("    if (fs.existsSync(resultPath)) {")
            sb.appendLine("      let result;")
            sb.appendLine("      try {")
            sb.appendLine("        result = JSON.parse(fs.readFileSync(resultPath, 'utf8'));")
            sb.appendLine("      } catch (e) {")
            sb.appendLine("        console.error('shelly: failed to read install result: ' + (e && e.message ? e.message : e));")
            sb.appendLine("        process.exit(1);")
            sb.appendLine("      }")
            sb.appendLine("      try { fs.unlinkSync(resultPath); } catch (e) {}")
            sb.appendLine("      if (result.ok) {")
            sb.appendLine("        (result.lines || []).forEach(function (l) { console.log(l); });")
            sb.appendLine("        process.exit(0);")
            sb.appendLine("      } else {")
            sb.appendLine("        console.error(result.error || (\"install: pack '\" + packId + \"' failed\"));")
            sb.appendLine("        process.exit(1);")
            sb.appendLine("      }")
            sb.appendLine("    }")
            sb.appendLine("    Atomics.wait(waitFlag, 0, 0, 1000);")
            sb.appendLine("  }")
            sb.appendLine("  console.error(\"shelly: install '\" + packId + \"' timed out waiting for a result\");")
            sb.appendLine("  process.exit(1);")
            sb.appendLine("}")
            ...
            sb.appendLine("if [ \"\${SHELLY_DISABLE_APP_DATA_CODEX_RUNTIME:-0}\" != \"1\" ] && [ -f \"\$HOME/.shelly-runtime/codex/current/.healthy\" ] && [ -f \"\$HOME/.shelly-runtime/codex/current/manifest.json\" ] && [ -x \"\$__runtime_tui\" ]; then")
            ...
            sb.appendLine("  fi")
            sb.appendLine("  __shelly_codex_run_tui \"\$__tui\" exec \"\$@\"")
            sb.appendLine("  exit \$?")
            sb.appendLine("}")
            ...
            // codex_tui all fail HTTPS because Android's system trust
            ...
            // codex: v0.142.2+ codex-termux ships one unified binary. Route
            // `codex exec ...` by passing the exec subcommand to codex_tui,
            // matching upstream's codex-exec shell wrapper. Keeping a single
            // selected binary path also keeps SHELLY_CODEX_EXEC_PATH aligned
            // with /proc/self/exe and fs-helper self-relaunch.
            sb.appendLine("__shelly_codex_runtime_ready() {")
            sb.appendLine("  [ \"\${SHELLY_DISABLE_APP_DATA_CODEX_RUNTIME:-0}\" != \"1\" ] && [ -f \"\$HOME/.shelly-runtime/codex/current/.healthy\" ] && [ -f \"\$HOME/.shelly-runtime/codex/current/manifest.json\" ] && [ -x \"\$HOME/.shelly-runtime/codex/current/codex_tui\" ]")
            sb.appendLine("}")
            sb.appendLine("codex() {")
            ...
            sb.appendLine("    esac")
            sb.appendLine("  done")
            sb.appendLine("  # All public Codex entry points go through the unified codex_tui binary.")
            sb.appendLine("  # `exec` needs option injection after the subcommand, so that branch")
            sb.appendLine("  # removes the user's dispatch token and re-adds it before prepared args.")
            ...
            sb.appendLine("        __shelly_codex_run_tui \"\$__chosen_tui\" exec \"\${__codex_args[@]}\"")
            sb.appendLine("        return \$?")
            sb.appendLine("      else")
            sb.appendLine("        echo \"codex: codex_tui binary missing at \$__tui — update or reinstall Shelly\" >&2")
            ...
            sb.appendLine("  local __stamp __dir __log __tui __runtime_tui __chosen_tui")
            ...
            sb.appendLine("    /system/bin/toybox ls -l \"$libDir/codex_tui\" \"$libDir/libc++_shared.so\" \"$libDir/node\" 2>&1 || true")
            ...
            sb.appendLine("    echo \"[codex-smoke] T1 direct codex_tui --version\"")
            ...
            sb.appendLine("    echo \"[codex-smoke] T2 direct codex_tui exec --help\"")
            sb.appendLine("    SHELLY_LD_LIBRARY_PATH=\"\${__chosen_tui%/*}:$libDir\" _run \"\$__chosen_tui\" exec --help | sed -n '1,40p'")
            ...
            sb.appendLine("    echo \"[codex-smoke] result wrapper=\$__rc0 tui_version=\$__rc1 tui_exec_help=\$__rc2 auth=\$__rc4\"")
            ...
            sb.appendLine("  local __stamp __dir __log __tui __runtime_tui __chosen_tui __nonce __prompt __out __rc __matched")
            sb.appendLine("  __stamp=\$(date -u +%Y%m%dT%H%M%SZ 2>/dev/null || date +%s)")
            sb.appendLine("  __dir=\"\$( __shelly_debug_dir 2>/dev/null || echo /sdcard/Download/shelly-debug )\"")
            sb.appendLine("  mkdir -p \"\$__dir\" 2>/dev/null || true")
            ...
            sb.appendLine("  __runtime_tui=\"\$HOME/.shelly-runtime/codex/current/codex_tui\"")
            sb.appendLine("  __chosen_tui=\"\$__tui\"")
            sb.appendLine("  __shelly_codex_runtime_ready && __chosen_tui=\"\$__runtime_tui\"")
            ...
            sb.appendLine("    echo \"[codex-canary] tui=\$__chosen_tui\"")
            ...
            sb.appendLine("  local __stamp __dir __log __tui __runtime_tui __chosen_tui __nonce __work __prompt __out __rc __file_rc __matched")
            sb.appendLine("  __stamp=\$(date -u +%Y%m%dT%H%M%SZ 2>/dev/null || date +%s)")
            sb.appendLine("  __dir=\"\$( __shelly_debug_dir 2>/dev/null || echo /sdcard/Download/shelly-debug )\"")
            sb.appendLine("  mkdir -p \"\$__dir\" 2>/dev/null || true")
            ...
            sb.appendLine("    echo \"[codex-edit-canary] tui=\$__chosen_tui\"")
            ...
            sb.appendLine("  local __stamp __dir __log __tui __runtime_tui __chosen_tui __nonce __work __prompt __out __rc __readme_rc __notes_rc")
            sb.appendLine("  __stamp=\$(date -u +%Y%m%dT%H%M%SZ 2>/dev/null || date +%s)")
            sb.appendLine("  __dir=\"\$( __shelly_debug_dir 2>/dev/null || echo /sdcard/Download/shelly-debug )\"")
            sb.appendLine("  mkdir -p \"\$__dir\" 2>/dev/null || true")
            ...
            sb.appendLine("    echo \"[codex-repo-canary] tui=\$__chosen_tui\"")
            ...
            sb.appendLine("  local __stamp __dir __log __tui __runtime_tui __chosen_tui __nonce __work __work_canon __work_alias __work_canon_alias __prompt __out __rc __readme_rc __helper_error __patch_rejected __edit_tool __shell_tool")
            sb.appendLine("  __stamp=\$(date -u +%Y%m%dT%H%M%SZ 2>/dev/null || date +%s)")
            sb.appendLine("  __dir=\"\$( __shelly_debug_dir 2>/dev/null || echo /sdcard/Download/shelly-debug )\"")
            sb.appendLine("  mkdir -p \"\$__dir\" 2>/dev/null || true")
            ...
            sb.appendLine("    echo \"[codex-patch-canary] tui=\$__chosen_tui\"")
            ...
            sb.appendLine("    __out=\$(cd \"\$__work_canon\" && HOME=\"\$HOME\" CODEX_HOME=\"\${CODEX_HOME:-\$HOME/.codex}\" TERM=\"\${TERM:-xterm-256color}\" TMPDIR=\"\${TMPDIR:-\$HOME/.tmp}\" PATH=\"\$HOME/bin:$libDir:\${PATH:-/system/bin:/vendor/bin}\" timeout 240 /system/bin/env SHELLY_LIB_DIR=\"$libDir\" SHELLY_CODEX_EXEC_PATH=\"\$__chosen_tui\" SHELLY_CODEX_PROC_EXE_SHIM=1 SHELLY_CODEX_PROC_EXE_OPEN_SHIM=1 LD_PRELOAD=\"$libDir/libexec_wrapper.so\" LD_LIBRARY_PATH=\"\${__chosen_tui%/*}:$libDir\" /system/bin/linker64 \"\$__chosen_tui\" exec --sandbox danger-full-access --cd \"\$__work_canon\" --add-dir \"\$__work_canon\" --add-dir \"\$__work\" --add-dir \"\$__work_alias\" --add-dir \"\$__work_canon_alias\" --skip-git-repo-check \"\$__prompt\" 2>&1)")
            sb.appendLine("    __rc=\$?")
            sb.appendLine("    printf '%s\\n' \"\$__out\" | sed -n '1,220p'")
...
- X drafting and short-form reasoning: local Qwen3.5-2B-Q4_K_M first, Codex CLI only when explicitly selected.
...
""".trimIndent() + "\n"
            ...
            writeIfMissing(
                ...
                """
# Qwen3.5-2B-Q4_K_M vs Codex Article Evaluation
...
""".trimIndent(),
                toolJson = """{"type":"local","model":"Qwen3.5-2B-Q4_K_M"}""",
...
""".trimIndent(),
            ...
            seedAgent(
                ...
                description = "Compares Qwen3.5-2B-Q4_K_M local output against Codex CLI for article drafting.",
                prompt = """
Run an A/B article evaluation.

Use the same context and prompt for Qwen3.5-2B-Q4_K_M and Codex. Compare which output is more publishable for the STEAM x AI Substack thesis.

Focus on thesis alignment, source faithfulness, Japanese readability, structure, originality, and publishability.
""".trimIndent(),
                toolJson = """{"type":"ab-article-eval","localModel":"Qwen3.5-2B-Q4_K_M","codexCmd":"codex"}""",
    ...
    private fun migrateContentAgentEnv(envFile: File) {
        ...
        try {
            ...
            val legacyModelPathPattern = Regex("(Qwen3\\.5-(4B|9B)-Q4_K_M|Qwen3-(8B-Q4_K_M|4B-Instruct-2507-Q4_K_M))", RegexOption.IGNORE_CASE)
            val migrated = lines.mapNotNull { line ->
                ...
                when {
                    ...
                    assignment.startsWith("LOCAL_LLM_MODEL_PATH=") && legacyModelPathPattern.containsMatchIn(assignment) -> {
                        null
                    }
                    assignment == "LOCAL_LLM_MODEL=Qwen3-8B" -> {
                        hasModel = true
                        line.replace("LOCAL_LLM_MODEL=Qwen3-8B", "LOCAL_LLM_MODEL=Qwen3.5-2B-Q4_K_M")
                    }
                    assignment == "LOCAL_LLM_MODEL=\"Qwen3-8B\"" || assignment == "LOCAL_LLM_MODEL='Qwen3-8B'" -> {
                        hasModel = true
                        line.replace("LOCAL_LLM_MODEL=\"Qwen3-8B\"", "LOCAL_LLM_MODEL=Qwen3.5-2B-Q4_K_M")
                            .replace("LOCAL_LLM_MODEL='Qwen3-8B'", "LOCAL_LLM_MODEL=Qwen3.5-2B-Q4_K_M")
                    }
                    assignment == "LOCAL_LLM_MODEL=Qwen3-8B-Q4_K_M" ||
                        assignment == "LOCAL_LLM_MODEL=Qwen3.5-4B-Q4_K_M" ||
                        assignment == "LOCAL_LLM_MODEL=Qwen3.5-9B-Q4_K_M" ||
                        ...
                        assignment == "LOCAL_LLM_MODEL='Qwen3-4B-Instruct-2507-Q4_K_M'" -> {
                        hasModel = true
                        line.replace(Regex("LOCAL_LLM_MODEL=(['\"]?)(Qwen3-(8B-Q4_K_M|4B-Instruct-2507-Q4_K_M)|Qwen3\\.5-(4B|9B)-Q4_K_M)\\1"), "LOCAL_LLM_MODEL=Qwen3.5-2B-Q4_K_M")
                    }
            ...
            if (!hasModel) migrated.add("LOCAL_LLM_MODEL=Qwen3.5-2B-Q4_K_M")
    ...
    private val CONTENT_AGENT_VERSION = 5
