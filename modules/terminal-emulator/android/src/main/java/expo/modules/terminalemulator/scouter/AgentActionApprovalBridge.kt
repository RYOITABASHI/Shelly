...
object AgentActionApprovalBridge {
    ...
    // reply files above live under $HOME, which the generated agent script itself
    // (same uid) can read AND write — so `runId`/`requestSha256` equality alone
    // (what this bridge checked before this fix) let a same-uid script forge its
    // own action approval. Mirrors AgentEscalationBridge's fix for the analogous
    // escalation-reply forgery: sign every reply with an Android Keystore RSA key
    // whose private material never leaves the keystore (same-uid file access does
    // NOT grant the ability to produce a valid signature), and the generated bash
    // script verifies that signature (via a bundled node helper, pinned public key
    // sha256 injected as a readonly env var by AgentRuntime.kt) before trusting a
    // reply. Deliberately a SEPARATE keypair/alias from the escalation bridge's
    // (independent objects, independent trust anchors, independent rotation) even
    // though the signing mechanics are identical.
    fun verifierPublicKeyFile(context: Context): File =
        File(context.noBackupFilesDir, "shelly-agent-action-approval-public.der")
    ...
    fun ensureVerifierPublicKey(context: Context): File {
        ...
        if (!out.isFile || !out.readBytes().contentEquals(encoded)) {
            ...
            if (!tmp.renameTo(out)) {
                tmp.delete()
                error("failed to publish action approval verifier public key")
            }
        }
        return out
    }
    ...
    // Signing infrastructure below mirrors AgentEscalationBridge's Keystore-bound
    // RSA signer byte-for-byte in mechanics (same KeyGenParameterSpec shape, same
    // SHA256withRSA/PKCS1 choice), deliberately under its OWN alias
    // (KEY_ALIAS) so the two bridges have independent trust anchors.
    ...
    ): String = listOf(runId, decision, requestTs, requestSha256).joinToString("\n")
    ...
    private const val KEY_ALIAS = "shelly_agent_action_approval_reply_v1"
