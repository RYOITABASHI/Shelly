...
function collect() {
  ...
  return {
    ...
    claude: {
      ...
      npm_version: nodeScriptVersion(path.join(HOME, '.shelly-cli/node_modules/@anthropic-ai/claude-code/cli.js')),
...
function printHuman(d) {
  ...
  line('claude npm (default)', `${mark(d.claude.npm_version.ok)} ${d.claude.npm_version.stdout || d.claude.npm_version.stderr}`);
  ...
  line('claude apk musl', `${mark(d.claude.apk_version.ok)} ${d.claude.apk_version.stdout || d.claude.apk_version.stderr}`);
