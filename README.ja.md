...
| サイドバーの Add Repository の存在チェックとゴーストパス時の Alert | 🟡 チェック自体は実在する(bug #73)が、2026-08-17の実機検証で本来防ぐはずだったghost pathがそのまま再現された。`readDirEntries`は親ディレクトリの読み取りエラー全般(存在しない/権限無し/実際に空——区別不能)で`[]`を返し、`tryAddRepo`はこれを「追加を許容」と誤って解釈するため、存在しない親パスでも(捏造されたバージョンバッジ付きで、本物と見分けがつかない状態で)リポジトリとして登録されてしまう。`.git`の有無も一切チェックしていないため、既存の非gitフォルダも通ってしまう。ghost entryはセッション限定で、`loadRepos()`の`find`ベース再走査により次回アプリ再起動で自己修復する(永続的な状態破損ではない)。修正作業は進行中 |
