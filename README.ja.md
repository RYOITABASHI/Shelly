...
| **通知トリガーのエージェント** | エージェントはスケジュールだけでなく、他アプリ（LINE、Discord、Slackなど、許可リストに入れたもの）が条件に合う通知を出したときにも発火できます。まずパッケージの許可リストで絞り込み、次に送信者名の完全一致でさらに絞ります（未設定なら fail-closed）。トリガーとなったテキストは常に「信頼できないデータ」として注入され、指示としては扱われません。端末内完結でサーバーも外部受信箱のポーリングもありません。 |
...
- **永続メモリ** — エージェントは実行をまたいで小さな事実を書き込み・想起できます(「〜を覚えておいて」)。プレーンなmarkdownノートとしてオンデバイスに保存され、閲覧用にObsidian vaultへもミラーされます。メモリノートに機密情報が含まれると、その実行は他の全実行経路と同じsecret-guardにより強制的にローカル/オンデバイスモデルに限定され、クラウドには一切送られません。
...
- **ユーザープロファイルの学習** — Shelly はあなたのコマンドの傾向と AI の使い方を観察して、提案をパーソナライズします（`lib/user-profile.ts`）。このデータは AsyncStorage に端末内で保持されます。ただし、クラウド AI にメッセージを送るときは、応答品質を上げるためにプロファイルのコンテキストが API リクエストに含まれます。これを無効化する設定トグルは現時点でまだ存在せず、常時動作します。
