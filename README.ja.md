...
Shellyは自分専用の日誌をつけています。会話が終わると自分自身のために要約を書き残し、あなたが覚えておいてと頼んだことはコンパニオン自身とすべてのバックグラウンドエージェントに想起されます——その全てを**Settings → Companion Memory**から閲覧・編集・削除できます。登録済みのエージェントが1つも無い段階でも到達可能です。会話の途中でペインを明示的なプロバイダーへ切り替えても、Shellyは直近のメッセージを一緒に運ぶので、新しいプロバイダーは最初の返信からもうそのスレッドの文脈を踏まえています——同じコンパニオンが、別の脳で考えているだけです。
