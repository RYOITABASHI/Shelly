...
| サイドバーの Add Repository の存在チェックとゴーストパス時の Alert | 🟡 2026-08-17に修正済み(`tryAddRepo`は親ディレクトリの読み取りエラーでfail-openせず対象を直接probeするフォールバックに変更、`.git`エントリ——worktree/submoduleを考慮しdirectoryまたはfileどちらも——の存在確認を追加)、ユニットテスト7件PASS。元のバグ自体は原因まで特定済み(`readDirEntries`が存在しない/権限無し/実際に空を区別せず`[]`を返すのを「追加を許容」と誤解釈、2026-08-17に捏造バージョンバッジ付きghost entryとして再現、アプリ再起動で自己修復することも確認)。ただし修正自体はまだ実機で再検証していない |
