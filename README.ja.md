...
**普通の言葉でスケジュールした自律エージェントが、自分で起きて仕事をする**

https://github.com/user-attachments/assets/ab272ae8-c741-4acb-b71d-2da151f50d75

`@agent` に続けて、やってほしいことを普通の言葉で書くだけ。Shelly はそれを端末上のスケジュール済みエージェントに変換し、自前のアラームで発火させます。この動画では 16:16 に自分でセットし、画面が消えた状態で起き上がり、ユーザー自身の API キーでソースを収集して、一次情報のサマリを Obsidian の vault に書き込んでいます。
...
| **マルチプラットフォーム配信** | エージェントの成果物は通知や Obsidian の下書きに限りません。Bluesky・Discord・Slack・Telegram・Mastodon・Misskey・WordPress に直接投稿したり、Webhook を叩いたりできます。プラットフォームごとのキーを一度保存すれば、あとは普通の言葉でエージェントに向き先を指示するだけ。*Bluesky はライブで端から端まで検証済みです。残り 6 つのコネクタは同じコードパスとテストカバレッジに乗っていますが、それぞれ実アカウントに対して発火させたわけではありません。* |
...
スケジュール指定は繰り返しだけでなく、相対的な*開始時点*も理解します。「来週から毎朝ニュースをチェックして」と言えば、エージェントはいま登録されつつ、最初の発火は解決された日付まで保留され、うっかり明日動き出すことはありません。（登録 / 確認カードのパスは実機で確認済みです。ただし、実際に数日待って遅延発火するところまでを観測できてはいません。）
