...
- **`app.act` のレシピ下書きキャプチャはまだ動作しません** — 設定 → 開発者向け → Automation の「画面をキャプチャしてレシピを下書きする」フローは常に失敗します。原因は、Shelly自身のUI内にある**キャプチャ**ボタンをタップした瞬間に、AndroidのフォアグラウンドウィンドウのフォーカスがShellyに移ってしまい、Accessibility ServiceがLINE/Xの画面を読み取れなくなるためです(実機で確認済み、実際の画面分割レイアウトでも同様)。バンドル済みの2つのレシピ(`line.send-message`、`x.post`)はこのキャプチャUIを必要としないため影響を受けませんが、この2つを超えて新しいレシピを追加するには、このUIが実装される前と同様、レシピJSONを手書きする必要があります。
