...
- **AI CLIs**: **2026-07-15 訂正** — 本行は v5.3.1 時点の記述として書かれたが、`3095aa479`（"chore: remove legacy AI CLI integrations"、2026-05-29）で Claude Code CLI / Gemini CLI・OAuth surface は current main から撤去済み（`HomeInitializer.kt` の `cleanupRemovedCliRuntime()` が起動時に残存 `claude` ランタイムを能動的に削除する）。**現行 main の正式 CLI surface は Codex のみ。** Claude Code CLI の再有効化は `feat/claude-on-device-reenable` ブランチで進行中・未マージ（`claude -p` が deep path で SIGSEGV する既知バグが残っている）。Shelly 管理 updater が staging に取得し、`--version` probe を通したものだけを live/current に昇格する仕組み自体は Codex runtime に対して現役（`~/.shelly-runtime/codex/current` + `$HOME/bin` wrapper）。Gemini CLI は bundle/patched investigation 用の Experimental 扱いで、Worktrees / Quick Launch には出さない。Gemini は AI Pane/background の API provider として使う。
...
- **bash wrapper**: `$HOME/bin/bash` に linker64 経由の shim を配置して `bash -c "…"` や `bash script.sh`（`#!/usr/bin/env bash` shebang付きスクリプト含む）を動作させる（bug #93）。**shebangスクリプトの直接実行（`./script.sh`）はKnoxの`binfmt_script`制限で依然失敗する**（2026-08-17 Fable5実機確認、`/usr/bin/env: bad interpreter: Permission denied` exit 126）— 必ず`bash script.sh`経由で呼ぶこと
- **Terminal pane background**: **2026-08-25 訂正 — 本行は長らく陳腐化していた（2026-08-10 に一度発見されながらドキュメント同期が先送りされ続けた点も含め経緯を残す）。** build 1560–1565 の wallpaper/panel tint グレー化回帰を受け、Terminal pane は当初 opaque black に fail-closed（2026-07-15 早朝 `fb85a0b0c` で a96cdd8a4 由来のバグを再修正）。同日 `61e9bea15`（17:38）で `settings.terminalWallpaperTransparency` という明示オプトイン・トグル（既定 **OFF**）として復元されたが、プロダクトオーナーの直接指示「ターミナルも他のペインと同様に透過させる」を受けた `70ccd1e63`（17:53、既定→true）→ 実機で「白っぽいスモークが残る」との指摘による `85da0d144`（18:20、既定→false）→ その指摘は透過自体の欠陥ではなくネイティブ側の色パレット未更新（`ShellyTerminalView.kt` の transparent-mode 分岐が `mCurrentColors[COLOR_INDEX_BACKGROUND]` を更新していなかった）と切り分けた上でのプロダクトオーナーの明示的な再訂正 `2211047a2`（18:23、既定→**true**、最終確定）と続いた。**現在の HEAD まで `terminalWallpaperTransparency` の既定値は `true` のまま3週間以上変更なし**——Settings の説明文「On by default, matching the other panes」はこの実体と一致しており、矛盾していたのは本 CLAUDE.md の記述の方だった（2026-08-10 Fable5実機②レビュー + CC調査、`docs/superpowers/DEFERRED.md` P1 エントリ「CLAUDE.md「Terminal pane background」節が陳腐化」参照）。ネイティブ色パレットのギャップ自体は解消済みと確認済みだが、`2211047a2` が要求した「first frame/theme apply/new tab/split layout/IME resize/settings modal 背面」6シナリオの実機スクショ証跡は今も未取得（DEFERRED.md P3「Terminal pane wallpaper transparency re-enable」参照、次回オンデバイスQA枠で消化予定）。**この行および `terminalWallpaperTransparency` の既定値を変更する PR は、変更前に DEFERRED.md の該当エントリを読み、上記6シナリオのスクショ証跡を添えること。**
...
| **エージェントのファンアウト・サブタスク (`parallelGroup`)**: ブランチのコンテキスト分離・宣言順集約セマンティクスは両 executor 共通、**wall-clock 並列ディスパッチは無人 PlanSpec executor のみ実装済み**（attended TS チェーンは意図的に直列のまま） | Hermes サブエージェント相当の「分解→各サブタスクを同一コンテキストから処理→集約」を、ブランチのコンテキスト分離（兄弟出力をプロンプト/重複検知から遮断）+宣言順集約+最終ステップ除外+`MAX_PARALLEL_BRANCHES=3` cap として両 executor に実装 (2026-08-13)。その後 Increment 1a-1c (2026-08-14) で `shelly-plan-executor.js`（無人 PlanSpec executor）側だけ async `spawn`+in-process semaphore+`Promise.allSettled`による実 wall-clock 並列dispatch・budget file lockを実装し、native `CURRENT_EXECUTOR_VERSION`を2へ同期済み（tsc/Jestレベルで検証済み、on-device検証は継続中）。**attended TS チェーン（`agent-manager.ts` `runAgentOrchestratedBody`）は今も意図的に直列のまま**: chain lock の回転 nonce、生成 .sh のグローバル `MAX_CONCURRENT=2`（phantom-process 上限保護）が per-agent 単一飛行を前提にしており、並列化にはその再設計を要する — DEFERRED.md 2026-08-13/2026-08-14 エントリ参照 | agent-orchestration.ts (`planParallelGroups`), agent-manager.ts, shelly-plan-executor.js (+APK asset ミラー), store/types.ts |
