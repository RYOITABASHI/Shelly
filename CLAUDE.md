...
- **Terminal pane background**: Terminal pane は wallpaper 透過対象外。native Canvas / GL terminal surface と Termux default background は opaque black 固定にしている。build 1560–1565 で wallpaper/panel tint がグレー化として露出したため、再有効化は `docs/superpowers/DEFERRED.md` の P3 条件を満たすまで行わない。**2026-06-20 の `a96cdd8a4` がこのP3ゲートを1日違反し（`TerminalPane.tsx` に `wallpaperActive` 分岐を追加）、2026-07-13 の cosmetic-store hydration 修正（`013b2658f`）でその潜在バグが可視化され、2026-07-15 に `TerminalPane.tsx`/`SettingsDropdown.tsx` の両方で opaque 固定へ再修正・実機不整合を解消済み。再発防止のため、この行を変更する PR は必ず P3 チェックリストのスクショ証跡を要求すること。**
