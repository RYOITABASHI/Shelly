...
| **エージェントのファンアウト・サブタスク (`parallelGroup`) はセマンティクスのみ、ディスパッチは直列** | Hermes サブエージェント相当の「分解→各サブタスクを同一コンテキストから処理→集約」を、ブランチのコンテキスト分離（兄弟出力をプロンプト/重複検知から遮断）+宣言順集約+最終ステップ除外+`MAX_PARALLEL_BRANCHES=3` cap として両 executor に実装 (2026-08-13)。**wall-clock 並列ディスパッチは意図的に無し**: 両 executor は per-agent 単一飛行が安全設計の根幹（chain lock の回転 nonce、生成 .sh のグローバル `MAX_CONCURRENT=2`（phantom-process 上限保護）、無人 executor の同期 spawnSync broker）で、並列化はその再設計を要する — DEFERRED.md 2026-08-13 エントリのブロッカー4点参照 | agent-orchestration.ts (`planParallelGroups`), agent-manager.ts, shelly-plan-executor.js (+APK asset ミラー), store/types.ts |
