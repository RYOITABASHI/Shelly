...
description: Draft a platform-appropriate social/chat post before an agent's Social Post action publishes it, instead of posting the raw run output.
---
An agent that ends in a Social Post (Discord/Slack/Telegram/Mastodon/
Misskey/WordPress/Bluesky/X connector) action is one step away from
publishing something publicly and irreversibly. Draft the post itself as a
distinct step, not as a byproduct of the run's raw output:
...
5. **When the content is ambiguous, ironic, or could be misread out of
