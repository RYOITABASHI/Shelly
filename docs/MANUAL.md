...
**By default, you are talking to Shelly.** The local-persona chat presents as one persistent companion named Shelly rather than exposing whichever model answered underneath — replies never carry a provider-name tag, no matter which model actually generated them; if you want to know who's currently bound to the pane, that's shown once in the pane header, not repeated on every message. Agent registration and confirmation happen as a conversation rather than a form. Every AI pane bound to this default persona opens the same continuous conversation, so moving to another pane or split does not start over. A pane explicitly bound to an external provider such as Gemini (via the pane-header agent badge or an `@mention`) keeps its own per-pane history.

**Switching a pane's binding carries a little context with it.** Whether you're moving a pane from the companion to an explicit provider or back again, Shelly copies the last few on-topic messages from the thread you're leaving into the one you're entering, with a small system notice explaining the hand-off. It's meant as a continuity nudge, not a full merge of the two histories — switching back and forth doesn't duplicate messages, and a pane-scoped copy doesn't survive an app restart (only the shared companion thread itself does).

**Shelly can remember a preference for itself and every agent.** In the default companion chat, say something natural such as “remember that my deploy branch is staging.” No all-agents scope marker or special syntax is required. Shelly asks for confirmation before writing the note; once confirmed, that `_global` memory is recalled in later companion conversations and included in every scheduled agent's execution context. (Phrasing like “I prefer …” or “I like …” is separately picked up by Shelly's ambient usage-pattern learning — see [Privacy](../README.md#privacy) — so a sentence combining both can leave two notes instead of one; a plain factual “remember that …” avoids that overlap.)

**Separately, Shelly also keeps a journal on its own — no confirmation needed.** Every time a pane switches away from the companion, Shelly distills what the conversation established into a short note using the on-device local LLM, and saves it to its own `_companion` memory scope. This is companion-only: unlike the `_global` notes above, journal entries are never handed to background agents. Later companion replies only pull back the handful of journal notes relevant to what you're currently asking (ranked by BM25 plus recency, capped at five), so it stays useful instead of turning into a wall of everything you've ever said. You can browse, edit, or delete these notes any time from **Settings → Companion Memory** — that screen is reachable even before you've registered a single agent.

**You can remove one message without clearing the thread.** Long-press any message bubble in the AI pane and confirm **Delete**. Only that individual message is removed from the conversation.
...
**A fenced ` ```bash ` reply block can go straight into your terminal.** It shows an **Insert** button next to **Copy** — tap it and the code lands in the focused Terminal pane's input line (no auto-Enter, so you can look it over before running it). If no terminal pane is open, Shelly opens one and queues the insert for the moment its shell is ready.
...
On a genuinely fresh install, the sidebar opens with Tasks and Skills expanded, while the more developer-facing sections — Repositories, File Tree, Device, Worktrees, Quick Launch, Codex Sessions — start collapsed under a single **DEVELOPER** divider. Nothing is removed or hidden away permanently; it's one tap to open any of them. If you've already got Shelly installed and have your own sections open or closed the way you like them, this default doesn't touch your existing choices.
...
Send that, and Shelly parses the schedule and the task separately, shows you what it understood, and — once you confirm — registers a real Android alarm. From that point on, nothing needs to be open. When the alarm fires, the phone wakes just enough to run the task and drops the result where you told it to. A notification confirms it ran, and Shelly also says the result in the shared companion conversation, so it is waiting in chat when you open the app later. The same chat re-entry happens after a manual **Run Now**. The agent's own row in the sidebar's Tasks section shows last-run and next-run times, with a missed-run flag in the row's detail view if an expected run didn't happen.
...
**Vague requests get a real back-and-forth, not a single canned question.** If you type `@agent help me out` with no object and no domain, an LLM — not a fixed script — drives the rest of the conversation in its own words, asking one follow-up at a time until it has enough to propose a registration (a fast cloud provider first if you've configured one, then the on-device model, then Shelly's older fixed-question fallback if nothing is reachable). If your answer turns out to describe several ordered steps — "look this up, summarize it, then post it" — it's registered as a real multi-step chain instead of getting flattened into one instruction. This is on by default; toggle it off in **Settings → Agents → LLM-Led Agent Registration** if you'd rather always get the fixed one-field-at-a-time prompts. Either way, nothing is registered until you confirm it, and any value the model proposes is independently re-checked — a schedule phrase still goes through the same deterministic parser, a posting destination still has to match a connector you actually registered.
...
**Output isn't limited to a note** — an agent's result can go to a notification, an Obsidian draft, or out to Bluesky, Discord, Slack, Telegram, Mastodon, Misskey, WordPress, X, or a webhook, configured with that platform's key once. Naming several platforms in one sentence — "post this to Bluesky and X" — can register a single agent that posts to all of them, as long as you have exactly one connector configured for each platform you named; if a named platform has no connector (or more than one), Shelly falls back to whichever platforms it could resolve and tells you which one it dropped. Bluesky is verified against a real account; posting to two platforms from one agent at once is implemented but hasn't yet been fired against a live multi-connector setup. If a single request asks for both — "post this and notify me" — the local notification wins and the post is dropped, on purpose: turning a private notification into a public post is the one direction here you can't take back, so it's the one direction Shelly won't do without a clearer, separate request. (You'll still see a run-completion notification for a posting agent regardless — that's Shelly telling you the agent ran, not the "notify" action type described here.) Unless you've added the destination host to an allowlist, a social post also waits for your approval tap before it actually goes out — it doesn't post unattended by default.

**Successful runs can be remembered and reused.** Put "remember the result" in an agent's instruction and each successful run leaves a note the agent recalls on later runs. Shelly can also distill a successful run into a reusable skill — the steps, provider, and budget that worked — and quietly reuses that plan when you ask for something similar later. Attended runs ask before saving ("Save as skill?"); an unattended run of a remembering agent saves the skill automatically and posts a notification with a one-tap **Delete skill** action instead of interrupting you. There's also a shared layer. An explicit command such as `@agent remember across all agents that I prefer short answers` needs both the remember phrasing and an all-agents phrase. In the default Shelly conversation, ordinary “remember …” wording is enough because the request is already addressed to the companion. Both paths ask for confirmation before writing the shared note that Shelly and every agent recall.

**You don't need the app open to register one, either** — the same `@agent` syntax works from the ASK box on Shelly's home-screen Scouter widget, typed or spoken through its mic button (the recognized text lands in the field for you to review, it never auto-submits). A widget-typed `@agent` command goes through the exact confirmation Shelly would show if you'd typed it in the AI Pane. If you'd rather widget-registered agents skip that confirmation and register immediately — with a notification afterward instead of a prompt beforehand — turn on **Settings → Widget No-Confirm Register** (off by default). It only changes how agents registered *from the widget* are confirmed; typing `@agent` in the AI Pane itself always asks first.
...
**If you use [Nacre](https://github.com/RYOITABASHI/Nacre) as your keyboard, Shelly and it talk to each other.** While Shelly is in the foreground, it writes your current directory, git branch, and a handful of sanitized recent-command terms (never raw commands or secrets) to a small context file that Nacre reads to bias its kana-kanji conversion toward what you're actually doing; the file is deleted the moment Shelly leaves the foreground. On Nacre's own side, its Dev Mode detects that focus is inside Shelly specifically and suppresses auto-punctuation conversion (no more `!=` silently becoming `≠` mid-command), defaulting its symbol panel to a programming tab instead. It's on by default — toggle it off in Settings → Nacre Bridge if you don't use Nacre.
...
**Security, briefly:** Shelly is a normal Android app sandbox, not a hardened VM — terminal commands and approved AI-agent actions run as the app's own uid and can touch whatever the app can touch. API keys are stored through Expo's SecureStore, Keystore-encrypted where the device supports it, and are never written to logs. There's no telemetry — no analytics, no crash reporting, no usage tracking — though note that, while Profile Learning is enabled, messaging a cloud AI provider includes Shelly's on-device usage-pattern profile in that request to improve response quality. You can disable this with **Settings → Agents → Profile Learning**, or erase the learned profile with **Reset Profile**. The full picture is in [README → Security](../README.md#security) and [SECURITY.md](../SECURITY.md).
