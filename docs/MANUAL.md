...
**By default, you are talking to Shelly.** The local-persona chat presents as one persistent companion named Shelly rather than exposing whichever model answered underneath, and agent registration and confirmation happen as a conversation rather than a form. Every AI pane bound to this default persona opens the same continuous conversation, so moving to another pane or split does not start over. A pane explicitly bound to an external provider such as Gemini keeps its own per-pane history; changing a pane's binding adds a small system notice so the handoff is visible.
