...
**Security, briefly:** Shelly is a normal Android app sandbox, not a hardened VM — terminal commands and approved AI-agent actions run as the app's own uid and can touch whatever the app can touch. API keys are stored through Expo's SecureStore, Keystore-encrypted where the device supports it, and are never written to logs. There's no telemetry — no analytics, no crash reporting, no usage tracking — though note that when you message a cloud AI provider, Shelly's on-device usage-pattern profile is included in that request to improve response quality. There's no Settings toggle to turn this off yet — it always runs. The full picture is in [README → Security](../README.md#security) and [SECURITY.md](../SECURITY.md).
