...
- Drafting:
  ...
  - X: local Qwen3.5-2B from Git history, build logs, Obsidian notes, and agent outputs.
  - Local comparison: Qwen3.5-2B vs Codex via the A/B eval agent.
- Local LLM:
  - Agents use the OpenAI-compatible endpoint at `LOCAL_LLM_URL`.
  - For loopback URLs such as `http://127.0.0.1:8080`, agents can auto-start `llama-server` with Qwen3.5-2B when the binary and GGUF model are present.
...
- X agents require `llama-server`, Node, and a Qwen3.5 GGUF model when using local Qwen3.5-2B. Set `LOCAL_LLM_MODEL_PATH` if the model is not in the standard search paths.
