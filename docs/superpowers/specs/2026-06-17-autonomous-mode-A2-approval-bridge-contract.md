...
### B2 config — UPDATED by Phase A driver host E2E `[2026-06-17]`
The initial danger-full-access matrix probe ([2026-06-17-autonomous-mode-A2-B2-danger-full-access-approval-probe.md](./2026-06-17-autonomous-mode-A2-B2-danger-full-access-approval-probe.md)) proved that `approvalPolicy: "on-request"` surfaces `rm -rf`, but the Phase A driver E2E ([2026-06-17-autonomous-mode-B2-agent-driver-phaseA.md](./2026-06-17-autonomous-mode-B2-agent-driver-phaseA.md)) found a larger boundary case: under `on-request`, `echo hi > /tmp/b2drv/a.txt` and `cp /tmp/b2drv/a.txt /tmp/elsewhere/a.txt` ran without `item/commandExecution/requestApproval`. Therefore **B2 must use `approvalPolicy: "untrusted"` for the app-server driver** if the gate must review in-workspace writes and workspace-boundary writes. Keep `sandbox: "danger-full-access"` and `approvalsReviewer: "user"`. `approvalPolicy: "never"` remains forbidden.
